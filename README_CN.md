...
- **官方一方技能** — 由百炼团队开发，配合 [`bailian-cli`](https://www.npmjs.com/package/bailian-cli)（命令别名 `bl`，安装见 [CLI_INSTALL.md](./CLI_INSTALL.md)）使用，覆盖对话、多模态、图像/视频、语音、视觉、应用、记忆、RAG、联网搜索等百炼平台核心能力。
...
| 技能 | 说明 |
|------|------|
| [`bailian-cli`](./skills/bailian-cli/) |  引导 Agent 使用 `bl` 命令完成对话、多模态、图像/视频、语音、视觉、应用、记忆、RAG 与联网搜索；详细命令参考 `reference/`。先装 CLI → [CLI_INSTALL.md](./CLI_INSTALL.md)。 |
