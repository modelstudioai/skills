...
**如何使用本文档：** 每个技能区块包含「来源 → 简介 → 安装 → 使用」四部分。复制安装命令到终端执行，安装完成后将「使用」中的示例话术发给 AI Agent 即可。
...
```bash
npx skills add skills-mcp/skills-mcp -y
```
...
```bash
npx skills add GPTtang/skill-atlas -y
```
...
```bash
npx skills add diao10/skills_repository -y
```
...
```bash
npx skills add mwpgxl/ai-toolkit -y
```
...
```bash
npx skills add ceoniuer/shaoqi-marketplace -y
```
...
```bash
npx skills add huangserva/servasyy_skills -y
```
...
```bash
npx skills add donghuixin/AI-Vibe-Writing-Skills -y
```
...
```bash
npx skills add gushuaialan1/marketing-writer -y
```
...
```bash
npx skills add blacktwist/social-media-skills -y
```
...
```bash
npx skills add gonelake/multi-agent -y
```
...
```bash
npx skills add digoal/blog/tree/master/skills -y
```
...
```bash
npx skills add louis030195/gptzero-mcp -y
```
...
```bash
npx skills add gogf/skills --skill goframe-v2 -y
```
...
```bash
npx skills add davila7/claude-code-templates --skill ui-ux-pro-max -y
```
...
```bash
npx skills add anthropics/skills --skill internal-comms -y
```
...
```bash
# 按需安装单个格式，或一次性安装全部
npx skills add anthropics/skills --skill xlsx -y
...
```bash
npx skills add anthropics/skills --skill doc-coauthoring -y
```
...
```bash
npx skills add JohnKeating1997/spark-video -y
```
...
```bash
npx skills add Shanyin-ai/shanyin-screenwriting-master -y
```
...
```bash
npx skills add affaan-m/everything-claude-code --skill e2e-testing -y
```
...
| 参数 | 说明 |
|------|------|
| `-y` | 跳过交互确认，适合脚本或 Agent 自动安装 |
| `--skill <name>` | 从多技能仓库中只安装指定技能（如 `anthropics/skills`） |
| `--all` | 安装仓库中的全部技能 |
| `-g` | 全局安装（用户级，跨项目可用） |
...
```bash
npx skills add anthropics/skills --skill skill-creator --all -g -y
```

更多参数见 [skills CLI 文档](https://github.com/vercel-labs/skills)。
