...
调用时将 model 参数指定为[支持的模型](https://help.aliyun.com/zh/model-studio/fast-mode#fast-models-h2)的 model ID 即可开启，无需额外参数。接入域名格式为 `https://{workspace_id}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1`，其中 `{workspace_id}`可在[业务空间管理](https://bailian.console.aliyun.com/cn-beijing/settings/workspace)页面切换到对应地域后查看。
