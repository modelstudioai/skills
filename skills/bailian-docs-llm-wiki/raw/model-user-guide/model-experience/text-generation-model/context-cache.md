...
-   [显式缓存](https://help.aliyun.com/zh/model-studio/context-cache#825f201c5fy6o)：需要**主动开启**的缓存模式。需要主动为指定内容创建缓存，以在有效期（5分钟）内实现确定性命中。除了输入 Token 计费，用于创建缓存的 Token 通常按输入 Token 标准单价的 125% 计费，后续命中通常仅需支付 10% 的费用，具体价格见[如何计费](https://help.aliyun.com/zh/model-studio/context-cache#904e3da8aatwp)。
-   [隐式缓存](https://help.aliyun.com/zh/model-studio/context-cache#2317ea09cfxok)：此为自动模式，无需额外配置，且无法关闭，适合追求便捷的通用场景。系统会**自动识别**请求内容的**公共前缀**并进行缓存，但缓存**命中率不确定**。对命中缓存的部分，通常按输入 Token 标准单价的 20% 计费，具体价格见[如何计费](https://help.aliyun.com/zh/model-studio/context-cache#7768a4362ailr)。
...
通常为输入 Token 单价的10%（[详见计费说明](https://help.aliyun.com/zh/model-studio/context-cache#904e3da8aatwp)）

通常为输入 Token 单价的20%（[详见计费说明](https://help.aliyun.com/zh/model-studio/context-cache#7768a4362ailr)）
...
系统将创建首个缓存块，记为 A 缓存块。 2. **发起第二个请求：**发送以下结构的请求：
...
千问 Max：qwen3.8-max、qwen3.8-max-0902、qwen3.7-max、qwen3.7-max-2026-05-20、qwen3.7-max-2026-06-08、qwen3.6-max-preview、qwen3-max
...
千问 Flash：qwen3.8-flash、qwen3.7-flash、qwen3.7-flash-2026-07-15、qwen3.6-flash、qwen3.5-flash、qwen-flash
...
-   **全球服务部署范围：**
    ...
    千问 Flash：qwen3.8-flash、qwen3.7-flash、qwen3.7-flash-2026-07-15、kimi-k2.7-code
...
-   **全球服务部署范围：**
    ...
    千问 Flash：qwen3.8-flash、qwen3.7-flash、qwen3.7-flash-2026-07-15、qwen3.5-flash、qwen-flash
    
    千问 VL：qwen3-vl-plus
    
    千问 Coder：qwen3-coder-plus、qwen3-coder-flash
    
    Kimi：kimi-k2.5、kimi-k2.7-code
...
-   **全球服务部署范围：**
    ...
    千问 Flash：qwen3.8-flash、qwen3.7-flash、qwen3.7-flash-2026-07-15、qwen3.6-flash
...
-   **例外**：qwen3.8-max、qwen3.8-flash、qwen3.8-2.4t-a95b 的显式缓存命中价格不是标准输入单价的 10%，具体价格请参见百炼控制台。
...
-   系统消息（System Message）
    ...
    **说明**若请求包含 `tools` 参数（Function Calling 场景），工具定义会作为系统消息的一部分参与缓存计算。工具定义不支持独立缓存，在工具定义中添加缓存标记会被忽略，缓存标记只能添加在 messages 的 content 中。
...
client = OpenAI(
    api_key=os.getenv("DASHSCOPE_API_KEY"),
    # 以下为华北2（北京）base_url，新加坡地域的base_url为https://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1
    base_url="https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1",
)
...
client = OpenAI(
    api_key=os.getenv("DASHSCOPE_API_KEY"),
    # 以下为华北2（北京）base_url，新加坡base_url为https://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1
    base_url="https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1",
)
...
-   **行业模型**
    -   [数据挖掘](raw/model-user-guide/model-experience/text-generation-model/specialized-models/data-mining-qwen-doc.md)：qwen-doc-turbo
    -   [对话分析](raw/model-user-guide/support/model-studio-model-list/model-list-text-generation/tongyi-xiaomi-analysis-pro.md)：tongyi-xiaomi-analysis-pro、tongyi-xiaomi-analysis-flash
...
-   **文本生成模型**
    ...
    -   千问 Turbo：qwen-turbo
    -   千问 Coder：qwen3-coder-plus、qwen3-coder-flash
    -   千问 Character：qwen-plus-character、qwen-flash-character、qwen-plus-character-ja
    -   千问开源：qwen3.8-2.4t-a95b、qwen3.8-27b
    -   DeepSeek：deepseek-v4.1-flash、deepseek-v4-pro、deepseek-v4-flash、deepseek-v3.2
    -   GLM（阿里云百炼部署）：glm-5.3、glm-5.1
...
-   **全球服务部署范围：**
    ...
    -   文本生成模型
        ...
        -   千问 Coder：qwen3-coder-plus、qwen3-coder-flash
        -   千问 Character：qwen-plus-character
        -   Kimi（阿里云百炼部署）：kimi-k2.5、kimi-k2.7-code
        -   GLM（阿里云百炼部署）：glm-5.3、glm-5.2
    -   视觉理解模型
...
-   **全球服务部署范围：**
    ...
    -   文本生成模型
        ...
        -   千问 Coder：qwen3-coder-plus、qwen3-coder-flash
        -   千问 Character：qwen-plus-character
        -   Kimi（阿里云百炼部署）：kimi-k3、kimi-k2.5、kimi-k2.7-code
        -   GLM（阿里云百炼部署）：glm-5.3、glm-5.2
    -   视觉理解模型
...
-   **全球服务部署范围：**
    ...
    -   文本生成模型
        ...
        -   千问 Character：qwen-plus-character
        -   DeepSeek（阿里云百炼部署）：deepseek-v4.1-flash、deepseek-v4-pro、deepseek-v4-flash
        -   GLM（阿里云百炼部署）：glm-5.3、glm-5.1
        -   Kimi（阿里云百炼部署）：kimi-k3、kimi-k2.5、kimi-k2.7-code
...
> [OpenAI兼容-Batch（文件输入）](raw/model-api-reference/toolkits-and-frameworks/batch-interfaces-compatible-with-openai.md)方式调用无法享受缓存折扣。
...
虽然提问的问题不同，但都基于同一篇文章。相同的系统提示和文章内容构成了大量重复的前缀信息，有较大概率命中缓存。 2. **代码自动补全**

在代码自动补全场景，大模型会结合上下文中存在的代码进行代码自动补全。随着用户的持续编码，代码的前缀部分会保持不变。上下文缓存可以缓存之前的代码，提升补全速度。 3. **多轮对话**
...
随着对话轮数的增加，缓存带来的推理速度优势与成本优势会更明显。 4. **角色扮演或 Few Shot**
...
使用上下文缓存后，即使用户频繁更换询问的产品类型（如从智能手表到笔记本电脑），系统也可以在触发缓存后快速响应。 5. **视频理解**
...
**说明**此处的有效期指通过 API 调用时上下文缓存的生命周期，与**体验中心**的**模型调试**面板中展示的历史对话记录不是同一功能。
...
### Q：如何查看一段时间内（按天/按周）的缓存命中 Token 数量？

A：可通过以下方式获取指定时间段内的缓存命中数据：

-   **模型监控：**在百炼控制台「[模型监控](https://bailian.console.aliyun.com/cn-beijing/model/telemetry)」页面，选择目标模型和时间范围，查看 `cache_tokens` 指标（支持按分钟/按小时精度统计）。
-   **账单明细：**在百炼控制台「账单详情」页面导出账单明细，按计费类型筛选"缓存命中"（`cache`）相关记录，按日期分组汇总，可获取每日的缓存命中 Token 计量数据。详细操作请参见[账单查询与成本管理](raw/model-user-guide/test-1/bill-query-and-cost-management.md)。
