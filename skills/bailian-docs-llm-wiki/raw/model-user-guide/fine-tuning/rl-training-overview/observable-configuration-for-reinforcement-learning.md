...
本篇以 _agentic-rl-example_ 项目（CalcX 计算器场景）为主线，演示从零配置到控制台查看效果的完整流程，并提供指标参考字典、问题排查决策、FAILED 排查流程。训练提交与超参配置请参见 [强化学习训练配置 — 提交与配置](raw/model-user-guide/fine-tuning/rl-training-overview/rl-training-config-monitoring.md)。
