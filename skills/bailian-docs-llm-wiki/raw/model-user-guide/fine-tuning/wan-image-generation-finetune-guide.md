...
-   **适用部署模式及地域**：本文描述的功能仅在华北2（北京）地域可用，且必须使用该地域的[API Key](https://bailian.console.aliyun.com/model/settings/api-key)。
...
**输出图像（微调前-文生图）**![7217b6ac-789d-43c3-aaa5-22647532de52\_0](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9433450871/p1075807.png)

> 无参考图无法生成特定人物形象。

**输出图像（微调后）**![1\_24](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9433450871/p1075796.png)
...
**输入图像**![29\_0-combine](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6448182871/p1084644.webp)

**输出图像（微调前）**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6448182871/p1084667.png)

> 无法通过提示词每次生成固定风格的“末日废土红黑机甲”特效。

**输出图像（微调后）**![29\_1](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6448182871/p1084647.jpg)

> 微调后的模型无需提示词即能复现训练集中的特定“末日废土红黑机甲”特效。

运行下述代码前，请[开通百炼服务](raw/model-api-reference/preparations/get-api-key.md)，并[配置API Key](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)。
...
请将`<替换为训练数据集的文件id>`完整替换为上一步获取的`file_id`。完整参数说明与格式约束请参见[超参数](https://help.aliyun.com/zh/model-studio/wan-generation-finetune-api-reference)。
...
**训练轮数**。模型在训练集上完整迭代的次数。轮数越多模型拟合越充分，但过多可能导致过拟合，且训练 Token 消耗与费用同比增加。
...
**验证间隔轮数**。训练期间每隔多少轮进行一次验证评估，用于阶段性评估模型训练效果。同时保存当前轮次的模型文件。
...
**训练图片的最大分辨率**。取值为字符串档位："1k" 即 1024×1024，"2k" 即 2048×2048。设置训练集中图片分辨率的像素总数（宽×高）上限，系统仅对超过该值的图片进行缩放处理，未超限的图片保持原样。该参数直接影响训练 Token 消耗与费用，请合理设置。
...
**验证图生成分辨率**。训练过程中验证评估时生成图片的目标分辨率。取值同 max\_pixels。
...
**梯度裁剪**。对所有可训练参数做全局梯度范数裁剪的阈值，防止梯度爆炸。
...
**LoRA 低秩矩阵的维数**。该值决定了微调参数量的大小。数值越大，模型拟合能力越强，但训练速度会变慢。
...
**训练集划分比例**。仅在未指定 `validation_datasets` 时生效。此参数用于从训练集中自动按比例拆分出验证集。例如，0.9 表示 90% 训练集，10% 验证集。
...
除了使用本文示例数据体验微调流程外，您也可以构建自己的数据集进行微调。本章节的数据集格式与规模限制对万相和千问图像生成模型**通用**。
...
在[创建微调任务](https://help.aliyun.com/zh/model-studio/wan-generation-finetune-api-reference)时，如果没有单独上传验证集（即未传入`validation_datasets`参数），系统将根据`split`从训练集划分验证集，默认 0.9。即 90% 用于训练，10% 用作验证。
...
1.  **准备验证集**：将验证数据打包成一个独立的 `.zip` 文件，请参见[验证集格式](raw/model-user-guide/fine-tuning/wan-image-generation-finetune-guide.md)。
...
系统将按照[超参数](https://help.aliyun.com/zh/model-studio/wan-generation-finetune-api-reference)`eval_steps`设定的间隔，在**验证集**上运行Checkpoint并生成预览图像。
