...
运行下述代码前，请[获取与配置 API Key](raw/model-api-reference/preparations/get-api-key.md)，并[配置API Key](raw/model-api-reference/preparations/get-api-key.md)。
...
此参数是否生效，取决于[aigc\_config.use\_input\_prompt](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api) 的配置：

-   当`use_input_prompt=true`时：此参数生效。系统将根据这段提示词来生成视频。
    
-   当`use_input_prompt=false`时：此参数会被忽略。系统将使用预置模板[aigc\_config.prompt](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)自动生成提示词。
...
-   当`use_input_prompt=false`时：此参数会被忽略，无需传入。系统将使用预置模板[aigc\_config.prompt](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)自动生成提示词。
...
在[创建微调任务](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)时，如果没有单独上传验证集，即未传入`validation_datasets`参数，系统将根据以下两个[超参数](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)，自动从**训练集**中划分出一部分作为验证集：
...
1.  **准备验证集**：将验证数据打包成一个独立的 `.zip` 文件，请参见[验证集格式](https://help.aliyun.com/zh/model-studio/wan-video-generation-finetune-guide#4a214aadd60df)。
...
系统将按照[超参数](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)`eval_epochs`设定的间隔，在**验证集**上运行Checkpoint并生成预览视频。
...
[视频生成模型微调API参考](https://help.aliyun.com/zh/model-studio/create-fine-tuning-job-api)
