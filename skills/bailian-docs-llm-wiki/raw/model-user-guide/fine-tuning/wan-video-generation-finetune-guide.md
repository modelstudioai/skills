...
-   **支持微调的模型**：
    ...
    -   图生视频-基于首尾帧：wan2.7-i2v（通过设置超参数`task_type`为`kf2v`）、wan2.2-kf2v-flash。
...
--data '{
    ...
    “input”: {
        “media”: [
            {
                “type”: “first_frame”,
                “url”: “https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20260113/typemn/kf2v-first.webp”
            },
            {
                “type”: “last_frame”,
                “url”: “https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20260113/ekzmff/kf2v_last.webp”
            }
        ]
    },
    “parameters”: {
...
多模态素材数组（Wan2.7模型使用）。通过 `type` 和 `url` 指定首帧和尾帧。

-   `type`：素材类型，设置为`first_frame`（首帧）或`last_frame`（尾帧）。
...
wan2.2微调模型支持 480P、720P。默认值为720P。

wan2.7微调模型支持 720P、1080P。默认值为1080P。
...
为了获得质量较高的Prompt，推荐使用[Qwen-VL](raw/model-user-guide/model-experience/vision-model/vision.md)等多模态大模型来辅助生成视频的Prompt。
...
1.  **使用 AI 辅助生成初始描述**
    ...
    2.  固定句式（规范输出）：当有了大概思路后，可基于标注公式设计一套固定句式，引导 AI 生成符合格式的 Prompt。
        ...
        > 代码调用详见[图像与视频理解](raw/model-user-guide/model-experience/vision-model/vision.md)。
