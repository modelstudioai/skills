...
-   [RL 功能开发指南](raw/model-user-guide/fine-tuning/rl-overview/rl-function-development-guide.md)
-   [RL 训练配置与监控](raw/model-user-guide/fine-tuning/rl-overview/rl-training-config-monitoring.md)
-   [强化学习可观测性配置](raw/model-user-guide/fine-tuning/rl-overview/observable-configuration-for-reinforcement-learning.md)
...
按月购买模型训练单元，使用专属训练资源，**训练速度更快，无需排队等待**。 最小计费粒度：**小时**。提前退订按小时单价的 **1.2** 倍计费。 如需购买或管理预付费训练单元，可在[模型调优控制台](https://bailian.console.aliyun.com/cn-beijing/model/tuning)点击**管理训练资源**。
...
如需编写自己的 Rollout 和 Reward 函数，请参见[强化学习开发指南](raw/model-user-guide/fine-tuning/rl-overview/rl-function-development-guide.md)。
...
**说明**也可以通过配置文件或 CLI 提交训练任务（`dashscope rl run`），三种提交方式与字段逐项说明详见 [《训练配置 — 提交与配置》](raw/model-user-guide/fine-tuning/rl-overview/rl-training-config-monitoring.md)。
...
各页签的详细说明请参见[强化学习的可观测配置](raw/model-user-guide/fine-tuning/rl-overview/observable-configuration-for-reinforcement-learning.md)。
...
-   了解函数开发的完整细节 → [强化学习开发指南](raw/model-user-guide/fine-tuning/rl-overview/rl-function-development-guide.md)
-   了解训练配置和监控 → [强化学习训练配置 — 提交与配置](raw/model-user-guide/fine-tuning/rl-overview/rl-training-config-monitoring.md)
-   了解可观测性配置、轨迹查看与训练指标参考 → [强化学习的可观测配置与指标参考](raw/model-user-guide/fine-tuning/rl-overview/observable-configuration-for-reinforcement-learning.md)
