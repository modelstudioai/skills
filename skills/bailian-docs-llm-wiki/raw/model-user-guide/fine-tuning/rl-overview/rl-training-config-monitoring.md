...
-   训练指标速查、13 组指标全集、问题排查（看到指标异常如何处理）、控制台 5 个页签、Tracing 装饰器、自定义指标 → 见基础 [强化学习的可观测配置与指标参考](raw/model-user-guide/fine-tuning/rl-overview/observable-configuration-for-reinforcement-learning.md)
...
-   了解函数开发 → [强化学习开发指南](raw/model-user-guide/fine-tuning/rl-overview/rl-function-development-guide.md)
-   了解可观测性配置与轨迹查看 → [强化学习的可观测配置与指标参考](raw/model-user-guide/fine-tuning/rl-overview/observable-configuration-for-reinforcement-learning.md)
