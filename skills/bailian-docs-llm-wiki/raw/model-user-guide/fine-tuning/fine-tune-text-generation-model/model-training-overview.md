...
当您在尝试如 Prompt 工程、插件调用等优化方法后，模型表现仍然不及预期时，请使用阿里云百炼的模型调优。模型调优作为改进模型表现的核心策略，可以很好地提升模型在特定行业/业务的表现，对齐人类偏好，降低输出延迟。模型调优包含模型微调（SFT）、继续预训练（CPT）、模型偏好训练（DPO）和强化学习（RL）四种模型训练方式。
...
控制台支持三种计费方式：**按 Token 计费**（按训练实际消耗的 Tokens 数量计费）、**训练单元·预付费**（预先购买训练单元按单元扣减）和**训练单元·后付费**（按实际使用量后付费扣减）。通过 API 创建的训练任务仅支持按 Token 计费。

**计费公式**

模型训练费用 = （训练数据 Token 总数 + 混合训练数据 Token 总数）× 循环次数 × 训练单价（最小计费单位：1 token）

> 您可以查看[模型调优控制台](https://bailian.console.aliyun.com/cn-beijing/model/tuning)底部的预估训练费用，并单击**计算详情**，查看训练 Token 总数、循环次数和训练单价**。**
...
-   `h_bar、w_bar`：缩放后的图像长宽，模型在处理图像前会进行预处理，会将图像缩小至特定像素上限内，像素上限与`max_pixels`和`vl_high_resolution_images`参数的取值有关。
...
在[模型调优](https://bailian.console.aliyun.com/cn-beijing/model/tuning)页面可查看所有训练任务。列表展示以下列：任务名称/ID、优先级、基础模型、训练方法、训练状态、计费方式、产出、任务耗时和创建时间。

**优先级**分为 L0、L1、L2、L3 四级，优先级从高到低排列，影响训练任务的调度顺序。优先级越高，训练任务越早被调度执行。

**计费方式**支持三种：按 Token 计费、训练单元·预付费和训练单元·后付费（详见下方计费说明）。

页面右上角提供**训练费用记录**、**资源池管理（预付费）**、**创建训练任务**和**重置**按钮。列表支持按优先级筛选。每行操作列提供**详情**、**停止**、**日志**和**删除**按钮，可用性取决于任务状态。
...
-   文本生成模型调优虽然能在特定业务/场景取得非常好的效果，但有以下限制：
    ...
    -   **费用较高，**调优后的模型部署后才能使用，[模型部署计费](https://help.aliyun.com/zh/model-studio/model-training-and-deployment-billing#0c4e677c48lrp)较高。
...
-   [角色扮演](https://www.aliyun.com/solution/tech-solution-deploy/2978350)
    ...
    ![角色扮演调优效果](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7339071671/p1019093.gif)
-   [目标检测](https://www.aliyun.com/solution/tech-solution-deploy/2978352)
    
    训练前后模型在目标检测任务上的表现对比，详见各模型调优效果基准。
...
1.  登录[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/tuning)，在左侧导航栏选择**模型训练** > **模型调优**，单击**创建训练任务**。
    
    ![创建训练任务](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6609399771/p1075286.png)
2.  在**训练配置**区域完成以下配置：
...
3.  在**超参配置**区域，**参数配置**保持默认即可。百炼已为微调超参提供推荐配置。
    
4.  在**数据配置**区域完成以下配置：
    
    -   **训练集**：选择已上传的调优数据集。数据样例：[SFT-ChatML格式示例.jsonl](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20241014/utjdbx/SFT-ChatML%E6%A0%BC%E5%BC%8F%E7%A4%BA%E4%BE%8B.jsonl)。
    -   **混合训练**：保持关闭。
    -   **验证集**：选择**自动切分**，从训练集中切分 10% 作为验证集。
    
    ![数据配置](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6609399771/p1075287.png)
5.  在**训练产出**区域配置模型参数快照（Checkpoint）的保存方式，以下配置均保持默认即可：
    
    -   **模型名称**：训练完成后，产出的最后一个 Checkpoint 将以该名称自动发布至**我的模型**。
    -   **最多保存**：设置训练任务最终导出的模型个数。
    -   **Checkpoint保存间隔**：可选**按 epoch 计算**或**按 step 计算**。
    
    ![训练产出](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3889372671/p1024332.png)
    
    **说明**模型调优完成后需导出参数快照，导出后才能基于该版本的参数快照在百炼上部署模型。导出的参数快照保存在云存储中，暂不支持访问或下载。
...
SFT/DPO/CPT 各训练方式的数据格式规格、打包规则、大小与数量限制、API 上传配额详见[调优数据上传规则](https://help.aliyun.com/zh/model-studio/text-generation-tuning-data-upload-rules#w80m77fe66000)。各训练方式支持的文件格式概览：
...
如果您缺乏数据，建议构建[智能体应用（Agent 1.0）](raw/application-user-guide/llm-application/single-agent-application.md)，使用知识库索引来增强模型能力。当然在很多复杂的业务场景，可以综合采用模型调优和知识库检索结合的技术方案。
...
2.  使用阿里云百炼的[数据处理](https://bailian.console.aliyun.com/cn-beijing/model/data?tab=data_flow)功能，对您的数据集进行数据清洗、数据增强。
