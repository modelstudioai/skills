...
前往[模型调优](https://bailian.console.aliyun.com/cn-beijing/model/tuning)页面，点击”**创建训练任务**”按钮。
...
各训练方式的数据文件格式详见[调优数据上传规则](https://help.aliyun.com/zh/model-studio/text-generation-tuning-data-upload-rules#w80m77fe66000)。

各训练方式的数据量要求请参见[调优数据上传规则](https://help.aliyun.com/zh/model-studio/text-generation-tuning-data-upload-rules#ao7nn4pajyv00)。

**阿里云百炼推荐您以先 CPT（可选），后 SFT，再 DPO 的顺序使用模型调优：**

1.  先收集海量（至少1000万Token）的特定领域的无标签样本，进行CPT继续预训练，将模型训练成特定行业/领域的专家。
...
字符与 token 之间的关系请参考 [Token和字符串之间怎么换算](https://help.aliyun.com/zh/model-studio/billing-for-model-studio)。取值范围 \[500, 32768\]。
...
-   **CPT**（继续预训练）：仅支持批次大小、学习率、循环次数、验证步数、学习率调整策略、序列长度，不支持 LoRA 相关参数、学习率预热比例和权重衰减。

### 学习率调整策略介绍

**学习率调整策略**（lr\_scheduler\_type）位于**超参配置**区域，单击**展开配置**后可见，共包含 8 种策略：

-   **Linear**：学习率线性递减。适合训练过程较短的任务。
    
    ![plot\_linear](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1007176271/p846167.png)
-   **Polynomial**：学习率按照一个预定义的多项式函数随训练迭代次数或周期数逐渐减少，可以更精细地控制学习率减少速度，适用于任务比较复杂的场景。**但内置多项式系数为 1，曲线与效果同 Linear，不推荐使用。**
    
-   **Cosine**：学习率变化符合余弦函数曲线。适合需要进行精细调整、训练过程较长的任务。
    
    ![plot\_cosine](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1007176271/p847202.png)
-   **Cosine\_with\_restarts**：学习率按照余弦函数的形状周期性地减少至某个最小值，并在每个周期结束时“重启”成预设值，然后开启下一周期。适用于需要模型从局部最优解中跳出来、尝试寻找更好全局解的情况。**但经过实测，学习率并不会在函数曲线底部“重启”成预设值，不推荐使用。**
    
-   **Constant**：学习率不变，**学习率预热比例**参数无效。适合初步探索模型性能。
    
    ![plot\_constant](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1007176271/p846163.png)
-   **Constant\_with\_warmup**：学习率不变，但**学习率预热比例**参数有效。适合初步探索模型性能。
    
    ![plot\_constant\_with\_warmup](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1007176271/p846196.png)
-   **Inverse\_sqrt**：学习率逐渐减小，减小量与迭代次数的平方根的倒数正相关。适合于 SFT 微调，能较好地平衡学习效率与模型收敛。
    
    ![plot\_inverse\_sqrt](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1007176271/p846497.png)
-   **reduce\_lr\_on\_plateau**：当监控的指标（验证损失或验证准确率）在连续多个 epoch 内没有显著改进时，自动降低学习率。当模型很难进一步提高性能时，该策略可以帮助模型继续优化和提升。
...
数据集构建技巧请参考[数据集构建技巧](https://help.aliyun.com/zh/model-studio/model-training-overview#aebd25a7f1g2v)。上传调优数据集请前往[数据管理](https://bailian.console.aliyun.com/cn-beijing/model/data)页面。

调优数据的上传格式规格、打包规则、大小与数量限制详见[调优数据上传规则](https://help.aliyun.com/zh/model-studio/text-generation-tuning-data-upload-rules#ew7a7senrgj00)。
...
在**训练资源配置**区域，选择训练任务的计费方式。控制台支持以下三种计费方式：

-   **按 Token 计费**：使用平台闲时共享资源，按训练实际消耗的 Tokens 数量计费。训练速度取决于资源可用情况，可能存在排队等待。
-   **训练单元·预付费**：预先购买训练单元，按单元扣减，资源有保障，无需排队。适合有稳定训练需求的场景。需先通过**资源池管理（预付费）**购买训练单元。
-   **训练单元·后付费**：使用训练单元资源，按实际使用量后付费扣减。适合有训练需求但不想预先购买的灵活场景。

**说明**详细计费说明和价格请参见[模型调优简介 - 计费说明](raw/model-user-guide/fine-tuning/fine-tune-text-generation-model/model-training-overview.md)。
...
-   **导出数量上限**：通过**最多保存**下拉框设置训练任务最终导出的模型个数。模型每次保存会增加额外的任务时长。
-   **Checkpoint 保存间隔**：设置 Checkpoint 的保存频率，可选**按 epoch 计算**或**按 step 计算**。
...
> 如遇权限不足，请参考：[模型调优时报权限不足怎么办？](https://help.aliyun.com/zh/model-studio/model-training-on-console#0b5443f069rmw)
...
在[模型调优](https://bailian.console.aliyun.com/cn-beijing/model/tuning)页面可查看所有训练任务。列表展示以下列：任务名称/ID、优先级（L0-L3）、基础模型、训练方法、训练状态、计费方式、产出、任务耗时和创建时间。页面右上角提供以下按钮：

-   **训练费用记录**：查看训练任务的费用明细。
-   **资源池管理（预付费）**：管理预付费训练单元资源池。
-   **创建训练任务**：新建训练任务。
-   **重置**：清空筛选条件。

列表支持按**优先级**筛选（选择优先级后仅展示对应级别的任务）。每行操作列提供**详情**、**停止**、**日志**和**删除**四个按钮，可用性取决于任务状态。

## 步骤七：发布模型用于部署

> 仅 SFT微调训练支持选择发布训练中间状态的模型快照

模型训练完成后，根据[步骤五：训练产出](https://help.aliyun.com/zh/model-studio/model-training-on-console#ae6a2ec3ee3nw)中的配置，产出的最后一个 Checkpoint 会以设定的模型名称**自动发布**至[我的模型](https://bailian.console.aliyun.com/cn-beijing/model/custom)页面。
...
**说明**Checkpoint 在平台的保留时长为 30 天，到期后自动删除且不可恢复，届时将无法再发布该 Checkpoint。请及时发布所需的模型快照。

发布完成后的模型可以在[我的模型](https://bailian.console.aliyun.com/cn-beijing/model/custom)页面查看，并进行部署。
...
使用阿里云百炼[模型评测](https://bailian.console.aliyun.com/cn-beijing/model/evaluate)功能评估自定义模型的训练效果，相关信息请参见[帮助中心：模型评测简介](raw/model-user-guide/model-evaluation-introduction/model-evaluation-overview.md)。
...
1.  账号拥有发起调优任务所在的业务空间下 **模型调优-操作**、**模型部署-操作**、**模型评测-操作** 的页面权限。
    ...
    相关介绍请参考：[账户权限管理](raw/model-user-guide/security-and-compliance/permission-management-overview.md)。控制台链接：[百炼-账号管理](https://bailian.console.aliyun.com/settings/user-management)。
    
2.  需要发起调优任务所在的业务空间拥有对**特定模型**进行**模型训练**（调优）的权限。
    
    相关介绍请参考：[授权子业务空间模型调用、训练和部署](https://help.aliyun.com/zh/model-studio/use-workspace)。控制台链接：[百炼-业务空间管理](https://bailian.console.aliyun.com/cn-beijing/settings/workspace)。
...
1.  在创建训练任务时，在**超参配置**区域单击**展开配置**，找到**max\_length**（序列长度）参数。
...
身份设定依赖 system prompt 生效。如果调用时未传入微调阶段使用的 system prompt，模型仍可能回答基座模型的身份。
...
-   **方案二：在控制台体验中心测试**
    1.  在左侧导航栏选择**体验中心** > **文本模型**。
    2.  单击模型名称，选择您部署的微调模型。
    3.  单击右上角的**模型调试**，在打开的配置面板中的**System Prompt**输入框填写身份设定内容。
    4.  在对话框中发送测试问题，确认模型按照微调后的身份作答。
