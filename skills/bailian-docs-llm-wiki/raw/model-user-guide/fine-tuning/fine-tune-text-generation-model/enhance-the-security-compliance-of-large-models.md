...
**重要**本文涉及的所有敏感问题示例仅用于说明如何让模型拒绝高危请求，模型微调目标是使模型严格遵守中国法律法规和阿里云相关算法安全规范。
...
提升训练集的[数据准备](https://help.aliyun.com/zh/model-studio/training-set-and-evaluation-set#aebd25a7f1g2v)可以改善 SFT 微调效果，可以让训练样本覆盖多个安全领域：政治安全、历史安全、社会安全、网络安全、心理健康、金融安全、青少年保护、公共安全等。
...
> 该设置直接影响微调的效果。阿里云百炼平台提供预设默认参数，如无特殊需求可直接使用默认值训练。点击[在控制台进行模型调优](raw/model-user-guide/fine-tuning/fine-tune-text-generation-model/model-training-on-console.md)了解各个超参数以及如何通过损失曲线来决定是否应该继续微调等细节。
...
1.  在训练过程中，可以点击任务右侧的查看日志，观察训练损失（Training Loss）和验证损失（Validation Loss）曲线，根据损失值的变化趋势判断训练效果：
...
1.  等待任务状态变为![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5881933771/p1057947.png)训练成功，表示模型已完成微调。
...
1.  在配置页面，选择上一步微调完成的模型，点击开始部署。

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5881933771/p1056319.png)

1.  等待部署任务完成（约 3-5 分钟），模型状态变为![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5881933771/p1056314.png)运行中。阿里云百炼将为该服务生成一个唯一的模型 Code（形如`qwen3-8b-ft-xxx-xxx`），后续评测和 API 调用将使用它。
...
1.  在阿里云百炼的[模型评测](https://bailian.console.aliyun.com/?tab=model#/efm/model_evaluate)页面，[模型评测](raw/model-user-guide/model-evaluation-introduction/model-evaluation-overview.md)。

> 阿里云百炼提供人工评测、自动评测、基线评测等多种评测方式以满足不同需求，详见[模型评测](raw/model-user-guide/model-evaluation-introduction/model-evaluation-overview.md)。
...
1.  裁判大模型对每条回复进行自动化评估，输出 `Pass`、`Warn`、`Fail` 三档结果，其中 `Warn` 和 `Fail` 均归为不合格。被 **AI 安全护栏** 拦截的回复同样视为不合格。
    
2.  下表是不同参数配置的示意评测得分，显示在**全量微调、n\_epochs = 3、learning\_rate = 1e-5** 时效果相对更好。也可以尝试调整超参数，进一步优化模型表现。若选择 LoRA 的训练方式，n\_epochs = 3、learning\_rate = 3e-4 时效果也很好，且更具性价比。
