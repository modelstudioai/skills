...
提升训练集的[数据准备](https://help.aliyun.com/zh/model-studio/training-set-and-evaluation-set#9dcde92ee21ky)可以改善 SFT 微调效果，可以让训练样本覆盖多个安全领域：政治安全、历史安全、社会安全、网络安全、心理健康、金融安全、青少年保护、公共安全等。
...
1.  等待部署任务完成（约 3-5 分钟），模型状态变为运行中。阿里云百炼将为该服务生成一个唯一的模型 Code（形如`qwen3-8b-ft-xxx-xxx`），后续评测和 API 调用将使用它。
