...
数据集类型（训练集或评测集）创建后不可变更。各训练方式与元素的支持情况详见[训练方式与元素支持矩阵](#sec-support-matrix)，各组合的格式构造规则见下文对应章节。

**警告**

发布与删除操作不可逆：已发布版本不可再编辑，仅草稿版本可删除或在线编辑；数据集类型创建后不可变更。切换类型、场景或训练方式会清空已上传文件。

管理类操作（创建参数、版本管理、评测集管理规则、导入方式选型、数据清洗、安全合规）引导到同级文档，详见[训练集与评测集](https://help.aliyun.com/zh/model-studio/training-set-and-evaluation-set)、[模型调优简介](https://help.aliyun.com/zh/model-studio/model-training-overview)等。建议在创建数据集页面下载对应场景的数据模板，按模板结构准备数据可避免导入失败。
...
日志回流导入方式详情见[日志回流](https://help.aliyun.com/zh/model-studio/model-log-backflow)。
...
命名示例与目录结构见下文代码块。云存储挂载授权操作见[在控制台进行模型调优](https://help.aliyun.com/zh/model-studio/model-training-on-console)，OSS 导入 Bucket 标签打标操作见[训练集与评测集](https://help.aliyun.com/zh/model-studio/training-set-and-evaluation-set)。
...
1.  核对当前训练方式对应的 maxCount 上限，详见[文件大小与数量限制](#sec-size-limits)。
...
3.  单文件超 300 MB 的承接方式见[API 上传配额](#sec-api-quota)。
...
3.  检查 data.jsonl 与文件格式符合打包规则后重试上传，详见[多模态压缩包打包规则](#sec-zip-package)。
...
验证集自动切分会从训练集中随机抽取 10% 数据作为验证集，减少实际训练数据量。小数据集建议选择独立验证集，验证集配置操作见[在控制台进行模型调优](https://help.aliyun.com/zh/model-studio/model-training-on-console)。

建议先下载对应场景的数据模板，按模板结构准备数据可避免导入失败，数据模板下载建议见[概述](#sec-overview)。常见问题排障详见下文折叠项。草稿数据清洗与增强操作见[数据清洗或增强](https://help.aliyun.com/zh/model-studio/data-processing)，安全合规数据领域见[0 代码强化大模型安全合规能力](https://help.aliyun.com/zh/model-studio/enhance-the-security-compliance-of-large-models)。
