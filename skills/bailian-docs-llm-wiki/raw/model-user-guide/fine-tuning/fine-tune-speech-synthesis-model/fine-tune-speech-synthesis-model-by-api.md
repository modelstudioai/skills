...
-   如果使用[阿里云子账号](raw/model-user-guide/security-and-compliance/permission-management-overview.md)（[RAM 用户](https://help.aliyun.com/zh/ram/user-guide/overview-of-ram-users)），请为该子账号授予调用、训练和部署所需的[权限](https://help.aliyun.com/zh/model-studio/use-workspace)。
...
**说明**请保存响应中的 `file_id`，这是上传数据集的唯一标识，下一步创建调优任务时需要使用。完整请求/响应字段说明请参见 [上传文件](https://help.aliyun.com/zh/model-studio/model-customization-file-management-service)。
...
1.  通过 `GET /api/v1/fine-tunes/{job_id}/checkpoints` 拉取本次任务全部 Checkpoint，从响应 `output[*].model_name` 字段获取每个 Checkpoint 对应的模型 ID（**仅在**`status=SUCCEEDED`**时返回**）。完整字段说明请参见[查询调优任务 Checkpoint 列表](https://help.aliyun.com/zh/model-studio/model-training-api-reference)。
