...
您需要[获取与配置 API Key](raw/model-api-reference/preparations/get-api-key.md)，并确保已开通阿里云百炼的模型服务。

**重要**Dify 属于工作流/自动化平台，**不支持**使用 Token Plan 个人版、Token Plan 团队版和 Coding Plan 接入。这些套餐仅限在 AI 编程工具和 OpenClaw 类型 Agent 中使用，详见[不支持的工具类型](raw/model-user-guide/use-chat-client-or-development-tool/more-tools.md)。
...
> 若插件内暂未包含最新版千问模型，可尝试安装 **OpenAI-API-compatible** 插件，在插件设置中的 **API endpoint URL** 填入`https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1`（华北2（北京）地域）或`https://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1`（新加坡地域）。其中`WorkspaceId`需替换为真实的[获取Workspace ID](https://help.aliyun.com/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
...
3.  **对话测试**
    ...
    您也可以使用 [Qwen-VL](raw/model-user-guide/model-experience/vision-model/vision.md) 或 [QVQ](raw/model-user-guide/model-experience/vision-model/visual-reasoning.md)模型针对图片进行提问。在选择视觉模型后界面左侧会出现**视觉**开关，打开即可在右侧的对话框输入图片。
...
2.  **添加LLM节点**
    ...
    如果您使用 [Qwen-VL](raw/model-user-guide/model-experience/vision-model/vision.md) 或 [QVQ](raw/model-user-guide/model-experience/vision-model/visual-reasoning.md)模型，请打开LLM节点的视觉开关：
...
-   使用了子业务空间的 API Key。`0.0.41`版本的千问插件会校验`qwen-turbo`模型调用权限，请为`qwen-turbo`[添加模型调用权限](https://help.aliyun.com/zh/model-studio/use-workspace)。
...
### Q2：如何使用 [Qwen-Omni](raw/model-user-guide/model-experience/omni-modal/qwen-omni.md)/[Qwen-Audio](https://help.aliyun.com/zh/model-studio/audio-language-model)/[Qwen-OCR](raw/model-user-guide/model-experience/vision-model/qwen-vl-ocr.md)模型？
