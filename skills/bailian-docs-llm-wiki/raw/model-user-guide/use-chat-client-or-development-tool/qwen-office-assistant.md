...
千问 APP 中的工作助理功能支持通过 Token Plan 接入阿里云百炼，使用 Token Plan 模型处理文档、表格等办公任务。桌面端和手机 APP 均可使用。

## 前提条件

-   已开通 Token Plan（个人版或团队版）并获取专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/overview)。
-   已安装[千问 APP](https://www.qianwen.com/download) 桌面端或手机 APP（手机版需升级到 **7.1.2** 及以上版本）。

## 接入步骤

1.  打开千问 APP 或 PC 客户端，进入**工作助理**，点击**添加模型**。
2.  在弹出的面板中粘贴阿里云 Token Plan 的 API Key，点击校验保存。
3.  回到工作助理，在模型面板中选中 Token Plan 模型（如 Qwen3.8-Max），即可直接发起任务。
...
千问工作助理仅支持 Token Plan（个人版和团队版），不支持按量计费或 Coding Plan。
