...
**说明**：百炼支持通过 Codex 使用中国内地模型，配置方式详见上文[配置接入凭证](https://help.aliyun.com/zh/model-studio/codex#cdx-config)。

**解决方案**：建议参照上文配置接入凭证，直接在`~/.codex/config.toml`中完成配置，无需依赖第三方工具的健康检查结果；配置完成后参照[验证配置](https://help.aliyun.com/zh/model-studio/codex#cdx-verify)启动 Codex，若能正常进入对话界面即表示可正常使用中国内地模型。
...
-   报错 `wire_api = "chat" is no longer supported`：将配置文件中的 `wire_api` 改为 `responses`，并确认 `base_url` 配置正确。详见上文[配置接入凭证](https://help.aliyun.com/zh/model-studio/codex#cdx-config)中对应方案的配置示例。
...
**解决方案**：确认`base_url`和`wire_api`与所选方案的配置一致。参见上文[配置接入凭证](https://help.aliyun.com/zh/model-studio/codex#cdx-config)中对应方案的配置示例。
