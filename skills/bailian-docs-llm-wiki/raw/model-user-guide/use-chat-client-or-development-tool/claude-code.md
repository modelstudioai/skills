...
将 YOUR\_API\_KEY 替换为 Token Plan 个人版专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/overview)。可用模型参见 Token Plan 个人版[支持的模型](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-personal-overview.md)。
...
将 YOUR\_API\_KEY 替换为 Token Plan 团队版专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/uac-admin/organization/members/list)。可用模型参见 Token Plan 团队版[支持的模型](raw/model-user-guide/token-plan-guide/token-plan-overview.md)。
...
将 YOUR\_API\_KEY 替换为 Coding Plan 专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/coding-plan)。可用模型参见 Coding Plan [支持的模型](raw/model-user-guide/token-plan-guide/coding-plan-guide/coding-plan.md)。
...
-   美国（弗吉尼亚）：`https://{WorkspaceId}.us-east-1.maas.aliyuncs.com/apps/anthropic`
...
1.  在 CC Switch 主界面顶部图标栏选中 Claude Code 橙色星形图标，点击右上角 **+** 进入**添加新供应商**，按下表填入配置后点击**添加**。
    ...
    **配置信息**
    ...
    请求地址：`[https://token-plan.cn-beijing.maas.aliyuncs.com/apps/anthropic](https://token-plan.cn-beijing.maas.aliyuncs.com/apps/anthropic)`
    ...
    请求地址：`[https://coding.dashscope.aliyuncs.com/apps/anthropic](https://coding.dashscope.aliyuncs.com/apps/anthropic)`
    
    按量计费
    
    供应商名称：百炼-按量计费
    
    API Key：[百炼API Key](raw/model-api-reference/preparations/get-api-key.md)
    
    请求地址：`https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/apps/anthropic`
    
2.  展开**高级选项**配置模型映射，将主模型与 Haiku、Sonnet、Opus 默认模型设置为对应套餐[支持的模型](raw/model-user-guide/token-plan-guide/token-plan-overview.md)。映射关系按需选择，示例如下：
...
**重要**请勿在桌面版的第三方推理配置中手动填写百炼 API Key。桌面版对 CC Switch 本地网关（地址 `http://127.0.0.1:15721/claude-desktop`）的鉴权令牌由 CC Switch 自动生成并写入，手动填入百炼 API Key 会因令牌不匹配导致鉴权失败。桌面版第三方配置写入目前仅支持 macOS、Windows。
...
2.  前往百炼控制台[获取 API Key](raw/model-api-reference/preparations/get-api-key.md)，告诉 Claude Code 配置：
...
-   按量计费：[Anthropic API兼容 - 错误码](raw/model-api-reference/qwen-api-reference/anthropic-api-messages.md)
...
若确实无法对话，请确认：请求地址以 `/apps/anthropic` 结尾、勿额外添加 `/v1`，并已在[高级选项的模型映射](raw/model-user-guide/use-chat-client-or-development-tool/claude-code.md)中填入对应套餐支持的模型。
