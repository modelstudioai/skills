...
填入 Token Plan 个人版专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/overview)。

Model ID

填入 Token Plan 个人版[支持的模型](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-personal-overview.md)。
...
填入 Token Plan 团队版专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/uac-admin/organization/members/list)。

Model ID

填入 Token Plan 团队版[支持的模型](raw/model-user-guide/token-plan-guide/token-plan-overview.md)。
...
填入 Coding Plan 专属 [API Key](https://bailian.console.aliyun.com/cn-beijing/subscription/coding-plan)。

Model ID

填入 Coding Plan [支持的模型](raw/model-user-guide/token-plan-guide/coding-plan-guide/coding-plan.md)。
...
根据地域，填入对应 URL（请将 URL 中的 `{WorkspaceId}` 替换为真实的[业务空间ID](https://help.aliyun.com/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)）：
...
2.  前往百炼控制台[获取 API Key](raw/model-api-reference/preparations/get-api-key.md)，告诉 Cline 配置：
