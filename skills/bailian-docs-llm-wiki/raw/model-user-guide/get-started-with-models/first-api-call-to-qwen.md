...
2.  **开通阿里云百炼：**使用**阿里云主账号**前往[阿里云百炼大模型服务平台](https://bailian.console.aliyun.com/model/market)，阅读并同意协议后，将自动开通阿里云百炼，如果未弹出服务协议，则表示您已经开通。
    
    > 如果开通服务时提示“您尚未进行实名认证”，请先进行[实名认证](https://help.aliyun.com/zh/account/verify-your-identity-individual-account)。
    
3.  **获取API Key：**前往[API Key](https://bailian.console.aliyun.com/model/settings/api-key)页面，单击**创建API Key，**即可通过API KEY调用大模型。
...
4.  **获取业务空间ID：**使用**华北2（北京）**、**新加坡**、**日本（东京）**、**德国（法兰克福）**或**中国香港**地域的模型时，需在Base URL中填入业务空间ID（WorkspaceId），可在[业务空间管理](https://bailian.console.aliyun.com/cn-beijing/settings/workspace)页面中查看。
...
您的Python需要为3.9或以上版本，请您参考[安装Python](https://help.aliyun.com/zh/sdk/developer-reference/installing-python)进行安装。
...
示例代码仅完成了简单问答，如果您想了解千问 API 的更多用法，如[流式输出](raw/model-user-guide/model-experience/text-generation-model/stream.md)、[结构化输出](raw/model-user-guide/model-experience/text-generation-model/qwen-structured-output.md)、[Function Calling](raw/model-user-guide/model-experience/text-generation-model/tool-calls/qwen-function-calling.md)等，请参见[概述](raw/model-user-guide/model-experience/text-generation-model/text-generation.md)目录。

**在线体验大模型**

如果您想像[千问官网](https://tongyi.aliyun.com/qianwen/)一样，通过**对话框**与大模型互动，请访问[体验中心](https://bailian.console.aliyun.com/model/experience/text)。

> 千问官网将千问 API 与联网搜索、网页解析等工具进行了集成，与直接调用千问 API 效果略有差异。

图像生成模型仅支持单轮对话，不支持多轮对话编辑。每次生成图片都是独立操作，无法在同一对话上下文中引用上一次的生成结果。如需在原图基础上修改，请将图片保存到本地，单击对话窗口的**加号（+）**按钮重新上传，然后输入新的编辑指令。
