...
Token Plan 个人版采用月限额，无周额度使用限制，限额单位为 Credits。自订阅当日起向后延伸 30 天作为订阅周期，订阅周期内累计消耗达到套餐额度后暂停服务，需等待下一个订阅月后额度恢复。订阅月内未用完的额度不结转至下一周期。

例如：Standard 套餐的月额度为 45,000 Credits。9 月 10 日订阅，本订阅月为 9 月 10 日 ~ 10 月 10 日。9 月 20 日累计消耗达到 45,000 Credits 触顶，服务暂停；10 月 10 日下一个订阅月开始后，额度重置为 45,000 Credits，服务恢复。
...
不是。额度按订阅月自动刷新：以订阅日为起点每 30 天为一个订阅月，每个订阅月开始时额度自动重置。重置时间取决于订阅日期，而非固定的日历日期（如每月 1 日）。
...
不占用、不计入套餐月额度；套餐月额度用尽后，自动开始抵扣用量包额度。
...
在[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/subscription/token-plan)的 **Token Plan > 我的订阅** 页面查看当前订阅的 Credits 额度及消耗情况；切换到 **Harness 权益** 页签查看 AgentStudio 工具的免费额度与折扣明细，详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。
...
-   **模型单价影响**：高价模型（如 qwen3.7-max）会导致相同 Token 数下 Credits 消耗更高。
...
先抵扣服务本身免费额度，再抵扣 Token Plan 附赠权益额度，用尽后按折扣后付费。详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。

### Harness 调用使用哪个 API Key？

百炼 API Key（以 `sk-` 开头），不是 Token Plan 专属 API Key（以 `sk-sp-` 开头）。详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。

### Harness 调用会消耗套餐 Credits 吗？

不消耗。Harness 权益独立核算，超出按折扣后付费，不占用 Credits 或用量包。详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。

### 在哪查看 Harness 权益的剩余额度？

在控制台**我的订阅**页面切换到**Harness 权益**页签查看。详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。

### 加量包能否抵扣 Harness 服务的费用？

不能。用量包仅补充 Credits，Harness 权益独立核算。详见 [Harness 权益](raw/model-user-guide/token-plan-guide/token-plan-personal/token-plan-harness-benefits.md)。
...
限流阈值会根据整体负载动态调整以保障服务稳定性。套餐限流额度不支持提升。
...
支持从低档位升级到更高档位。升级不重置当前订阅周期：按剩余天数补缴差价，并按新档位与原档位的月额度差折算发放当前周期新增 Credits，自下一个订阅月起按新档位月额度计量，折算公式详见[概述·升级](https://help.aliyun.com/zh/model-studio/token-plan-personal-overview#tpp01-h-upgrade)。
...
登录[百炼控制台 Token Plan](https://bailian.console.aliyun.com/cn-beijing/subscription/token-plan) 页面，在订阅管理中关闭自动续费。

### 如何查看 Token Plan 套餐的生效时间和剩余天数？

-   **生效时间查看路径**：登录[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/subscription/token-plan)的 **Token Plan** 页面查看套餐生效时间。
