...
访问 [Token Plan 团队版购买页面](https://bailian.console.aliyun.com/cn-beijing/subscription/overview)，选择套餐档位和订阅周期并完成订阅，主账号和 RAM 账号均可订阅。
...
## 步骤二：获取 Token Plan 团队版 API Key 和 Base URL
...
-   **API Key**：在 Token Plan 控制台的成员管理页面或管理平台创建成员账号，分配席位后，为成员生成 API Key。详见团队管理。
    ...
    -   Token Plan API Key 以 `sk-sp-` 开头，与百炼 API Key（`sk-` 开头）格式不同，两者不可混用。
...
-   **按量付费**：使用百炼 API Key（`sk-` 开头）+ `https://dashscope.aliyuncs.com/compatible-mode/v1`。

如果购买了 Token Plan 但使用了百炼 API Key 或其他计费模式的 Base URL，会导致调用走按量计费通道产生意外扣费，或返回 401/403 鉴权失败错误。
...
通过模型内置工具调用，模型可以在对话中调用联网搜索、文搜图、图搜图、网页抓取、代码解释器等扩展能力，通过 Responses API 直接调用，模型内置工具按抵扣系数消耗 Credits。
