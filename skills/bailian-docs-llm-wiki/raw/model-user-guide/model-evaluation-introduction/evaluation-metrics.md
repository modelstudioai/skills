...
创建维度模板后，您可以在评测任务中引用该模板作为评判标准。关于评测任务的创建和执行，请参见[模型评测](raw/model-user-guide/model-evaluation-introduction/model-evaluation-overview.md)。

## 定义评测维度

登录[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/evaluate/dimension_template)，选择左侧导航栏的**模型评测**，切换到**评测维度** Tab，单击**创建评测维度**进入创建页面。控制台页面顶栏显示当前业务空间（可切换）和地域（默认华北2·北京）。
...
2.  选择**评分器模版**：选择预置模板（标准匹配、情感分析）或自定义评分器。选择预置模板后，系统自动填充 Prompt 和默认标签。详见[配置评分器Prompt](https://help.aliyun.com/zh/model-studio/evaluation-metrics#h2-sec-scorer-prompt)。
...
在[评测维度列表页](https://bailian.console.aliyun.com/cn-beijing/model/evaluate/dimension_template)，您可以查看、修改和删除已创建的维度模板。
...
根据评测场景选择合适的类型：有标准答案的场景优先使用规则评估（成本低、速度快），需要语义理解的场景使用大模型评估，主观质量判断使用人工评估。各类型的详细对比见[评测维度概述](https://help.aliyun.com/zh/model-studio/evaluation-metrics#h2-sec-overview)。
