...
**Important**删除操作不可恢复：仅移除百炼侧的模型记录，须重新导入才能恢复；不会删除 OSS 中的源文件。

列表顶部搜索框可按模型名称筛选模型，支持清空重置。**来源**列按导入来源渲染：OSS 导入显示 oss://bucket/path，训练任务显示来源任务 ID（已删除则显示「训练任务已删除」）。**支持部署方式**列展示模型支持的部署方式和训练方式标签（如全参、LoRA 或量化标签），以及计费方式（如按 Token 计费、按模型单元计费），无可选部署方式时显示「-」。通过 API 调用已部署模型详见[使用 API 进行模型部署](raw/model-user-guide/model-deployment-index/model-deployment-quick-start.md)。
