...
通用模型部署流程详见[专属部署概述](raw/model-user-guide/model-deployment-index/model-deployment-introduction.md)与[预置吞吐](raw/model-user-guide/model-deployment-index/ptu-long-input-and-cache.md)。

## 方案选型

百炼为推理调用提供多种容量与计费方案。DTU 适用于需要独占部署、微调模型部署、低延迟高并发或数据隔离的场景。各方案的对比如下表所示。

Token 按量、PTU 等全部计费方式的完整对比详见[模型部署](raw/model-user-guide/model-deployment-index/model-deployment-introduction.md)主表。
...
通用模型部署的基础流程详见[专属部署概述](raw/model-user-guide/model-deployment-index/model-deployment-introduction.md)。
...
在百炼控制台**专属部署**的部署列表和详情页可查看已购的输入/输出 TPM 额度，详见[专属部署概述](raw/model-user-guide/model-deployment-index/model-deployment-introduction.md)。
