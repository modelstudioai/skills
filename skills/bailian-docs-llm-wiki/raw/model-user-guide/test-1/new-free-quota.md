...
免费额度的有效期为 90 天，从开通阿里云百炼、模型发布或模型申请通过之日起计算（以较晚者为准）。额度到期或耗尽后，继续调用模型推理服务将[产生计费](raw/model-user-guide/test-1/model-pricing.md)。
...
2.  单击账单列表右上角的图标，找到**用量信息**，勾选**用量**，单击**确定**。
