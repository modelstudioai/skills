...
> 您可以查看[模型训练控制台](https://bailian.console.aliyun.com/model/tuning/create)底部的预估训练费用，并单击**计费详情**，查看训练 Token 总数、循环次数和训练单价。
...
训练Token总量 = n\_epochs × N × (max\_pixels / 压缩率) × GPU系数

其中：

-   n\_epochs：训练时指定的超参数，表示训练轮数（创建微调任务时配置）。
-   N：训练集中的图片总数。
-   max\_pixels：训练时指定的超参数，表示图片的最大像素数（创建微调任务时配置）。取值为 `1k`（即 1024×1024）或 `2k`（即 2048×2048）。
...
-   单张图片的Token量 = max\_pixels / 压缩率 = 1024×1024 / 1024 = 1024
-   训练Token总量 = 1000 × 1 × 1024 × 8 = 8192000 = 8192千Token
-   模型训练费用 = 8192 × 0.02 = 163.84元

下表为 GPU系数按 8 计算时，**每 1 张训练图片**的Token消耗与费用预估。训练集包含 N 张图片时，将表中数值乘以 N 即可。
...
-   GPU系数随任务调度情况动态调整，上述示例与预估表取 8 仅用于演示计算过程。实际训练消耗的Token数请以[查询微调任务状态](raw/model-api-reference/model-production/fine-tuning-jobs-api/get-fine-tuning-job-api.md)接口返回的 `usage` 字段为准，费用以账单为准。
-   `batch_size` 不参与计费，不影响训练Token总量。
...
训练Token总量 = (∑i=1N 视频i的计费时长) × (max\_pixels / 1024) × n\_epochs
...
消耗Tokens = (lm\_max\_epoch + fm\_max\_epoch) × 25 × 训练集总时长(秒)
...
-   预付费订单支持提前退订，已使用部分按 1.2 倍系数结算退费。详见[降配退款规则说明](https://help.aliyun.com/zh/user-center/description-of-downgrade-refund-rules)。
...
经过SFT-LoRA高效微调的千问图像生成模型，部署免费，调用按微调的基础模型的标准调用价格计费。模型训练和部署流程请参见[图像生成模型调优](raw/model-user-guide/fine-tuning/wan-image-generation-finetune-guide.md)。
