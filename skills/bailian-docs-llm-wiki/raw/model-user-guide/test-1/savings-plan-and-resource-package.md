...
为最大化成本效益，建议优先了解并选择 [AI 通用型节省计划](https://help.aliyun.com/zh/model-studio/savings-plan-and-resource-package#ghoteqo7uv9wa)。
...
**调用方式**：百炼控制台体验中心、API 代码调用、接入第三方工具（如[OpenClaw](raw/model-user-guide/use-chat-client-or-development-tool/openclaw.md)等，填写按量计费接入凭证）。
...
华北2（北京）、美国（弗吉尼亚）、新加坡、德国（法兰克福）、日本（东京）、中国香港

-   不同地域的**节省计划**独立计费，不可跨地域抵扣。在华北2（北京）购买的节省计划，仅抵扣华北2（北京）地域产生的模型调用费用。
-   建议选择与业务服务器地理位置最近的地域，以降低网络延迟。
-   如需在多个地域调用模型服务，需为每个地域分别购买节省计划。

**支持的抵扣范围**

不同档位享受不同的折扣。

-   A 类：千问（不含 qwen3.6-max-preview）、千问-开源、文本向量、多模态向量、排序模型、行业模型、模型原生工具调用（[Function Calling](raw/model-user-guide/model-experience/text-generation-model/tool-calls/qwen-function-calling.md)、[网页抓取](raw/model-user-guide/model-experience/text-generation-model/tool-calls/web-extractor.md)等）
    
-   B 类：图像生成、语音合成、语音识别与翻译、视频生成与编辑
    
-   C 类：qwen3.6-max-preview、DeepSeek、Kimi、GLM、MiniMax、HappyHorse
    
    > C 类中仅**阿里直供**版本支持抵扣，**三方直供**版本不支持抵扣。模型是否有阿里云直供版本，请以控制台模型广场实际展示为准。详情参见[三方直供模型支持抵扣 AI 通用型节省计划吗？](https://help.aliyun.com/zh/model-studio/savings-plan-and-resource-package#c14b3d84925t1)
...
请参考[折扣信息](https://help.aliyun.com/zh/model-studio/savings-plan-and-resource-package#fcce9aa0f450o)。
...
ASR模型按秒计费，TTS模型按字符计费，请前往[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/market)查看模型调用价格。
...
请前往[百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/market)查看全部模型及其调用价格。
...
以上模型属于「百炼大模型推理」商品。wan2.7-r2v 等模型属于「百炼大模型-万相模型」商品，不在本节省计划抵扣范围内。如需抵扣百炼全量模型，建议购买 AI 通用型节省计划。
...
qwen-plus 及 qwen-plus-latest的实时推理服务（[非思考模式](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)）

qwen-max的实时推理服务（[非思考模式](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)）
...
-   **qwen-plus**、**qwen-plus-latest**
    ...
    -   仅支持抵扣单次请求输入在`0<Token≤128K`阶梯范围内的实时推理费用（[非思考模式](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)，包含输入和输出）。
...
-   资源包：未发生使用的部分，可按未使用额度费用[申请退款](https://billing-cost.console.aliyun.com/refund/refund?commodityType=RESOURCE_PLANS&refundType=NOREASON_REFUND)；已使用的部分则无法退款。
    ...
    节省计划与资源包均须在**到期前**申请退订；若已过期，将自动失效/作废，无法再申请退订。
...
7.  **额度已用完**：节省计划的承诺额度用尽后，超出部分按量付费出账。可在[节省计划总览页面](https://usercenter2.aliyun.com/resource/spn/overview)查看剩余金额，确认额度是否已耗尽。
8.  **抵扣设置已关闭**：在节省计划管理页面单击**设置抵扣规则**，检查各计费项的抵扣开关。各计费项默认开启抵扣；若某计费项的开关被关闭，该计费项产生的费用不再由节省计划抵扣。
9.  **商品 code 不在抵扣范围**：在[节省账单使用明细](https://billing-cost.console.aliyun.com/resource/spn/detail)中查看该笔账单的商品 code，并与所购节省计划的抵扣范围比对。节省计划仅抵扣特定商品 code 的账单，例如 AI 通用型节省计划对应 `sfm_GenAI_spn_cn`、大语言模型推理节省计划对应 `sfm_llminference_spn_public_cn`、语音模型节省计划对应 `sfm_VoiceModel_spn_cn`。

### 三方直供模型支持抵扣 AI 通用型节省计划吗？

[C 类模型](raw/model-user-guide/test-1/savings-plan-and-resource-package.md)中，阿里直供的模型支持抵扣，三方直供的模型不支持抵扣。您可以在[百炼模型广场](https://bailian.console.aliyun.com/cn-beijing/model/market)中通过模型卡片右上角标识（"阿里直供"或"三方直供"标签）确认模型的供应方式。

### 购买节省计划后如何使用？

节省计划在生效后无需重新配置 **API Key** 或 **Base URL**，购买后自动生效。您通过百炼控制台体验中心、API 代码调用或接入第三方工具调用模型时，产生的费用会按[抵扣顺序](https://help.aliyun.com/zh/model-studio/savings-plan-and-resource-package#59b7ecf08fiac)自动抵扣。节省计划仅对**生效后产生的费用**进行抵扣，购买前已产生的费用无法追溯抵扣。您可以在[节省计划总览页面](https://usercenter2.aliyun.com/resource/spn/overview)查看抵扣明细和剩余额度。
