...
仅在实际发生调用后生成账单：**大模型推理**分钟级出账（通常 2~10 分钟），**批量推理、模型训练、知识库**等小时级出账，业务高峰期请以系统最终出账时间为准。

### **费用概览**

登录百炼控制台[费用概览](https://bailian.console.aliyun.com/?tab=model#/costing-balance/overview)页，选择**账期月份**：

-   **查看总消费与构成**：顶部展示当月**总消费金额**，拆分为**订阅购买费用**（Token Plan、预置吞吐、模型单元、节省计划等）与**账单费用**（按量产生的模型调用与训练）；单击卡片**查看明细**可展开分项。
