...
**警告**此操作不可逆。删除后文件数据将被永久移除，无法恢复。如果该文件已被知识库引用，删除后相关的文档索引也将失效。
...
```
curl -X POST "$BASE_URL/api/v1/connector/dash/deleteFile" \
...
删除结果。子字段 `fileId`（string，已删除的文件 ID）、`status`（string，删除后状态，为 `DELETED`）。
