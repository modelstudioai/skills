...
-   您可以将对话过程中的特定信息（即记忆片段，详见长期记忆）存储到长期记忆体（Memory）中，智能体应用将在后续对话中持续引用这些信息（非自动，您需要先调用 CreateMemory 接口创建长期记忆体，获取memoryId。然后通过 API 调用智能体应用时，传入该memoryId）。
...
-   若传入memoryId，系统会根据对话记录，在指定长期记忆体下自动创建记忆片段（MemoryNode）。您也可以调用 CreateMemoryNode 接口手动创建记忆片段。
...
[调试](https://api.alibabacloud.com/api/bailian/2023-12-29/CreateMemory)
