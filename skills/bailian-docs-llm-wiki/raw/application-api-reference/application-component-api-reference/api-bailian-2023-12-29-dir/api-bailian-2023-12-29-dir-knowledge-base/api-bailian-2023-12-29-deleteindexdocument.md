...
-   RAM 用户（子账号）需要首先获取阿里云百炼的 API 权限（需要AliyunBailianDataFullAccess，已包括 sfm:DeleteIndexDocument 权限点），然后才能调用本接口。阿里云账号（主账号）可直接调用无须授权。建议您通过最新版阿里云百炼 SDK来调用本接口。
...
[调试](https://api.alibabacloud.com/api/bailian/2023-12-29/DeleteIndexDocument)
...
文件 ID，即 **AddFile** 接口返回的`FileId`。您也可以在[应用数据](https://modelstudio.console.alibabacloud.com/?tab=app#/data-center)\-文件页签，单击文件名称旁的 ID 图标获取。
