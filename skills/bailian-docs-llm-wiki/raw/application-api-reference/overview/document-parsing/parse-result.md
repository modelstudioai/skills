...
查询解析任务的状态、结果或分片。`data.status` 为 `processing` 时继续轮询，为 `success` 或 `failed` 时停止。
...
**说明**当状态为 `processing` 时继续查询本接口；当状态为 `failed` 时，结合[错误码](raw/application-api-reference/overview/errors.md)判断失败原因。
