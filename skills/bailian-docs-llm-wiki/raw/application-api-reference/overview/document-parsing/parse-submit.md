...
提交文档或音视频解析任务，返回 `biz_id`。文档解析为异步任务：提交文件获得 `biz_id`，再使用该 ID 调用[查询解析结果](raw/application-api-reference/overview/document-parsing/parse-result.md)获取处理状态和结果。
...
**重要**提交后，使用[查询解析结果](raw/application-api-reference/overview/document-parsing/parse-result.md)轮询任务状态并获取解析结果。
