...
查询抽取任务状态和结构化结果。`data.status` 为 `processing` 时继续轮询，为 `success` 或 `failed` 时停止。
...
**说明**当状态为 `processing` 时继续查询本接口；当状态为 `failed` 时，结合[错误码](raw/application-api-reference/overview/errors.md)处理失败原因。
