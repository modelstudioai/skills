# HappyOyster-Acting-删除World API参考

删除当前主账号名下的 Acting World。同账号下 World 已不存在时幂等返回 deleted=false。

## 适用范围

删除当前主账号名下的 Acting World。调用前请确认以下事项：
...
-   **前置条件**：使用[创建World](raw/model-api-reference/world-model-api-reference/happyoyster/happyoyster-acting-openapi-reference/happyoyster-acting-create-world-api-reference.md)接口返回的 `encryptedWorldId` 删除。
...
要删除的 Acting 加密 World ID。由[创建World](raw/model-api-reference/world-model-api-reference/happyoyster/happyoyster-acting-openapi-reference/happyoyster-acting-create-world-api-reference.md)接口返回。
...
-   ID 可解析但跨账号、workspace 不匹配或资源不是 Acting World 时，返回 `403001`。
-   删除后继续调用详情、构建状态或换凭证会按资源不存在处理。
-   已经签发但尚未使用的 ticket 不应被视为长期有效引用；进房时仍会重新校验 World 可用状态。
...
-   [查询World列表](raw/model-api-reference/world-model-api-reference/happyoyster/happyoyster-acting-openapi-reference/happyoyster-acting-query-world-list-api-reference.md)：查看剩余 Acting World。
-   [创建World](raw/model-api-reference/world-model-api-reference/happyoyster/happyoyster-acting-openapi-reference/happyoyster-acting-create-world-api-reference.md)：创建新的 Acting World。
