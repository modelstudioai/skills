...
-   阿里云百炼API-KEY：[获取与配置 API Key](raw/model-api-reference/preparations/get-api-key.md)并[配置API Key到环境变量](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)
...
> 关于通过file\_id进行文档分析，请参考[长上下文（Qwen-Long）](raw/model-user-guide/model-experience/text-generation-model/specialized-models/long-context-qwen-long.md)。
...
`file-extract`: 用于[Qwen-Long](raw/model-user-guide/model-experience/text-generation-model/specialized-models/long-context-qwen-long.md)与[Qwen-Doc](raw/model-user-guide/model-experience/text-generation-model/specialized-models/data-mining-qwen-doc.md)模型的文档理解与数据提取；
...
文件的当前状态。可能的状态有：`uploaded`（已上传，等待解析）, `processing`（解析中，需等待解析完成后再引用）, `processed`（解析完成，可正常引用调用）, `error`（解析失败）。
