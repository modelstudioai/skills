...
**semantic\_punctuation\_enabled**`boolean`（可选）

**重要**仅Paraformer（v2）支持该参数。
...
**multi\_threshold\_mode\_enabled**`boolean`（可选）
...
**punctuation\_prediction\_enabled**`boolean`（可选）
...
**inverse\_text\_normalization\_enabled**`boolean`（可选）
...
客户端生成的任务 ID（UUID 格式），需与[run-task](https://help.aliyun.com/zh/model-studio/paraformer-client-events#b618a7624b06f)事件中的 task\_id 保持一致。
