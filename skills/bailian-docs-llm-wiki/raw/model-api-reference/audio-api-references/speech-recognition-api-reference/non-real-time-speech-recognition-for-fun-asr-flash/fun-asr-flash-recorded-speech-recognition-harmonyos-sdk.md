# 非实时语音识别（Qwen-Audio-3.0-ASR-Flash/Fun-ASR-Flash）HarmonyOS SDK

Qwen-Audio-3.0-ASR-Flash/Fun-ASR-Flash非实时语音识别HarmonyOS SDK可将语音转换为文本。
...
服务地址，固定为 `wss://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation`。调用时请将`{WorkspaceId}`替换为真实的[Workspace ID](https://help.aliyun.com/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
