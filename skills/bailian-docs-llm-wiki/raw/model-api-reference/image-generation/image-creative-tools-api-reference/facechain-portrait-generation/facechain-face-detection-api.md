...
3.  图像质量：图像分辨率大于256_256，小于4096_4096，文件大小不超过5MB，图像清晰，没有复杂的光线和阴影关系；
...
输入的图像 URL，分辨率不小于256_256，不超过4096_4096，文件大小不超过5MB, 支持格式包括JPEG, PNG, JPG, WEBP。
...
服务内部算法执行异常。常见原因为输入的图片URL包含非ASCII字符（如中文），请对URL进行编码处理后重试。
