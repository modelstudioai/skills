...
本文介绍万相-图像局部重绘模型的输入输出参数。

**相关指南**：[图像局部重绘](raw/model-user-guide/model-experience/image-model/vary-region.md)
...
万相-图像局部重绘根据用户输入的原始图片、局部区域涂抹图和prompt提示词文字内容，在涂抹区域生成与文字描述相对应的内容，而涂抹区域外的部分则基本保持不变。
