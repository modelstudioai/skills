...
1.  需要[开通模型服务并获取API Key](raw/model-api-reference/preparations/get-api-key.md)，再[配置API Key到环境变量](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)。
...
1.  若操作系统为macOS或Linux，可在终端执行curl命令。
2.  若操作系统为Windows，可使用Postman、Apifox等接口平台发送HTTP请求。
