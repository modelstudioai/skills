...
1.  前往[阿里云百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/market)，搜索 **PixVerse**，找到 PixVerse 模型卡片，单击**立即开通**；
2.  在弹窗内确认开通及授权。
...
-   media 中设置的 ref\_name 在同一请求内必须全局唯一，且须与 prompt 中的引用一一对应。
...
-   支持 HTTP 或 HTTPS 协议。
-   示例值：[](https://xxx/xxx.jpg)[https://xxx/xxx.jpg](https://xxx/xxx.jpg)。
...
**aspect\_ratio** `string` **（omni 模型必选）**
...
**重要**duration直接影响费用，请在调用前确认[爱诗-参考生视频](https://help.aliyun.com/zh/model-studio/model-pricing#pxr002a0h3001)。
...
**重要**audio直接影响费用，请在调用前确认[爱诗-参考生视频](https://help.aliyun.com/zh/model-studio/model-pricing#pxr002a0h3001)。
...
通过 `media` 传入参考图像，设置 `size` 和 `duration` 控制视频分辨率和时长。
...
在 `media` 传入参考图像，并传入`ref_name`用于设置参考图像的主体名称。
