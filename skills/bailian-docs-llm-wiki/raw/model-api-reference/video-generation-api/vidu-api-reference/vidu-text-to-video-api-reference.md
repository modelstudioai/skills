...
**重要**resolution直接影响费用，请在调用前确认[模型价格](https://help.aliyun.com/zh/model-studio/model-pricing#2143d64593wck)。
...
**重要**duration直接影响费用，按秒计费，时间越长费用越高，请在调用前确认[模型价格](https://help.aliyun.com/zh/model-studio/model-pricing#2143d64593wck)。
...
SDK 的参数命名与[HTTP接口](https://help.aliyun.com/zh/model-studio/vidu-text-to-video-api-reference#vd101a0h2http)基本一致，参数结构根据语言特性进行封装。
...
**A：** 不必。二者均为可选参数，但**推荐同时传入**。同时传入可精准控制生成视频的宽高比，具体参见[size取值](raw/model-api-reference/video-generation-api/vidu-api-reference/vidu-text-to-video-api-reference.md)。
