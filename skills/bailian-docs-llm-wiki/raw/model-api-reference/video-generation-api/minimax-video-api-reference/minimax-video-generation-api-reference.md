...
请前往[阿里云百炼控制台](https://bailian.console.aliyun.com/cn-beijing/model/market)，搜索"MiniMax"，找到**MiniMax**模型卡片，单击**立即开通**，在弹窗内确认开通及授权。
...
**说明**图生视频与多模态参考生视频互斥：media中出现`image_url` / `feature` / `driving_audio`任一类型时，不能再出现`first_frame` / `last_frame`（反之亦然），二者不可混用。
...
**input\_seconds** `integer`

输入参考视频的总秒数。所有输入视频的毫秒级时长求和后四舍五入取整。无参考视频时为0。

**output\_seconds** `integer`

输出视频的总秒数。

**total\_seconds** `integer`
