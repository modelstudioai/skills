...
**相关文档**：[使用指南](raw/model-user-guide/model-experience/video-generate-edit-model/text-to-video-guide.md)

## 适用范围

为确保调用成功，请务必保证**模型、Endpoint URL 和 API Key 均属于同一地域**。跨地域调用将会失败。

-   [**选择模型**](https://bailian.console.aliyun.com/cn-beijing/model/market)：前往模型广场选择模型，并确认其所属地域。
...
模型名称。可选模型请参见[模型列表](https://help.aliyun.com/zh/model-studio/model-pricing#ba6f7744d5e0o)。

示例值：wan2.7-t2v、wan2.7-t2v-2026-06-12。
...
-   wan2.7-t2v、wan2.7-t2v-2026-06-12：不超过5000个字符。
...
1.  公网URL：
    ...
    -   支持 HTTP 和 HTTPS 协议。
    -   示例值：[](https://help-static-aliyun-doc.aliyuncs.com/xxx.mp3)[https://help-static-aliyun-doc.aliyuncs.com/xxx.mp3](https://help-static-aliyun-doc.aliyuncs.com/xxx.mp3)。
2.  临时URL：
...
视频处理参数。如设置视频分辨率、开启prompt智能改写、添加水印等。

属性

**resolution** `string` （可选）

**重要**resolution直接影响费用，请在调用前确认[模型价格](https://help.aliyun.com/zh/model-studio/model-pricing#ba6f7744d5e0o)。

生成视频的分辨率档位，用于控制视频的清晰度（总像素）。

-   wan2.7-t2v、wan2.7-t2v-2026-06-12：可选值：720P、1080P。默认值为`1080P`。
...
**重要**duration直接影响费用，按秒计费，请在调用前确认[模型价格](https://help.aliyun.com/zh/model-studio/model-pricing#ba6f7744d5e0o)。

生成视频的时长，单位为秒。

-   wan2.7-t2v、wan2.7-t2v-2026-06-12：取值为\[2, 15\]之间的整数。默认值为5。
...
通过 `prompt` 自然语言控制镜头结构，无需配置`shot_type` 参数，配置也不生效。
...
通过 negative\_prompt 排除“花朵”元素，避免其出现在视频画面中。
...
dashscope video-synthesis create -m wan2.7-t2v -p "小猫在奔跑"
```
