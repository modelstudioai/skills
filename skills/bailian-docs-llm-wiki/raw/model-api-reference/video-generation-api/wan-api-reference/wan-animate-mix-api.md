...
万相-视频换人模型wan2.2-animate-mix提供标准模式`wan-std`和专业模式`wan-pro`两种服务模式，不同模式在效果和计费上存在差异，详情参见[计费与限流](https://help.aliyun.com/zh/model-studio/wan-animate-mix-api#ce68f54f7bbqe)。
...
本地文件可通过[上传文件获取临时URL](raw/model-api-reference/more-about-models/get-temporary-file-url.md)。
...
详情请参见[效果示例](https://help.aliyun.com/zh/model-studio/wan-animate-mix-api#55a79354d2s4j)和[计费与限流](https://help.aliyun.com/zh/model-studio/wan-animate-mix-api#ce68f54f7bbqe)。
...
```
curl -X GET https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api/v1/tasks/0385dc79-5ff8-4d82-bcb6-xxxxxx \
    --header "Authorization: Bearer $DASHSCOPE_API_KEY"
```
...
**数据时效**：任务task\_id和视频URL均只保留 24 小时，过期后将无法查询或下载，请及时[下载视频到本地](raw/model-api-reference/video-generation-api/wan-api-reference/wan-animate-mix-api.md)。

**内容审核**：输入与输出内容均会经过内容安全审核，包含违规内容的请求将报错“IPInfringementSuspect”或“DataInspectionFailed”，具体参见[错误码](raw/model-api-reference/preparations/error-code.md)。
...
如果调用失败，请参见[错误码](raw/model-api-reference/preparations/error-code.md)。

## 常见问题

#### Q: 如何查看模型调用量？

A: 调用数据存在分钟级延迟。请在模型监控（[北京](https://bailian.console.aliyun.com/cn-beijing/model/telemetry)或[新加坡](https://bailian.console.aliyun.com/ap-southeast-1/model/telemetry)）页面查看调用量、调用次数和成功率等指标。[如何查看模型调用记录？](https://help.aliyun.com/zh/model-studio/new-free-quota#ab6ba5c538rn3)
