...
-   动作模板需先通过[AnimateAnyone 动作模板生成API](raw/model-api-reference/video-generation-api/portrait-animation-api-reference/animateanyone-quick-start/animate-anyone-template-api.md)制作，请确保动作模板视频的来源符合相关法律法规，且已获得该视频内容（包含音频）的使用许可。
...
-   输入的动作模板由[AnimateAnyone 动作模板生成API](raw/model-api-reference/video-generation-api/portrait-animation-api-reference/animateanyone-quick-start/animate-anyone-template-api.md)生成；
-   输入的图像已通过[AnimateAnyone 图像检测API](raw/model-api-reference/video-generation-api/portrait-animation-api-reference/animateanyone-quick-start/animate-anyone-detect-api.md)检测。
...
-   上传的图像文件支持HTTP链接，不支持本地路径。也可使用平台提供的[临时存储空间](raw/model-api-reference/more-about-models/get-temporary-file-url.md)，上传本地文件并创建链接。
...
-   输入图像与视频生成类型的适用关系：
    ...
    **（即use\_ref\_img\_bg设为true）**
    
    **按视频背景生成**
    
    **（即use\_ref\_img\_bg设为false）**
...
用户上传的图片 URL，该图应先通过[AnimateAnyone 图像检测API](raw/model-api-reference/video-generation-api/portrait-animation-api-reference/animateanyone-quick-start/animate-anyone-detect-api.md)，并结合所需生成的画幅进行适当裁剪。

-   图像文件＜5M，宽高比≤2，最大边长≤4096
-   格式支持jpg、png、jpeg、bmp。

**说明**上传文件支持HTTP或HTTPS链接方式，不支持本地链接方式。您也可在此[获取临时公网URL](raw/model-api-reference/more-about-models/get-temporary-file-url.md)。
...
动作模板ID，用于指明所需使用的动作模板。应输入[AnimateAnyone 动作模板生成API](raw/model-api-reference/video-generation-api/portrait-animation-api-reference/animateanyone-quick-start/animate-anyone-template-api.md)生成的template\_id。

**说明**动作模板ID会进行权限校验，请确保所使用的template\_id是由当前的云账号创建得到的。

> 使用建议：提高模板视频的分辨率和帧率，可有效提升生成视频的画质效果。

AACT.xxx.xxx-xxx.xxx

parameters.use\_ref\_img\_bg
...
**说明**选择按视频背景生成时，即use\_ref\_img\_bg设false时，该参数不生效。将按模板视频的比例生成新视频。
...
def sample_async_call():
    ...
    rsp = VideoSynthesis.async_call(api_key=api_key,
                                model='animate-anyone-gen2',
                                img_url='https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20251224/pkswoc/p883941.png',
                                template_id='AACT.xxx.xxx-xxx.xxx',
                                use_ref_img_bg=False,
                                video_ratio='9:16')
    print(rsp)
...
-   已提交的异步任务列表查询和异步任务的取消管理，详见[管理异步任务](raw/model-api-reference/more-about-models/manage-asynchronous-tasks.md)。
...
平台输出的视频结果，**video\_url有效期为任务完成后24小时**

[https://xxx/1.mp4](https://xxx/1.mp4)"
