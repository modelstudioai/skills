...
任务对象字段含义与状态机见 压缩任务对象；错误码统一见文末[错误码](https://help.aliyun.com/zh/model-studio/model-compression-api#ec_fpebohap72z5w)。
...
3.  已获取 API Key（参考 [获取 API Key](raw/model-api-reference/preparations/get-api-key.md)）。

> 压缩产出的模型支持的部署单元规格由所选量化模板决定，部署数量在百炼控制台**专属部署**中配置。当前压缩功能限时免费。
...
量化后产出的模型 ID（仅 SUCCEEDED 时有值），可用于[部署模型](raw/model-api-reference/model-production/deployments-api/model-deployment-text-generation-api/create-deployment-api.md)接口进行模型部署
