...
> 压缩文件体积请参见[如何将图像或视频压缩到满足要求的大小？](https://help.aliyun.com/zh/model-studio/vision)
...
-   **原因一： 模型名称与API 端点不匹配**：例如调用纯文本模型时使用了多模态接口，或者调用多模态模型时使用了纯文本接口。
    ...
    **解决方案：**
    ...
    3.  通过DashScope使用CosyVoice**声音复刻**接口时：该接口包含 `model` 与 `target_model` 参数：需将 `model` 设置为 `voice-enrollment`， `target_model` 设置为具体的 CosyVoice 模型，具体参考[CosyVoice声音复刻/设计API](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)。
...
**原因：** 超过23秒未向服务发送数据。该报错信息在使用[实时语音合成（Sambert）](https://help.aliyun.com/zh/model-studio/sambert-speech-synthesis)、[语音识别（Paraformer）](https://help.aliyun.com/zh/model-studio/paraformer-real-time-speech-recognition-api-reference)和[实时语音合成（Qwen-Audio-TTS/CosyVoice）](https://help.aliyun.com/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)时产生。
...
1.  **检查** `voice` **参数赋值**：
    ...
    -   如果使用的是声音复刻音色，请通过[CosyVoice声音复刻/设计API](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)接口确认音色状态为“OK”，并确保音色归属账号与调用账号一致。
...
> 千问VL模型支持的Content Type为：image/bmp、image/bmp、image/icns、image/x-icon、image/jpeg、image/jp2、image/png、image/sgi、image/tiff、image/webp。详情可参见[千问VL模型支持的图像](https://help.aliyun.com/zh/model-studio/vision)。
...
**解决方案：**请根据[限制与约束](https://help.aliyun.com/zh/model-studio/ssml-latex-user-guide)进行排查
...
**原因：**[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)时，上传的音频格式不符合模型要求。
...
-   [CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)更新、查询或删除接口调用时，对应音色不存在。
...
**原因：** [CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)音色数量已达限额。

**解决方案：** [删除](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)部分音色。
...
**解决方案：** 请[删除](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)一部分音色。
...
**原因：** 用于[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)的音频有效时长过短。
...
**原因：** [CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)音频文件为静音或非静音长度过短。
...
-   **调用标准模型**：使用子业务空间的API-KEY调用标准模型（例如`qwen-plus`）时，子业务空间需具备该模型的调用权限。详见[模型调用授权](raw/model-user-guide/security-and-compliance/permission-management-overview.md)。
...
-   若在使用 Coding Plan 时遇到此问题，通常是配置错误所致。Coding Plan 需要配置专属的 Base URL 和 [API Key](https://bailian.console.aliyun.com/cn-beijing/?tab=model#/efm/coding_plan)，详情请参见[Coding Plan快速开始](raw/model-user-guide/token-plan-guide/coding-plan-guide/coding-plan.md)。
...
**解决方案：** 前往[限流](raw/model-user-guide/get-started-with-models/rate-limit.md)文档查看模型限流条件并调整调用策略；如果默认额度无法满足业务需求，可在百炼控制台的[**限流提额**](https://help.aliyun.com/zh/model-studio/rate-limit#h2_temp_limit_raise)中提升模型的临时限流额度（TPM）。
...
**原因**：[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)音频文件解码失败。
...
**原因：** [CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)音频文件大小超限。
...
**原因：** [CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)音频文件采样率不支持。
...
-   如果您使用[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)，则可能的原因是：
    
    -   音频文件不规范，比如声音本身有问题，有杂音或者声音忽高忽低。请参见[录音操作指南](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)录音后重试。
    -   录音文件URL无法访问，请按照[CosyVoice声音复刻/设计API](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)中的说明操作后重试。
...
使用[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)：

-   **原因：**音频文件不规范，比如声音本身有问题，有杂音或者声音忽高忽低。
    
    **解决方案**：请参见[录音操作指南](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)录音后重试。
    
-   **原因**：录音文件URL无法访问，
    
    **解决方案**：请按照[CosyVoice声音复刻/设计API](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)中的说明操作后重试。
...
**原因：**使用[CosyVoice声音复刻](https://help.aliyun.com/zh/model-studio/voice-cloning-user-guide)时，音频文件不规范，未包含有效人声或声音不清晰，杂音大。
...
**解决方案：** 具体配置API Key的方法，请参见[配置API Key到环境变量](raw/model-api-reference/preparations/get-api-key.md)。
...
-   **通过环境变量传入 API Key 来源（推荐）**
    ...
    将`DASHSCOPE_API_KEY`设为环境变量（参见[配置API Key到环境变量](raw/model-api-reference/preparations/get-api-key.md)），初始化`client`时，通过`os.getenv`读取：
...
**解决方案：** 您可以重启客户端或IDE后重试。更多情况请参考[常见问题](https://help.aliyun.com/zh/model-studio/get-api-key#e6a57c054bfzy)。
