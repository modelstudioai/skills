...
2.  [音视频对话示例](https://github.com/aliyun/alibabacloud-bailian-speech-demo/tree/master/samples/conversation/omni/java)：麦克风和摄像头采集实时音视频输入，开启[VAD 模式](raw/model-user-guide/model-experience/omni-modal/realtime.md)（自动检测语音起止），支持语音打断。
    
    > enableTurnDetection 参数需设为 `true`。
    
    > 推荐您使用耳机播放音频，避免回声触发语音打断。
    
3.  [本地调用](https://github.com/aliyun/alibabacloud-bailian-speech-demo/tree/master/samples/conversation/omni/java)：本地音频和图片作为输入，开启[Manual 模式](raw/model-user-guide/model-experience/omni-modal/realtime.md)（手动控制发送节奏）。
