...
1.  **获取子业务空间的API Key：**在子业务空间中[创建API Key](raw/model-api-reference/preparations/get-api-key.md)，并[配置到环境变量](raw/model-api-reference/preparations/get-api-key.md)（变量名使用`DASHSCOPE_API_KEY`），用于调用大模型API。
...
> 调用在阿里云百炼[调优](raw/model-user-guide/fine-tuning/fine-tune-text-generation-model/model-training-overview.md)后的模型：仅支持通过[DashScope](https://help.aliyun.com/zh/model-studio/model-calling-in-sub-workspace#32a12dae0d6ze)调用，不支持通过OpenAI兼容方式调用。
