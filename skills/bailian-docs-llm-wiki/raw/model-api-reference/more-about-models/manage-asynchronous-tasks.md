...
异步任务接口与具体任务类型无关，`fetch`、`list`、`cancel` 方法在任意异步任务类（如 `ImageSynthesis`、`VideoSynthesis`、`Transcription` 等）上均可调用，底层均请求 `/api/v1/tasks`，可查询当前账号下任意任务。
...
# 查询指定任务状态（以 video-synthesis 为例）
dashscope video-synthesis fetch <task_id>
```

**说明**dashscope CLI 目前仅 `video-synthesis` 子命令支持 `fetch`/`list`/`cancel` 任务管理；`image-synthesis`、`transcription` 等子命令暂未提供。查询其他类型任务请用上方 Python SDK 或 curl。
...
`list` 还支持按 `model_name`、`api_key_id`、`region` 等条件筛选，参数与 HTTP API 的 Query 参数一一对应。

**说明**`fetch`、`list`、`cancel` 在工作空间专属域名（`{WorkspaceId}.{region}.maas.aliyuncs.com`）和通用域名（`dashscope.aliyuncs.com`）均支持，跨地域（如 cn-beijing、cn-shanghai、cn-hangzhou）均可调用。调用时 API Key 须与对应业务空间匹配。

#### dashscope CLI

```
export DASHSCOPE_API_KEY="your-api-key"
# 列出视频合成任务（支持 --start-time/--end-time/--status/--model-name 等筛选）
dashscope video-synthesis list --status RUNNING -s 10
```

**说明**dashscope CLI 目前仅 `video-synthesis` 子命令支持任务管理；查询其他类型任务（图像、语音转写等）请用上方 Python SDK 或 curl。
...
仅状态为 `PENDING` 的任务可取消，其他状态会返回失败。

#### dashscope CLI

```
export DASHSCOPE_API_KEY="your-api-key"
# 取消任务（以 video-synthesis 为例，仅 PENDING 状态可取消）
dashscope video-synthesis cancel <task_id>
```

**说明**dashscope CLI 目前仅 `video-synthesis` 子命令支持 `cancel`；取消其他类型任务请用上方 Python SDK 或 curl。
