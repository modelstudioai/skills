...
在[API Key（北京）](https://bailian.console.aliyun.com/model/settings/api-key)、[API Key（新加坡）](https://bailian.console.aliyun.com/model/settings/api-key)或[API Key（弗吉尼亚）](https://bailian.console.aliyun.com/us-east-1/model/settings/api-key)或[API Key（中国香港）](https://bailian.console.aliyun.com/cn-hongkong/settings/api-key)页面创建永久API Key，并将其配置为环境变量 `DASHSCOPE_API_KEY`。配置方法请参见[配置API Key 到环境变量](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)。
