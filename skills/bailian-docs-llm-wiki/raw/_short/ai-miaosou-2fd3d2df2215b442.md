...
-   妙搜的收费方式是基于实际调用模型所消耗的输入和输出总Token数，以后付费的方式进行计费。具体请参见[妙搜-计费文档](raw/_short/miaosou-miaodu-api-billing-ef8dfbbfa6d4026e.md)。
-   您可点击[链接](https://common-buy.aliyun.com/?commodityCode=sfm_quanmiaoAPI_public_cn)开通全妙智能检索生成应用后付费API。
-   您可通过直接调用妙搜API（[妙搜-数据源](raw/_short/api-aimiaobi-2023-08-01-dir-wonderful-search-dat-5a0812c7a1fd2fb7.md)、[妙搜-智能搜索](raw/_short/api-aimiaobi-2023-08-01-dir-wonderful-search-sma-0c7d8a5864b0de00.md)）或者通过[妙搜产品控制台](https://aimiaobi.console.aliyun.com/?product_code=g_broadscope_search#/home)来使用妙搜的功能。
...
妙搜支持对接云存储（OSS）作为存储数据源，实现对其中数据的搜索与问答。具体的对接授权步骤参考《[云存储（OSS）设置指南](raw/_short/oss-setup-guide-93a972563f8f04a6.md)》。

#### 类型三：视频理解Agent引入数据源

对于视频素材，若需更精细的内容解析，可通过“影视传媒视频理解 Agent”对视频进行深度理解与分析，并将分析结果作为一路元数据（meta data）源导入妙搜。该元数据将与妙搜自有存储中的数据融合，共同参与后续的搜索与问答流程。

**说明**影视传媒视频理解的操作文档参考《[影视传媒视频理解](raw/_short/media-video-understanding-70e151fb2cf096b8.md)》。
...
1.  点击**配置API**，设置数据源的名称。
2.  修改配置信息接入外部API数据源，详情请参见[妙搜-通过API引入数据源](raw/_short/miaosou-introduce-data-source-through-api-0dcfe8b545d9a547.md)。
...
支持，具体的配置方法可以参考《[云存储（OSS）设置指南](raw/_short/oss-setup-guide-93a972563f8f04a6.md)》。
