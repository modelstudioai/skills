...
-   **前置条件**：Travel 必须尚未进入终态，并处于服务端可结束的状态。可通过[查询Travel状态](raw/_short/happyoyster-adventure-query-travel-status-api-re-c8c769da81dc5895.md)接口确认。
-   **调用方**：您的服务端或客户端均可调用。
...
要结束的 Adventure 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-adventure-enter-travel-api-reference-3eb107188008bbaf.md)接口返回。

**failCode** `string` **（可选）**

无推流超时的失败结束原因。省略时按正常结束处理。当前仅支持：

-   `TRAVEL_NO_STREAM_AUTO_END`：用于[进入房间](raw/_short/happyoyster-adventure-enter-travel-api-reference-3eb107188008bbaf.md)后超过 `noStreamAutoEndTimeoutSec` 仍未收到推流的场景，该结束方式不生成回放产物
...
-   正常结束为 `completed` 后，合成产物仍可能继续处理，应按[查询Travel产物](raw/_short/happyoyster-adventure-query-travel-artifacts-api-ae37da872a0bc999.md)查询并轮询。
...
-   [查询Travel产物](raw/_short/happyoyster-adventure-query-travel-artifacts-api-ae37da872a0bc999.md)：轮询获取录制原片和合成变体。
-   [查询Travel列表](raw/_short/happyoyster-adventure-query-travel-list-api-refe-5cc62593b2e05dc6.md)：查看历史 Travel。
