...
-   **前置条件**：Travel 的 `creationModel` 必须为 `simple`，状态需可接收指令。可通过[查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)接口确认。
-   **调用方**：您的服务端或客户端均可调用。
...
要控制的 Directing 加密 Travel ID，`creationModel` 必须为 `simple`。由[客户端进入房间](raw/_short/happyoyster-directing-enter-travel-api-reference-094ec4d460aff9ef.md)接口返回。
...
-   仅 `creationModel=simple` 支持本接口；`scriptlist` Travel 应使用[剧本全量更新](raw/_short/happyoyster-directing-update-script-api-referenc-559783f11176cb54.md)，误调用 `instruct` 返回 `403006`。
...
-   [查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)：查看指令执行状态和章节信息。
-   [暂停Travel](raw/_short/happyoyster-directing-pause-travel-api-reference-21bb582a9c4e8b95.md) / [恢复Travel](raw/_short/happyoyster-directing-resume-travel-api-referenc-594ac8725680fe8b.md) / [回溯Travel](raw/_short/happyoyster-directing-rewind-travel-api-referenc-869fcb622ca2907a.md)。
