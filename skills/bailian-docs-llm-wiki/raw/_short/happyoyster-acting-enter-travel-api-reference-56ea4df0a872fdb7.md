...
-   **前置条件**：`ticket` 由[获取体验凭证](raw/_short/happyoyster-acting-get-travel-credential-api-ref-dfc7dc5832959ca8.md)接口换取，未过期且未使用，对应 World 状态为 `ready`。
-   **调用方**：您的客户端调用。
...
未过期且未使用的一次性凭证。由[获取体验凭证](raw/_short/happyoyster-acting-get-travel-credential-api-ref-dfc7dc5832959ca8.md)接口换取。同一 `ticket` 再次使用返回 `401011`。
...
新创建的加密 Travel ID。后续[查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)、暂停、恢复、结束、产物查询均使用此值。
...
无推流自动结束超时秒数，默认 30，以实际响应为准。进房后若在此时间内 `rtcStatus` 未进入推流态，应调用[结束Travel](raw/_short/happyoyster-acting-end-travel-api-reference-07665d664cc3dbc3.md)并传入 `failCode=TRAVEL_NO_STREAM_AUTO_END`。
...
-   [查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)：每 2–5 秒轮询，Travel 为 `running` 或 `paused` 时可发送指令。
-   [发送文本过程指令](raw/_short/happyoyster-acting-instruct-travel-api-reference-5c29e8efed3870dd.md)：驱动后续角色表演。
-   [暂停Travel](raw/_short/happyoyster-acting-pause-travel-api-reference-65ff563386f561f7.md) / [恢复Travel](raw/_short/happyoyster-acting-resume-travel-api-reference-e2cfa0a73db72db0.md)：按需控制运行节奏。
-   [结束Travel](raw/_short/happyoyster-acting-end-travel-api-reference-07665d664cc3dbc3.md)：结束时进入 `completed`，开始处理产物。
