...
-   **前置条件**：World 必须属于当前主账号和 Directing 模型，且状态为 `ready`。可通过[查询World构建状态](raw/_short/happyoyster-directing-query-world-build-status-a-48bf75e27c43945d.md)接口确认。
...
当前主账号名下、状态为 `ready` 的 Directing World ID。由[创建World](raw/_short/happyoyster-directing-create-world-api-reference-6f3bdbbb6537960b.md)接口返回。
...
-   调用本接口不会创建 Travel；Travel 在[进入房间](raw/_short/happyoyster-directing-enter-travel-api-reference-094ec4d460aff9ef.md)成功时创建。
...
-   [客户端进入房间](raw/_short/happyoyster-directing-enter-travel-api-reference-094ec4d460aff9ef.md)：客户端使用 `ticket` 创建 Travel 并获取 RTC 配置。
