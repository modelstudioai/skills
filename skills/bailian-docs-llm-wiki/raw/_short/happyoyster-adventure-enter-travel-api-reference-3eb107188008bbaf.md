...
-   **前置条件**：`ticket` 由[获取体验凭证](raw/_short/happyoyster-adventure-get-travel-credential-api--d269f4908c7288f2.md)接口换取，未过期且未使用，对应 World 状态为 `ready`。
-   **调用方**：您的客户端调用。
...
未过期且未使用的一次性凭证。由[获取体验凭证](raw/_short/happyoyster-adventure-get-travel-credential-api--d269f4908c7288f2.md)接口换取。同一 `ticket` 再次使用返回 `401011`。
...
新创建的加密 Travel ID。后续[查询Travel状态](raw/_short/happyoyster-adventure-query-travel-status-api-re-c8c769da81dc5895.md)、结束、产物查询均使用此值。
...
无推流自动结束超时秒数，默认 30，以实际响应为准。进房后若在此时间内 `rtcStatus` 未进入推流态，应调用[结束Travel](raw/_short/happyoyster-adventure-end-travel-api-reference-f46d5ab187c27653.md)并传入 `failCode=TRAVEL_NO_STREAM_AUTO_END`。
...
-   [查询Travel状态](raw/_short/happyoyster-adventure-query-travel-status-api-re-c8c769da81dc5895.md)：每 2–5 秒轮询，读取可用动作池。
-   Travel 为 `running` 后，客户端通过 SDK `sendCommand` 发送方向、视角和动作控制。
-   [结束Travel](raw/_short/happyoyster-adventure-end-travel-api-reference-f46d5ab187c27653.md)：结束会话并处理产物。
