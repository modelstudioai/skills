...
**用户指南**：关于模型介绍和选型建议请参见[实时（Qwen-Omni-Realtime）](raw/model-user-guide/model-experience/omni-modal/realtime.md)或[非实时（Qwen-Omni）](raw/model-user-guide/model-experience/omni-modal/qwen-omni.md)。

**重要**本文档专用于千问Omni和千问Omni-Realtime声音复刻接口；若您使用的是语音合成模型，请参见[语音合成](raw/model-api-reference/audio-api-references/speech-synthesis-api-reference.md)。
...
适用于千问Omni-Realtime系列模型，更多说明请参见[实时（Qwen-Omni-Realtime）](raw/model-user-guide/model-experience/omni-modal/realtime.md)。
...
适用于千问Omni系列模型，更多说明请参见[非实时（Qwen-Omni）](raw/model-user-guide/model-experience/omni-modal/qwen-omni.md)。
...
**重要**`parameters`中的`voice_clone_mode`参数在使用qwen3.8-omni系列模型时必传。不传该字段时，默认回退到qwen3.5-omni talker system instruct模式（兼容旧服务）。
...
-   **请求参数**
    ...
    **说明**
    ...
    用于复刻的音频（录制时需遵循[录音操作指南](https://help.aliyun.com/zh/model-studio/qwen-omni-voice-cloning#8d342f3949vge)，音频需满足[音频要求](raw/_short/qwen-omni-voice-cloning-717550bc449e9e29.md)）。
    ...
    声音复刻模式。使用qwen3.8-omni系列模型时必传，不传时默认回退到qwen3.5-omni talker system instruct模式（兼容旧服务）。
    ...
    是否跳过音频预处理。默认为`false`。该参数需放在`parameters`中。
...
-   **示例代码**
    ...
    **重要**注意区分如下参数：
    ...
    ```
    {"output":{"target_model":"qwen3.8-omni-flash-realtime","voice":"qwen-omni-vc-chinese-voice-20260911161744020-3f9d"},"usage":{"count":1},"request_id":"7a36a3cb-be34-9be8-a127-df2449d51ab2"}
    ```
