...
**相关指南：**[新版智能体应用（Agent 2.0）](raw/application-user-guide/llm-application/new-single-agent-application.md)。
...
此参数用于在[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)中切换思考模式和非思考模式。
...
图片列表。支持图像 URL 和 Data URL（Base64 编码）。应用内需选择**[图像与视频理解](raw/model-user-guide/model-experience/vision-model/vision.md)**模型。

`base64`编码格式可构建为 [Data URL](https://www.rfc-editor.org/rfc/rfc2397)：`data:[MIME_type];base64,{base64_image}`。详细说明和代码示例见本文档[视觉理解章节](raw/_short/new-agent-application-api-reference-d745b325d97fcf2e.md)。
...
通过 `image_list` 参数传入图像 URL 或 Data URL（Base64 编码）启用视觉理解功能。应用内需使用**[图像与视频理解](raw/model-user-guide/model-experience/vision-model/vision.md)**模型。
...
调用时将`has_thoughts`参数设置为True，即可在`thoughts`中查看[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)的思考过程。
...
1.  控制台应用内选择[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)，并成功发布；
2.  API 调用时将 `has_thoughts` 参数设为 `True`。

**action\_type** `string`

大模型返回的执行步骤类型。如reasoning表示[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)的思考过程。
...
**doc\_references** `null`

新版智能体应用（Agent 2.0）将知识库统一为工具由智能体自主调用，该字段固定返回 `null`（与旧版智能体应用 Agent 1.0 不同）。如需在回答中展示知识来源，请在应用配置中开启**展示回答来源**功能，引用信息将以角标形式嵌入回答文本中。
