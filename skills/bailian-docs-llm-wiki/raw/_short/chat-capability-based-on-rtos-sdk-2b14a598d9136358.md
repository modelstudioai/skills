...
参考[设备端C SDK](raw/application-user-guide/application-gallery/multimodal-products/multimodal-sdk/multimodal-mobile-sdk/mmi-rtos-sdk.md)，根据芯片型号，下载相应的 SDK 包。

如采用半托管模式，需要根据[设备端C SDK](raw/application-user-guide/application-gallery/multimodal-products/multimodal-sdk/multimodal-mobile-sdk/mmi-rtos-sdk.md)，完成服务端相关接口对接。
...
`qwen_sdk_<PLATFORM>_<SDK_VERSION>.tar.gz`
...
-   **内存模块（**`qwen_sdk/include/c_utils/hal_util_mem.h`**）**
...
-   **随机数模块（**`qwen_sdk/include/c_utils/hal_util_random.h`**）**
...
-   **存储模块（**`qwen_sdk/include/c_utils/hal_util_storage.h`**）**
...
-   **时间模块（**`qwen_sdk/include/c_utils/hal_util_time.h`**）**
...
-   **互斥锁模块（**`qwen_sdk/include/c_utils/hal_util_mutex.h`**）**
...
在实现以上各模块之后，加载`libqwen_test.a`，在主程序中直接调用函数`qwen_sdk_test()`，就会对以上工作模块进行测试，输出日志可查看测试结果。
...
```
[UT][I][qwen_sdk_test]********************* Hal Test Start *********************
[UT][I][qwen_sdk_test]********************* memory test done *******************
[UT][I][qwen_sdk_test]time is 1753344478377
[UT][I][qwen_sdk_test]********************* time test done *********************
[UT][I][qwen_sdk_test]********************* storage test done ******************
[UT][I][qwen_sdk_test]********************* random test done *******************
[UT][I][qwen_sdk_test]********************* mutex test done ********************
[UT][I][qwen_sdk_test]********************* Hal Test End ***********************
```
...
enum {
    ...
    C_MMI_EVENT_STATE_CHANGE,     // major state 切换时触发，param = uint8_t* 指向新状态值
    ...
    C_MMI_EVENT_WORD_TIMESTAMP,   // 此事件返回完整word_timestamps cJSON对象（流式更新），由应用按格式消费，param仅回调期间有效，需使能word_timestamp
    C_MMI_EVENT_LATENCY,          // 当收到云端下发的延迟统计时触发此事件，值为ASR结束到TTS首包生成的耗时（仅含云端时间），param指向uint32_t毫秒值，仅回调期间有效
