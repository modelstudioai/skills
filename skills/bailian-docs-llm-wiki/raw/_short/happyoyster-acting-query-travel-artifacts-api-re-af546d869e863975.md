...
-   **前置条件**：Travel 状态为 `completed`。原片 URL 可用是返回产物响应的前提。可通过[查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)接口确认。
...
状态为 `completed` 的 Acting 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-acting-enter-travel-api-reference-56ea4df0a872fdb7.md)接口返回。
...
-   收到 `404000` 时不要一律展示为「尚未完成」：请先通过[查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)或[查询Travel列表](raw/_short/happyoyster-acting-query-travel-list-api-referen-643c3b55678ff459.md)读取 `status` 与 `errorCode`，`failed` 的 Travel 应展示失败原因。
...
-   [查询Travel列表](raw/_short/happyoyster-acting-query-travel-list-api-referen-643c3b55678ff459.md)：查看其它历史 Travel。
-   [结束Travel](raw/_short/happyoyster-acting-end-travel-api-reference-07665d664cc3dbc3.md)：结束新的 Travel 后再来查询产物。
