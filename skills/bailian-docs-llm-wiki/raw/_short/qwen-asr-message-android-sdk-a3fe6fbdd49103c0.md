# Qwen-Audio-ASR-Message实时语音识别Android SDK

本文档提供了Qwen-Audio-3.1-ASR-Flash-Message实时语音识别Android SDK的详细使用指南，帮助您将语音转换为文本。
...
1.  初始化 SDK
2.  按业务需求设置参数：通过[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口的`parameters`参数设置[连接与控制参数](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#57acf5ecc1w8j)；通过[setParams](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#a23e0d85d7ymt)接口设置[语音识别效果参数](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#d20cce9518kla)。
3.  调用[startDialog](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#7d33691bdb32v)启动识别流程。
4.  在[onNuiAudioStateChanged](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#bc71fe2545pfy)回调中，根据音频状态开启录音设备。
5.  在[onNuiNeedAudioData](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#12577ebc97awb)回调中持续提供录音数据，或者通过[updateAudio](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ed321b83adxga)持续推送录音数据。
6.  在[onNuiEventCallback](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#163c1ef871tqt)回调中监听事件并获取语音识别结果。
7.  调用[stopDialog](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#047417083bahi)停止识别，并通过监听EVENT\_TRANSCRIBER\_COMPLETE事件确认识别已结束。
8.  当识别功能不再使用时，调用[release](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#44bf12ed9a4g9)接口释放 SDK 资源。

## 请求参数

### 连接与控制参数

通过在[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口的`parameters`参数中传入一个JSON字符串来配置。
...
此参数仅在调用[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口时将`save_log`设为true时生效。此时必须设置日志文件路径，否则将报错。
...
此参数仅在调用[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口时将`save_log`设为true时生效。 同时，`debug_path`也必须被设置。
...
此参数仅在调用[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口时将`save_log`设为true时生效。
...
注意：`log_track_level`与`level`（通过[initialize](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#ae6d7dd9cfad3)接口设置）共同决定最终回调的日志。一条日志的级别数值必须同时大于或等于`log_track_level`和`level`的值，才会被回调。例如，`log_track_level`设为2 (INFO)，`level`设为3 (WARNING)，则只有WARNING及以上级别（数值>=3）的日志才会被回调。
...
通过在[setParams](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#a23e0d85d7ymt)接口的`params`参数中传入一个JSON字符串来配置。
...
初始化语音识别SDK实例。SDK为单例模式，在调用[release](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#44bf12ed9a4g9)前禁止重复初始化。
...
JSON字符串，包含鉴权、连接和调试参数。参见[连接与控制参数](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#57acf5ecc1w8j)。
...
如果[连接与控制参数](https://help.aliyun.com/zh/model-studio/qwen-asr-message-android-sdk#57acf5ecc1w8j)的`apikey`参数使用的是[临时API Key](raw/model-api-reference/more-about-models/generate-temporary-api-key.md)，当其过期时，可在此处进行更新。
