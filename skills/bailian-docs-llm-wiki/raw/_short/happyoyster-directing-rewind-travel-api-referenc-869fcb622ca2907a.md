...
-   **前置条件**：Travel 必须先进入 `paused` 状态，不能在 `running` 时直接回溯。可通过[查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)接口确认。
-   **调用方**：您的服务端或客户端均可调用。
...
要回溯的 Directing 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-directing-enter-travel-api-reference-094ec4d460aff9ef.md)接口返回。
...
-   [暂停Travel](raw/_short/happyoyster-directing-pause-travel-api-reference-21bb582a9c4e8b95.md)存在约 3 秒的异步确认屏障。收到 pause 响应后，仍应轮询[查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)，确认对外状态已经是 `paused` 再调用本接口。
...
-   [查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)：确认运行状态。
-   普通模式：[发送过程指令](raw/_short/happyoyster-directing-instruct-travel-api-refere-2cd5aa6eabceb2c4.md)。
-   [暂停Travel](raw/_short/happyoyster-directing-pause-travel-api-reference-21bb582a9c4e8b95.md)：再次暂停以进行下一次回溯。
