...
-   **调用模式**：推荐使用异步模式。
    ...
    -   **异步模式**（默认）：`async=true`，接口立即返回 `encryptedWorldId`，需轮询[查询World构建状态](raw/_short/happyoyster-acting-query-world-build-status-api--5436f98af63bbe85.md)获取进度。
...
-   `true`：立即返回，World 在后台构建，客户端轮询[查询World构建状态](raw/_short/happyoyster-acting-query-world-build-status-api--5436f98af63bbe85.md)。
...
-   [查询World构建状态](raw/_short/happyoyster-acting-query-world-build-status-api--5436f98af63bbe85.md)：每 3–5 秒轮询，直到 World 进入 `ready`。
-   World 进入 `ready` 后，调用[获取体验凭证](raw/_short/happyoyster-acting-get-travel-credential-api-ref-dfc7dc5832959ca8.md)换取一次性 `ticket`。
