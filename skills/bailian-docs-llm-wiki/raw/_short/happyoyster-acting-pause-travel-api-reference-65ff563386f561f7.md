...
-   **前置条件**：Travel 状态需为 `running`。可通过[查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)接口确认。
-   **调用方**：您的服务端或客户端均可调用。
...
要暂停的 Acting 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-acting-enter-travel-api-reference-56ea4df0a872fdb7.md)接口返回。
...
-   暂停确认是异步过程。收到成功响应后，如果需要立即恢复或发送依赖暂停态的操作，应继续轮询[查询Travel状态](raw/_short/happyoyster-acting-query-travel-status-api-refer-bb36f15c26e6c92c.md)，直到查询状态为 `paused`。
...
-   [恢复Travel](raw/_short/happyoyster-acting-resume-travel-api-reference-e2cfa0a73db72db0.md)：恢复已暂停的 Travel 继续生成。
-   [发送文本过程指令](raw/_short/happyoyster-acting-instruct-travel-api-reference-5c29e8efed3870dd.md)：在 `paused` 状态发送指令只会接受指令，不会自动恢复。
-   [结束Travel](raw/_short/happyoyster-acting-end-travel-api-reference-07665d664cc3dbc3.md)：直接结束 Travel。
