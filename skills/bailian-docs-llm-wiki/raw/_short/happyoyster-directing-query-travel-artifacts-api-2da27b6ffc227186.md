...
-   **前置条件**：Travel 状态为 `completed`。原片可用是返回产物响应的前提。可通过[查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)接口确认。
...
状态为 `completed` 的 Directing 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-directing-enter-travel-api-reference-094ec4d460aff9ef.md)接口返回。
...
-   不要将 `404000` 一律展示为「尚未完成」：请先通过[查询Travel状态](raw/_short/happyoyster-directing-query-travel-status-api-re-d536083e25c2aee5.md)或[查询Travel列表](raw/_short/happyoyster-directing-query-travel-list-api-refe-d243ea07e641f43d.md)读取 `status` 与 `errorCode`；对 `failed` 的 Travel 展示失败原因。
...
-   [查询Travel列表](raw/_short/happyoyster-directing-query-travel-list-api-refe-d243ea07e641f43d.md)：查看其它历史 Travel。
-   [结束Travel](raw/_short/happyoyster-directing-end-travel-api-reference-5fe76226682ff253.md)：结束新的 Travel 后再来查询产物。
