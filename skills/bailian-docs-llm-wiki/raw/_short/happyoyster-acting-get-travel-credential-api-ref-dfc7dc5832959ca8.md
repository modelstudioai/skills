...
-   **前置条件**：World 必须属于当前主账号和 Acting 模型，且状态为 `ready`。可通过[查询World构建状态](raw/_short/happyoyster-acting-query-world-build-status-api--5436f98af63bbe85.md)接口确认。
...
当前主账号名下、状态为 `ready` 的 Acting World ID。由[创建World](raw/_short/happyoyster-acting-create-world-api-reference-47f1f4ec26522c7d.md)接口返回。
...
-   调用本接口不会创建 Travel；Travel 在[进入房间](raw/_short/happyoyster-acting-enter-travel-api-reference-56ea4df0a872fdb7.md)成功时创建。
...
-   [客户端进入房间](raw/_short/happyoyster-acting-enter-travel-api-reference-56ea4df0a872fdb7.md)：客户端使用 `ticket` 创建 Travel 并获取 RTC 配置。
