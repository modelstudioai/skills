...
用于更新会话配置，建议在连接建立后首先调用该方法进行设置。若未调用该方法，系统将使用默认配置。只需关注[请求参数](raw/_short/qwen-asr-realtime-java-sdk-70b383598ecfae85.md)中的涉及到的参数。
...
-   [请求参数](raw/_short/qwen-asr-realtime-java-sdk-70b383598ecfae85.md)`enableTurnDetection`设为`true`，音频缓冲区用于检测语音，服务端决定何时提交。
-   [请求参数](raw/_short/qwen-asr-realtime-java-sdk-70b383598ecfae85.md)`enableTurnDetection`设为`false`，客户端可以选择每个事件中放置多少音频量，最多放置 15 MiB。 例如，从客户端流式处理较小的数据块可以让 VAD 响应更迅速。
...
**禁用场景：**[请求参数](raw/_short/qwen-asr-realtime-java-sdk-70b383598ecfae85.md)`enableTurnDetection`设为`true`时。
