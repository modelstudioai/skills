...
-   **前置条件**：Travel 状态为 `completed`。原片 URL 可用是返回产物响应的前提。可通过[查询Travel状态](raw/_short/happyoyster-adventure-query-travel-status-api-re-c8c769da81dc5895.md)接口确认。
...
状态为 `completed` 的 Adventure 加密 Travel ID。由[客户端进入房间](raw/_short/happyoyster-adventure-enter-travel-api-reference-3eb107188008bbaf.md)接口返回。
...
-   不要将 `404000` 一律展示为「尚未完成」：请先通过[查询Travel状态](raw/_short/happyoyster-adventure-query-travel-status-api-re-c8c769da81dc5895.md)或[查询Travel列表](raw/_short/happyoyster-adventure-query-travel-list-api-refe-5cc62593b2e05dc6.md)读取 `status` 与 `errorCode`；对 `failed` 的 Travel 展示失败原因。
...
-   [查询Travel列表](raw/_short/happyoyster-adventure-query-travel-list-api-refe-5cc62593b2e05dc6.md)：查看其它历史 Travel。
-   [结束Travel](raw/_short/happyoyster-adventure-end-travel-api-reference-f46d5ab187c27653.md)：结束新的 Travel 后再来查询产物。
