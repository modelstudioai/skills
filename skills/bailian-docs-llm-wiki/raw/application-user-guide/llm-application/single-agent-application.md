...
1.  访问阿里云百炼控制台[应用管理](https://bailian.console.aliyun.com/?tab=app#/app-center)页面，单击**创建应用**，在**智能体应用**页签，单击**立即创建**。
2.  在应用配置界面的模型选择下拉菜单中选择模型，例如 `千问-Plus`。其他参数可保持默认。
3.  创建完成后，在左侧对话框中输入 `你好` 进行测试。
...
1.  **模型选择**：在模型选择下拉菜单中选择模型，例如 `千问-Plus`。单击**更多模型**可选择其他模型。
2.  **参数配置**：单击模型下拉框右侧的设置图标，配置以下参数：

-   **最长回复长度**：模型生成内容的长度上限，不含提示词。上限因模型不同而有所差异。
-   **携带的上下文轮数**：输入模型的最大历史对话轮数，轮数越多，对话相关性越强。
-   **温度系数**：控制生成内容的随机性。数值越高，多样性越强；数值越低，一致性越强。取值范围为 \[0, 2)。
-   **推理模式**：是否开启推理模式。不支持推理模式的模型无法配置此参数。
...
智能体通过调用插件，完成代码执行、网络搜索、文本生成图片等具体任务。插件适用于需要执行超出内置知识范围的操作或内容生成时。百炼提供多种官方插件，同时支持添加自定义插件，详情参见[插件概述](raw/application-user-guide/plug-in/plug-in-overview.md)。
...
应用发布后，支持通过 API/SDK 调用，也可发布到钉钉、微信公众号等第三方平台，或发布为可复用组件集成到业务系统中。

### 发布应用

**警告**发布应用是后续调用和集成的前提条件。

单击智能体应用管理界面右上角的**发布**，再单击**确认发布**，即可完成发布。

**说明**若非首次发布，弹窗会展示自上次发布以来的变更详情。如果应用由RAM账号创建，发布前请确认已拥有服务关联角色权限 `ram:CreateServiceLinkedRole`，详情参见[服务关联角色](raw/application-api-reference/more/bailian-service-linked-role.md)。

### API 调用

在智能体应用的**发布渠道**页签，单击**API 调用**右侧的**查看 API**，查看API调用方法。

**说明**将 `YOUR_API_KEY` 替换为实际的百炼API Key 后方可发起调用。

## 智能体管理

### 复制与删除

在**应用管理**页面找到应用卡片，通过**更多 > 复制应用/删除应用**菜单，对智能体进行复制、删除或重命名操作。
...
**说明**数据更新可能存在延迟，模型支持情况以智能体应用内显示为准。
