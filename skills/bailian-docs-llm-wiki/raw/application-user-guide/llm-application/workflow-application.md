...
1.  连接大模型节点和结束节点。再单击结束节点，在其配置面板的编辑框中输入`/`插入变量`大模型1/result`，未提及的参数保持默认即可。

配置样例：

1.  单击右上角**测试**，输入`你的包裹已在驿站存放多日未提取，请抽空过来取件`，单击。等待工作流运行结束后，查看输出结果。

1.  继续输入`你有一条中奖100万的信息，请查收`，单击。等待工作流运行结束后，查看输出结果。

1.  如果当前工作流应用可以正常工作，请单击右上角**发布**应用，以备后续[调用工作流应用](raw/application-user-guide/bailian-application-calling/invoke-workflow-application.md)。
...
1.  将左侧**意图分类**节点拖入画布，在其配置面板中配置以下参数。未提及的参数保持默认即可。
    ...
    -   **意图分类**：单击**添加意图**，分别添加以下3个意图
        ...
        -   消费者想选购手机
        -   消费者想选购冰箱
    -   **记忆**：让模型在对话中记住之前的聊天内容。单击启用，选择**自定义缓存**。
...
1.  将左侧**大模型**节点拖入画布，在其配置面板中配置以下参数。未提及的参数保持默认即可。配置完成后关闭面板，连接意图分类的**消费者想选购冰箱**和**大模型**节点。
...
1.  将左侧**大模型**节点拖入画布，在其配置面板中配置以下参数。未提及的参数保持默认即可。配置完成后关闭面板，连接意图分类的**消费者想选购电视**和**大模型**节点。
...
1.  将左侧**大模型**节点拖入画布，在其配置面板中配置以下参数。未提及的参数保持默认即可。配置完成后关闭面板，连接意图分类的**消费者想选购手机**和**大模型**节点。
...
1.  将左侧**变量处理**节点拖入画布，在其配置面板中配置以下参数。未提及的参数保持默认即可。配置完成后关闭面板，连接意图分类节点的**默认**意图和**变量处理**节点。
...
1.  将三个大模型节点和**变量处理**节点都连接到**结束**节点。再单击结束节点，在其配置面板的编辑框中分别输入`/`插入4个变量`大模型1/result``大模型2/result``大模型3/result``变量处理1/result`，未提及的参数保持默认即可。

配置样例：

2.  单击右上角**测试**，在对话框输入`给我介绍一下你们的冰箱`，单击，查看输出结果。
...
1.  访问[应用管理](https://bailian.console.aliyun.com/?tab=app#/app-center)页面，单击**创建应用>智能体应用**，填写**应用名称**后单击**立即创建**。[创建并发布](raw/application-user-guide/llm-application/single-agent-application.md)信息收集智能体应用和数据整理智能体应用。
...
1.  配置智能体群组节点：从左侧节点列表中拖动一个**智能体群组**节点到画布中。在其配置面板中配置以下参数。
...
1.  配置结束节点：配置完成后关闭面板，再连接开始节点、智能体群组节点、结束节点。
    
    再单击结束节点，在其配置面板的编辑框中输入`/`插入变量`智能体群组1/result`，未提及的参数保持默认即可。
    

1.  测试应用：单击右上角**测试**，输入`后天早上11点对齐百炼项目进度`，测试应用效果。
...
-   **如何使用**
    -   **开始节点**
        ...
        **说明**
        ...
        触发器用于设置工作流的自动启动方式。未配置触发器时，工作流通过手动测试或 API 调用启动。一个开始节点可添加多个触发器，各触发器独立生效。支持以下触发方式：
        
        -   **定时触发**：按时间规则周期性启动工作流，支持可视化配置和 Cron 表达式两种规则设定方式。
            
        -   **预置事件**：由外部渠道事件启动工作流，支持微信智能机器人-长连接版本、Gitee、QQ 机器人长连接、钉钉应用机器人长连接、飞书长连接、GitHub。
            
        -   **Webhook**：由外部系统通过 HTTP 请求启动工作流，支持配置 Header、Param 和 Body。
...
-   **节点参数配置**
    ...
    **批处理配置**：
    ...
    -   **批处理次数上限**（范围1-100，普通用户默认100）：批处理运行的次数上限。
        ...
        **说明**实际批次处理次数取决于用户输入数组中的最小长度，若没有输入变量，则取决于配置中的批次数量。
    ...
    如需了解各模型的 API 调用速率限制，请参阅[限流](raw/model-user-guide/get-started-with-models/rate-limit.md)。
    ...
    **开启搜索**：启用后，允许大模型在互联网上搜索相关信息。如果没有看到该参数，则代表当前模型不支持开启搜索开关。
    ...
    以下参数仅选择 [深度思考](https://help.aliyun.com/zh/model-studio/deep-thinking) 模型时支持：
    ...
    仅在 API 调用应用时生效，用于决定是否输出节点内容。如需了解该组件的用途，请参阅[应用调用](https://help.aliyun.com/zh/model-studio/application-calling-guide)。
    
    **说明**如需通过API集成应用，请参阅[应用调用](https://help.aliyun.com/zh/model-studio/application-calling-guide)。
...
-   **输入配置**
    ...
    **说明**
    ...
    需要检索的图片信息。可输入公网可访问的图片链接（如 `[https://xxx.xxx.com/xxx/xxx.jpeg](https://xxx.xxx.com/xxx/xxx.jpeg)`），或在编辑框中输入 `/` 触发变量选择，引用上游节点的输出变量。
...
-   **知识库选择方式**
    ...
    **配置说明**
    ...
    配置 `CodeList` 变量（类型为 `Array<String>`），通过变量引用或直接输入指定知识库列表。
...
-   **定义**
    ...
    MCP 节点可以为工作流接入海量 [MCP](raw/application-user-guide/model-context-protocol/mcp-introduction.md) 服务工具，帮助您快速接入外部资源。每个 MCP 节点可以接入一个工具。
...
-   **定义**
    ...
    智能体或工作流应用可发布为模块化的组件，供其他智能体或工作流应用接入使用，实现功能复用，加速复杂 AI 应用开发。您也可以将智能体或工作流应用发布为组件，详情请参考[发布为组件](raw/application-user-guide/application-publishing-and-sharing/use-agent-or-workflow-as-component.md)。
...
**说明**为确保 API 节点能成功访问目标服务，请将百炼应用服务 IP 地址（`47.93.216.17`、`39.105.109.77`、`60.205.180.248`和`59.110.152.173`）添加到您目标服务器的安全组（或防火墙）的入方向规则白名单中。
...
-   **参数配置**
    ...
    **参数说明**
    ...
    -   URL：在编辑框中输入完整的API地址。支持直接输入或插入变量引用之前节点的值。例如 `[https://dashscope.aliyuncs.com/compatible-mode/v1/files](https://dashscope.aliyuncs.com/compatible-mode/v1/files)`。
    ...
    例如，`[https://dashscope.aliyuncs.com/compatible-mode/v1/files?limit=5](https://dashscope.aliyuncs.com/compatible-mode/v1/files?limit=5)`中的`limit`。
    ...
    **说明**如需通过API集成应用到您的业务，请参阅[应用调用](https://help.aliyun.com/zh/model-studio/application-calling-guide)。
    
-   **节点示例**
    
    使用 GET 方法，调用接口。以[OpenAI兼容-File](raw/model-api-reference/toolkits-and-frameworks/openai-file-interface.md)中的HTTP代码为例进行配置，返回5个文件。
...
-   **定义**
    ...
    > 更多信息，请参见[插件概述](raw/application-user-guide/plug-in/plug-in-overview.md)。
    
-   **前提条件**
    -   如果要引用官方插件或三方插件，您需要先申请开通插件。具体操作，请参见[插件概述](raw/application-user-guide/plug-in/plug-in-overview.md)。
    -   如果要引用自定义插件，您需要先创建[自定义插件](raw/application-user-guide/plug-in/custom-plug-ins.md)。
-   **参数配置**
...
-   **定义**
    ...
    **说明**关于AppFlow的相关文档，请参见[产品简介](https://help.aliyun.com/zh/compute-nest/product-introduction)，关于AppFlow的具体应用，请参见[实践教程](https://help.aliyun.com/zh/compute-nest/practice-tutorial/)。
...
-   **参数配置**
    ...
    **参数说明**
    ...
    仅在 API 调用应用时生效，用于决定是否输出本节点内容。如需了解该组件的用途，请参阅[应用调用](https://help.aliyun.com/zh/model-studio/application-calling-guide)。
...
-   **模型选择** 模型选择
    ...
    **模型特点**
    ...
    模型列表会根据平台能力持续更新，实际可选模型以界面显示为准。模型调用价格请参考[模型调用计费](raw/model-user-guide/test-1/model-pricing.md)。
...
-   不同模型支持的配置参数存在显著差异，配置面板会根据选择的具体模型动态显示相应的配置项。
    ...
    **描述**
    ...
    合成语音的音色风格。支持的音色请参考[实时语音合成](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide)。
...
**说明**使用数据连接器节点前，需先在数据连接页面创建连接器并导入数据。创建方法参见[数据连接](raw/application-user-guide/data-connection-overview/data-connection.md)。
...
1.  然后，请通过**测试文本对话**，先在文本对话中调试出符合预期的应用效果。
    
    > 输入：Let's get some conversation.
    

1.  文本会话效果满意后，切换到**语音互动**或**视频互动**，再单击**去配置**配置API Key用于调用应用。

1.  进行**音视频设置**，配置参数后，再点击**拨打**调试音视频互动效果。
...
1.  效果测试满意后，点击右上角**体验**按钮，生成临时体验二维码，通过手机微信/钉钉/浏览器扫码体验交互效果。二维码有效期为24小时。

1.  效果确认后，点击**发布**按钮，发布应用。再进入**发布渠道**，完成智能媒体服务开通和SLR授权后，创建互动智能体。
...
> 注：您需用您的API KEY对`DASHSCOPE_API_KEY`进行替换才可发起调用。
...
**说明**API调用的超时时间为 600 秒，暂不支持修改。如可能超时，建议采用以下方案：
...
1.  **导入/导出百炼工作流**
    ...
    点击工作流页面上方的，选择**导出DSL**或**导入百炼DSL**。
    
2.  **导入 Dify 工作流**
    
    百炼支持一键导入 Dify 工作流，便于迁移和复用。
    
    1.  点击工作流页面上方的，选择**导入Dify DSL**。
...
1.  点击页面顶部的**版本管理**，您可以在**历史版本**面板按需查看或使用（点击**覆盖当前草稿**或**回到当前版本**）当前工作流应用的不同版本。
...
您可以在**应用管理**找到已发布的应用卡片，单击图标，进行**删除与复制工作流、修改应用名**操作。
