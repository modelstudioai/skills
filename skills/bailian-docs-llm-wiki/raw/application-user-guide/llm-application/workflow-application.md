...
6.  如果当前工作流应用可以正常工作，请单击右上角**发布**应用，以备后续[调用工作流应用](raw/application-user-guide/bailian-application-calling/invoke-workflow-application.md)。
...
8.  **测试应用：**
    1.  单击右上角**测试**，在对话框输入`给我介绍一下你们的冰箱`，单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2128773571/p984246.png)，查看输出结果。
    2.  继续输入`家用`，查看输出结果。
    3.  继续输入`200L`，查看输出结果。
    4.  最后输入`一级能效`，查看输出结果。
...
9.  如果当前工作流应用可以正常工作，请单击右上角**发布**应用，以备后续[调用工作流应用](raw/application-user-guide/bailian-application-calling/invoke-workflow-application.md)。
...
-   **如何使用**
    ...
    -   **结束节点**
        ...
        **JSON输出**：按照 JSON 格式编排输出内容，适合输出结构化的内容。可自定义变量名，支持输入文本或引用变量。
        ...
        仅在 API 调用应用时生效，用于决定是否输出节点内容。如需了解该组件的用途，请参阅[通过“结果返回”控制节点内容的方法](https://help.aliyun.com/zh/model-studio/invoke-workflow-application#b3be03a1ff21e)。
...
2.  然后，请通过**测试文本对话**，先在文本对话中调试出符合预期的应用效果。
