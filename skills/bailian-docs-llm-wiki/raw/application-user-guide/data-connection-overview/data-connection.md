...
3.  **解析方式**可选**默认设置**或**自定义设置**（**自定义设置**可针对不同格式配置解析规则，以提升解析效果）。
    ...
    -   **音视频解析：**对文件进行语音识别、视频帧提取（仅限视频）和剧情解析（仅限视频），最终将所有声画信息按时间轴结构化对齐。
        ...
        -   **语音识别：**字幕内容解析器通过[录音文件识别](https://help.aliyun.com/zh/model-studio/non-realtime-speech-recognition-user-guide)将人类语音转为文本。暂不支持识别音乐或自然环境声（如喇叭声、钟声、雷声等）。
...
在左侧**数据表管理**下，选择一个现有数据表，或点击图标新建数据表。
...
2.  填写数据源相关配置：
    ...
    **数据源名称**
    ...
    数据源名称在同一个[业务空间](raw/model-user-guide/security-and-compliance/permission-management-overview.md)中应是唯一的。即使数据源创建失败，该名称也无法再次使用。
...
[RAM用户](raw/model-user-guide/security-and-compliance/permission-management-overview.md)需主账号授予`管理员`权限，参见[页面权限](raw/model-user-guide/security-and-compliance/permission-management-overview.md)。
...
3.  为目标 OSS Bucket 添加`bailian-datahub-access`标签（值为`read`）。
    ...
    2.  悬停鼠标在其图标上，点击**编辑**（若未设置过标签）或**前往编辑**。
