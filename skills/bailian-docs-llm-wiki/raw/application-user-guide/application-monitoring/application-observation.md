...
[阿里云百炼应用](raw/application-user-guide/llm-application/application-introduction.md)内部的多[节点](https://help.aliyun.com/zh/model-studio/application-observation#fbcfa652edg9s)架构为后续开发带来诸多挑战，例如：
...
[智能体应用（Agent 1.0）](raw/application-user-guide/llm-application/single-agent-application.md)、[工作流应用](raw/application-user-guide/llm-application/workflow-application.md)和[高代码应用](raw/application-user-guide/llm-application/rich-code-application.md)。

> 应用观测目前暂不支持[通过Assistant API创建的智能体应用](https://help.aliyun.com/zh/model-studio/what-is-assistant-api)。
...
> 请使用[主账号](raw/model-user-guide/security-and-compliance/permission-management-overview.md)操作，开通后通常分钟级生效，但高峰期可能会稍有延迟。

> 如需使用[子账号](raw/model-user-guide/security-and-compliance/permission-management-overview.md)开通，需主账号为该子账号[配置必要权限](raw/application-user-guide/application-monitoring/application-observation.md)。

### 使用方法

#### 1\. 选择被观测的应用

访问[应用观测](https://bailian.console.aliyun.com/tab=app?tab=app#/app-observe)，单击**选择被观测的应用添加**。如果列表中没有您已创建的应用，可能是因为：

-   该应用尚未发布**。**
    
    > 您可以单击**管理应用**，在列表中找到您想要发布的应用后，单击**管理发布**。
...
1.  单击**操作**列的**查看详情**，查看过往（最长可选30天）所有在该应用中输入的Prompt的内容、**输出**、**延时**、**调用时间**以及**Token量**（说明参见[附录](https://help.aliyun.com/zh/model-studio/application-observation#e61c428fd89d1)）等概要信息。
    
    > **互动式体验**：列表中的**[CHAIN](https://help.aliyun.com/zh/model-studio/application-observation#fbcfa652edg9s)**节点表示一次完整的应用内部调用追踪，支持展开。查看[应用观测支持的所有节点类型](https://help.aliyun.com/zh/model-studio/application-observation#fbcfa652edg9s)。
...
1.  单击指定[节点](https://help.aliyun.com/zh/model-studio/application-observation#fbcfa652edg9s)的**名称**，即可查看**详情**、**原始数据**、**标注记录**等信息（如有下属节点，支持展开）。
...
当**Chain**作为根节点时，**名称**可能值为：**AgentApp**（[智能体应用（Agent 1.0）](raw/application-user-guide/llm-application/single-agent-application.md)）、**WorkflowApp**（[工作流应用](raw/application-user-guide/llm-application/workflow-application.md)）。
...
**Retriever**节点用于执行检索操作。**KnowledgeRetriever**表示在[知识库](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)中进行检索。
...
**Tool**节点表示对[插件](raw/application-user-guide/plug-in/plug-in-overview.md)的调用，支持官方插件和自定义插件，详见[插件概述](raw/application-user-guide/plug-in/plug-in-overview.md)。
...
表示[开始节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。
...
表示[大模型节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。
...
表示[API节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**CLASSIFIER**

表示[意图分类节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**TEXT\_CONVERTER**

表示[文本转换节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**SCRIPT**

表示[脚本转换节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**CONDITION**

表示[条件判断节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**FUNCTION\_COMPUTE**

表示[函数计算节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。

**APP\_FLOW**

表示[AppFlow节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。
...
表示[结束节点](https://help.aliyun.com/zh/model-studio/workflow-application#186ed63a97rvo)。
...
**FullCodeApp**指触发了一个已创建的[高代码应用](raw/application-user-guide/llm-application/rich-code-application.md)，目前不支持追踪其内部调用链路。

## 常见问题

**使用子账号开通应用观测，应如何配置权限？**

1.  为子账号配置`AliyunBailianFullAccess`[全局管理（阿里云百炼）权限](https://help.aliyun.com/zh/model-studio/member-management)。
    
2.  为子账号配置`应用观测-操作`（或`管理员`）[页面权限](https://help.aliyun.com/zh/model-studio/member-management)，使其可在应用观测页面执行写入类操作。
    
3.  创建并授予子账号**创建服务关联角色**系统策略。
    
    1.  登录[RAM控制台](https://ram.console.aliyun.com/)，在左侧导航栏，选择**权限管理权限策略**，然后单击界面上的**创建权限策略**。
...
4.  在左侧导航栏，选择**身份管理用户**。从界面列表中找到待授权的子账号，然后单击子账号**操作**列的**添加权限**。
5.  从**权限策略**列表中，选择刚创建的权限策略（CreateServiceLinkedRole），然后单击**确认新增授权**。至此，子账号拥有了创建服务关联角色的权限。
6.  完成以上所有权限配置后，返回[应用观测](https://bailian.console.aliyun.com/tab=app?tab=app#/app-observe)界面，使用子账号再尝试开启**应用观测**。
