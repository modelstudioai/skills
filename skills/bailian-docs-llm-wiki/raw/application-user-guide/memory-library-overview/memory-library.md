...
**重要**记忆库将于 **2026 年 8 月 20 日 10:00**（北京时间）正式开始商业化计费。
...
以下示例使用默认记忆库，3 步快速体验记忆的写入、查看和检索。如需自定义记忆规则，请参见[创建记忆库](https://help.aliyun.com/zh/model-studio/memory-library#heading-create)。

### 步骤一：写入记忆

调用 [AddMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md) 接口，将对话传入记忆库。调用前需配置 `DASHSCOPE_API_KEY`，获取方式请参考[获取 API Key](raw/model-api-reference/preparations/get-api-key.md)。
...
**说明**需要安装 agentscope-runtime，安装命令：`pip install agentscope-runtime`
...
如需在应用中集成检索能力，请参见 [SearchMemory API](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)。
...
2.  在对话框中配置以下参数：
    ...
    -   **记忆抽取策略版本**：选择 **Pro**（推荐）或 **Lite**。Pro 版使用更高质量的记忆抽取模型，¥0.03/次；Lite 版使用标准记忆抽取模型，¥0.018/次。不传时默认 Pro。详见[计费说明](https://help.aliyun.com/zh/model-studio/memory-library#h3-pro-lite)。
...
2.  在对话框中配置以下参数：
    ...
    -   **记忆抽取策略版本**：选择 **Pro**（推荐）或 **Lite**。Pro 版使用更高质量的记忆抽取模型，¥0.03/次；Lite 版使用标准记忆抽取模型，¥0.025/次。不传时默认 Pro。详见[计费说明](https://help.aliyun.com/zh/model-studio/memory-library#h3-pro-lite)。
...
3.  单击**确认**完成配置。详细参数说明请参见[配置用户画像规则](https://help.aliyun.com/zh/model-studio/memory-library#heading-profile-rules)。

配置后，调用 [AddMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md) 时传入画像模板 ID，系统会自动从对话中提取属性。提取结果可在**记忆详情**标签页中查看。
...
相关 API：[CreateProfileSchema](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)、[AddMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)、[GetUserProfile](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)。
...
1.  在记忆库详情页的**记忆检索**标签页中，配置以下参数：
    ...
    -   **记忆实体 ID：**输入[写入记忆](https://help.aliyun.com/zh/model-studio/memory-library#29baf0c8b5y5g)时设置的`user_id`字段值。
...
通过 API 对记忆片段进行列出、更新和删除操作。建议使用元数据（`meta_data`）对记忆进行分类管理，便于后续的精确检索和管理。相关 API：[ListMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)、[UpdateMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)、[DeleteMemory](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)。
...
[AddMemory - 添加记忆](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)

添加记忆。传入对话消息后，系统自动提取记忆片段；传入画像模板 ID 时同时提取用户画像。也支持直接指定要存储的记忆内容。

[SearchMemory - 搜索记忆](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)

检索记忆。基于语义检索返回与当前查询最相关的历史记忆。

[ListMemory - 列出记忆](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)

列出记忆。分页查看用户的所有记忆。

[DeleteMemory - 删除记忆](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)

删除记忆。支持删除指定的记忆条目。

[UpdateMemory - 更新记忆](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)

更新记忆。修改已有记忆的内容。

更多调用详情请参阅[长期记忆 API](raw/application-api-reference/long-term-memory-new/long-term-memory-api-reference.md)。
...
**说明**记忆写入（Add 调用） 的 4 个规格为：观察记忆 Pro、观察记忆 Lite、画像记忆 Pro、画像记忆 Lite。记忆检索（Search 调用） 的 2 个规格为：Pro、Lite。
...
4 个规格，每个规格 250 次（合计 1,000 次）

3 个月内有效，逾期未用作废

记忆检索（Search 调用）

2 个规格，每个规格 2,500 次（合计 5,000 次）
...
单价已含查询向量化与结果重排（Rerank）费用。Pro = 开启 Rerank，Lite = 关闭 Rerank。
...
记忆库的 Add 和 Search 调用均区分 **Pro** 和 **Lite** 两个策略版本，但两类调用的差异机制不同：Add 的版本对应不同质量的记忆抽取模型，Search 的版本决定是否开启 **Rerank**（结果重排序模型）。
...
-   **Add 调用**的版本由关联记忆规则（记忆片段规则 project 或用户画像规则 profile schema）的 `plan_version` 决定。创建规则时选择 Pro 或 Lite，不传时默认 Pro。Add 调用遵循其关联规则的版本设置。
...
4.  **记忆片段和用户画像有什么区别？何时使用哪个？**
    ...
    记忆片段适用于记录具体的事件和信息（如"用户上周去了北京"），用户画像适用于结构化的用户属性（如年龄、职业、偏好等）。如果需要记录固定的用户属性，建议使用**用户画像**；如果是动态的事件信息，使用记忆片段。两者可以同时使用。
...
9.  **Pro 和 Lite 策略版本有什么区别？如何选择？**
    ...
    Add 调用中，Pro 版使用更高质量的记忆抽取模型，Lite 版使用标准记忆抽取模型；Search 调用中，Pro 版开启 Rerank（结果重排序），Lite 版关闭 Rerank。对记忆抽取或检索准确性要求高的场景建议使用 Pro，高频调用且成本敏感的场景可使用 Lite。Add 调用的策略版本由记忆规则配置决定，Search 调用的策略版本由请求参数 `plan_version` 独立控制。详见[计费说明](https://help.aliyun.com/zh/model-studio/memory-library#h3-pro-lite)。
...
12.  **长期记忆功能支持在 APP 端使用吗？**
     ...
     支持。记忆库提供开放的 API 接口，记忆的写入与检索全部通过 API 完成，与调用方的客户端类型无关。Web 应用、移动 APP、小程序均可通过 AddMemory、SearchMemory 接口集成记忆能力。若需将智能体应用发布到移动端，多平台发布与集成方式请参见[应用分享](raw/application-user-guide/application-publishing-and-sharing/share-an-application.md)。
