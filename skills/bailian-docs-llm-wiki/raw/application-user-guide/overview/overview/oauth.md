...
除此之外，每个配置都有共有的 **配置名称** 字段，见[创建身份验证配置](raw/application-user-guide/overview/overview/create-config.md)。
...
**重要**只用一个 API Key 就能连的 App 见[API Key 配置](raw/application-user-guide/overview/overview/api-key.md)，不需要去对方系统注册应用。
