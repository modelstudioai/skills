...
体验页当前限制为单文件不超过 200 MB。该限制不等同于 API 上限；API 能接受的文件大小以正式接口文档和服务返回为准。完整格式列表见[支持的文件与限制](raw/application-user-guide/overview/configurations/supported-files-and-limits.md)。
...
Parse 和 Extract 分项记录用量。可在控制台查看用量信息；具体计量口径、价格、额度和结算规则以[计量与计费](raw/application-user-guide/overview/configurations/pricing.md)页面为准。
