...
凭证的具体获取路径见[OAuth 2.0 配置](raw/application-user-guide/overview/auth-guide/oauth.md)与[API Key 配置](raw/application-user-guide/overview/auth-guide/api-key.md)。
...
3.  **填写配置**：向导第二步是 **身份验证配置**。顶部三项是所有 App 共有的：
    ...
    下方是该 App 专属的凭证字段，各 App 不同，详见[OAuth 2.0 配置](raw/application-user-guide/overview/auth-guide/oauth.md)与[API Key 配置](raw/application-user-guide/overview/auth-guide/api-key.md)。
...
连接建好后在 App 详情页的 **已连接的用户** 区域可以看到，管理方式见[连接的账户](raw/application-user-guide/overview/auth-guide/connected-accounts.md)。
...
**重要**第一次接触这套流程，用[语雀](raw/application-user-guide/overview/apps-guide/yuque.md)练手最快，它只需要一个 API Key，不用去对方系统注册应用。
