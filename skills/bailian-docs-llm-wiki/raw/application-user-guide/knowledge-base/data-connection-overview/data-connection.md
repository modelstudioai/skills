...
数据集按数据的存储和访问方式，分为**平台托管**和**流处理**两大类、三种类型：
...
上传和管理非结构化文档（PDF、Word、Markdown等），支持本地上传与飞书、钉钉、SharePoint、语雀、OSS 等外部来源持续同步
...
连接 MySQL、PostgreSQL、PolarDB-X 2.0 数据库并执行 SQL 查询（仅DMS导入数据源方式支持执行）

**说明**语雀、OSS、钉钉、飞书、SharePoint 不作为独立数据集类型创建，仅作为文件数据集的导入与同步来源，详见下方[同步数据规则](#daede64826nqe)。

## 前置条件

在创建数据集前，请确保满足以下条件：

-   **账号权限**：主账号或具有数据集管理权限的 RAM 用户。RAM 用户需要主账号授权后才能使用数据集功能。授权方法请参见[权限管理](raw/application-user-guide/application-permission-management/application-permission-management-overview.md)。
    
-   **数据源准备：**
    -   **文件/表格数据集：**已准备好要上传的文档或表格文件，或已创建OSS Bucket。
    -   **MySQL数据库：**已有MySQL数据库实例（阿里云RDS或自建），并确保网络可达（公网或私网）。
    -   **PostgreSQL数据库：**已有PostgreSQL数据库实例，且已将`wal_level`参数设置为`logical`。
    -   **PolarDB-X 2.0数据库：**已有阿里云 PolarDB-X 2.0 实例，且实例所在地域支持私网访问。如需通过 DMS 导入数据源，请先在 DMS 中完成 PolarDB-X 实例的录入。
    -   **语雀同步来源：**已有语雀知识库（仅支持公网版本语雀），并获取了个人访问 Token。
    -   **OSS 同步来源：**已创建OSS Bucket，并开通了[向量检索服务](https://help.aliyun.com/zh/oss/user-guide/vector-retrieval/)。

## 创建数据集

1.  访问[数据集](https://bailian.console.aliyun.com/cn-beijing/rag/connector/list)页面，单击右上角的**创建**。
    
2.  填写**数据集名称**（不超过 20 个字符）和**描述**（不超过 200 个字符，描述会用于指导智能体调用的准确度，建议写明数据内容和用途）。
    
3.  选择**数据集类型**（文件、表格或数据库），并按类型填写配置：
    
    #### 文件
    
    文件数据集用于管理非结构化文档（PDF、Word等），支持本地上传与飞书、钉钉、SharePoint、语雀、OSS 等外部来源持续同步。
    ...
    MySQL 数据库数据集属于**流处理**类型,用于连接MySQL数据库,使应用可以执行SQL查询获取实时数据。
    
    **说明**仅通过**从DMS导入数据源**方式创建的MySQL数据集支持执行SQL查询。通过**创建自定义数据源**方式添加的MySQL数据集不支持直接执行SQL。
    
    1.  数据集类型选择**数据库**，数据库类型选择**MySQL**。
    ...
    PostgreSQL 数据库数据集支持连接阿里云RDS PostgreSQL实例或自建PostgreSQL数据库。
    
    **说明**仅通过**从DMS导入数据源**方式创建的PostgreSQL数据集支持执行SQL查询。通过**创建自定义数据源**方式添加的PostgreSQL数据集不支持直接执行SQL。
    ...
    **前置条件**
    ...
    1.  数据集类型选择**数据库**，数据库类型选择**PostgreSQL**。
    ...
    2.  **配置数据库连接：**
        ...
        -   **用户名**：具有高权限的数据库账号。
        -   **密码**：数据库密码。
    3.  单击测试连通性，确保配置正确。PostgreSQL 数据集使用DTS（数据传输服务）进行连通性检测。
    ...
    PolarDB-X 2.0 数据库数据集属于**流处理**类型,用于连接阿里云 PolarDB-X 2.0 分布式数据库,使应用可以执行SQL查询获取实时数据。
    
    **说明**仅通过**从DMS导入数据源**方式创建的PolarDB-X 2.0数据集支持执行SQL查询。通过**创建自定义数据源**方式添加的PolarDB-X 2.0数据集不支持直接执行SQL。
    
    1.  数据集类型选择**数据库**，数据库类型选择**PolarDB-X 2.0**。
    ...
    3.  **选择网络类型**:仅支持**私网**。需要选择**所属地域**,通过内网连接 PolarDB-X 实例,适用于生产环境。
    ...
    5.  （可选）单击**开始检测**,验证数据库连通性。检测通过后,可以从**选择DB**下拉列表中选择要连接的数据库。
...
2.  填写数据源相关配置：
    ...
    **数据库用户名**
    ...
    建议使用**高权限账号**。也可使用普通账号，但需为其授予目标数据库的 DML 和 DDL 权限。关于账号说明和获取方式请参见[创建账号](https://help.aliyun.com/zh/rds/apsaradb-rds-for-mysql/create-an-account-on-an-apsaradb-rds-for-mysql-instance#section-b3f-whz-q2b)。
...
在[数据集列表页](https://bailian.console.aliyun.com/cn-beijing/rag/connector/list)，单击目标数据集的**详情**按钮，进入数据集详情页。详情页包含以下标签页：

-   **概览**:显示数据集的基本信息（名称、描述、类型、创建时间、存储配额等）和自动生成的工具列表。
-   **文件/表格**（仅平台托管类型）：管理数据集中的文件或表格数据。
-   **工具：**查看数据集自动生成的工具详情,包括参数说明和在线测试。

您可以在数据集详情页展开工具,填写参数后单击**运行**按钮，在线测试工具的返回结果。

### 同步数据规则

在[文件数据集](https://bailian.console.aliyun.com/cn-beijing/rag/connector/list)的**文件**页签下，可点击右上角的**同步数据规则**\->**创建同步规则**进入页面创建规则。
...
2.  选择数据**同步来源**：
    ...
    **飞书应用ID**
    ...
    飞书自建应用的 App ID。[查看方式](https://open.feishu.cn/app)。
    
    **飞书应用密码**
    
    是
    
    飞书自建应用的 App Secret。在飞书开放平台应用的**凭证与基础信息**页面获取。
    
    **飞书知识库ID**
    
    三选一
    
    飞书知识库 ID、飞书目录 ID 和飞书文档 ID，必须填写一个。可通过飞书知识库 URL 获取。
    
    **飞书目录ID**
    
    三选一
    
    飞书知识库 ID、飞书目录 ID 和飞书文档 ID，必须填写一个。可通过飞书目录 URL 获取。
    
    **飞书文档ID**
    
    三选一
    
    飞书知识库 ID、飞书目录 ID 和飞书文档 ID，必须填写一个。可通过飞书文档 URL 获取。
    ...
    **飞书文档域名**
    ...
    用户文档域名为 xxx.feishu.cn/wiki/... 时填写 xxx，用于上传时回填文档源地址。
    
    **同步周期**
    
    是
    
    选择数据自动同步的频率，支持**一分钟**、**一小时**（默认）和**一天**。
    
    **连接检测**
    
    是
    
    单击**连接检测**验证飞书应用凭证和知识库访问权限是否正常。检测通过后方可提交规则。
    ...
    **重要**SharePoint 同步功能目前仅对白名单用户开放。如需使用，请联系阿里云百炼团队申请开通。
    ...
    2.  在左侧导航栏的**管理**下，单击**应用注册**，然后单击**新注册**，填写应用名称并选择受支持的账户类型，完成创建。如需了解详细步骤，请参见[Microsoft 官方文档：注册应用程序](https://learn.microsoft.com/zh-cn/entra/identity-platform/quickstart-register-app)。
        
    3.  在应用**概述**页面的**概要**区域，复制**目录(租户)ID**，即表单中的**租户ID**。
        
    4.  在同一**概要**区域，复制**应用程序(客户端)ID**，即表单中的**客户端ID**。
        
    5.  在左侧导航栏的**管理**下，单击**证书和密码**，切换到**客户端密码**页签，单击**新客户端密码**，填写描述并设置有效期。创建后立即复制**值**（仅显示一次，离开页面后无法再查看），即表单中的**应用机密值**。
    ...
    **租户ID**
    ...
    Azure AD 应用的目录（租户）ID。在 Azure 门户的应用注册**概述**页面**概要**区域获取。
    
    **客户端ID**
    
    是
    
    Azure AD 应用的应用程序（客户端）ID。在 Azure 门户的应用注册**概述**页面**概要**区域获取。
    
    **应用机密值**
    
    是
    
    Azure AD 应用的客户端密码值。在 Azure 门户的**证书和密码**页面，切换到**客户端密码**页签后创建获取。注意：密码值仅在创建时显示一次，离开页面后无法再查看，请及时复制保存。
    
    **SharePoint 文档或文件夹分享链接列表**
    
    是
    
    输入需要同步的 SharePoint 文档或文件夹的分享链接。多个链接之间用换行分隔。在 SharePoint 中通过**共享**功能获取分享链接。
    
    **云类型**
    
    否
    
    选择 SharePoint 所在的云环境。支持**全球公有云**（默认）和**中国世纪互联**。如果 SharePoint 账户部署在中国世纪互联云上，请选择对应选项。
    ...
    **连接检测**
    ...
    单击**连接检测**验证 SharePoint 凭证和分享链接是否可正常访问。检测通过后方可提交规则。
    ...
    **语雀Token**
    ...
    语雀访问 Token，支持个人 Token 或团队 Token。获取方式见上方说明。
    
    **文档权限等级**
    
    否
    
    需要同步的语雀文档权限等级，支持多选。`0`：私密，`1`：公开，`2`：企业内公开。例如同步私密和企业内公开文档：`0,2`。默认 `0,1,2`（当 URL 列表指定具体文档时，该配置对该文档不生效）。
    ...
    **连接检测**
    ...
    单击**连接检测**验证语雀 Token 和 URL 是否可正常访问。检测通过后方可提交规则。
...
-   **编辑**：点击卡片进入数据集详情页,单击右上角的**编辑**按钮，可以修改数据集的**名称**和**描述**。数据集类型和存储方式创建后不可更改。
    
-   **复制**：在数据集列表页或详情页，单击**复制**按钮，可以基于当前数据集的配置快速创建一个新的数据集。
    
-   **删除**：在数据集列表页，单击目标数据集卡片上的**更多**图标（`···`），选择**删除**；或在数据集详情页单击**删除**按钮。
    
    **重要**删除前需确认，**删除后无法恢复**。
    

**说明**知识库创建后，不支持修改关联的数据集，已有知识库编辑页的**数据来源**为只读字段。如需使用新的数据集，需重新创建知识库。
