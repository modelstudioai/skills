...
**重要**2026 年 1 月 4 日前创建但未开通服务的知识库数据将保留至 2026 年 6 月 30 日。逾期未开通服务，数据将被永久删除。请及时开通[知识库服务](https://bailian.console.aliyun.com/?tab=app#/knowledge-base)。

## 1\. 计费方式

知识库提供**后付费**（按量付费）和[预付费资源包](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#a06c023507qq3)两种计费方式。开通知识库服务后，默认采用后付费计费方式。资源包可前往控制台[RAG标准版资源包](https://common-buy.aliyun.com/?commodityCode=sfm_ragservicestandard_dp_cn)或[RAG旗舰版资源包](https://common-buy.aliyun.com/?commodityCode=sfm_ragserviceenterprise_dp_cn)开通**。**
...
-   **扣费模式**
    -   **免费额度：**自知识库开通时，开始计算[免费额度](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#3f33e1a3b18cg)（标准版知识库）的有效期。
    -   **资源包：**资源包按业务量级划分为不同规格，可一次性付费购买，不同规格及价格可查看[资源包](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#a06c023507qq3)。
    -   **后付费：**系统会根据其[规格](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#e7811e39b38al)，按运行时长累计从阿里云账户自动扣费，**按小时出账**。请确保账户余额充足（可通过[费用与成本](https://usercenter2.aliyun.com/home)页面进行充值）。
-   **停止计费：**如果不再需要某个知识库，请及时[删除](https://help.aliyun.com/zh/model-studio/rag-knowledge-base#362726005am7x)以停止计费。
    
    **重要**删除操作会**永久清除**知识库内的数据**且无法恢复**，请**谨慎操作**。
...
平台存储 ≤ **9,999** GB （如需更大存储空间，可在创建知识库时选择并配置自购的 ADB-PG 实例。参见[操作指南-创建知识库](https://help.aliyun.com/zh/model-studio/rag-knowledge-base#362726005am7x)）
...
-   **变配：**按变配发生时间[分段计费](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#d90304901atdb)。变配操作本身不收费。同一知识库 **1**个自然日内最多允许变配**1**次。
...
**重要****老用户**的免费额度有效期统一截至 2026 年 2 月 3 日 23:59，到期后将自动按量付费。
...
-   **适用范围：** 仅用于抵扣**标准版知识库**的[规格费用](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#c8e0de82bfjdy)，不适用于旗舰版。
...
**重要**模型调用费用是独立计费项，根据实际使用的**输入 Token 量**计算。其价格和免费额度策略遵循模型广场中对应模型的[调用计费标准](raw/model-user-guide/test-1/model-pricing.md)，不包含在知识库的规格费用中。
...
-   **检索流程与计费关系详解**
    ...
    2.  **重排序**
        ...
        2.  **费用 = 初步召回总切片数 × 平均切片Token数 × 模型单价（**参见[模型调用计费](raw/model-user-guide/test-1/model-pricing.md)**）**
            
            **重要**排序模型费用取决于**初步召回的总切片数**，而非最终返回召回的切片数量。
...
-   **问答生成模型：**系统根据您在应用中选择的问答模型（如 qwen-plus 等）生成回答，按该模型的 Token 用量计费。具体价格以[模型计费标准](raw/model-user-guide/test-1/model-pricing.md)为准。
...
-   **知识库路由（可选）：**若应用关联了多个知识库并开启了路由功能，系统会调用 [qwen-plus](https://bailian.console.aliyun.com/cn-beijing?tab=model#/model-market/detail/qwen-plus-latest) 进行路由判断（详见 [2.2.2 知识检索](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#c868ef3a653qx)）。

**重要**知识问答服务的完整费用 = **规格费用**（知识库运行时长）+ **检索阶段的模型费用**（向量化 + 排序 + 路由）+ **问答阶段的模型费用**（问答生成 + 预文件解析）。各模型费用按实际 Token 消耗量独立计算，请关注[模型计费标准](raw/model-user-guide/test-1/model-pricing.md)了解各模型的单价。
...
相关文档：[重排（Rerank）配置](https://help.aliyun.com/zh/model-studio/rag-knowledge-base#7ef3a9f29b095)。
...
**说明**点击知识库卡片上的**命中测试**，进入配置调试页面进行测试，会产生相应的模型（向量模型、排序模型）调用计费。

#### 2.2.5 节省计划抵扣说明

知识库使用的向量模型（如 text-embedding-v4）和排序模型（如 qwen3-rerank）属于百炼平台 A 类模型，其调用费用支持通过以下节省计划抵扣：

-   **AI 通用型节省计划**（推荐）：覆盖 A 类全部模型（含文本向量、多模态向量、排序模型），按月承诺消费享阶梯折扣。详情请参见[节省计划与资源包](raw/model-user-guide/test-1/savings-plan-and-resource-package.md)。
-   **向量及排序模型节省计划**：专门针对向量和排序模型的节省计划，一次性购买固定金额。详情请参见[节省计划与资源包 > 向量及排序模型节省计划](raw/model-user-guide/test-1/savings-plan-and-resource-package.md)。

**说明**节省计划仅可抵扣模型调用费用，不可抵扣知识库的规格费用（运行时长费用）。规格费用的优化请参见[资源包](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#a06c023507qq3)。
...
> 图中实例 ID 即[知识库ID](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)。
...
-   **向量存储使用自购 ADB-PG：**
    ...
    -   **≥8 天：**欠费后第 **8** 天，视为主动放弃 ADB-PG 后付费服务。ADB-PG 将会**清理知识库相关实例并永久删除其数据，无法恢复**。详见[ADB-PG欠费说明](https://help.aliyun.com/zh/analyticdb/analyticdb-for-postgresql/product-overview/expiration-or-overdue-payments)。
        ...
        **说明**使用自购 ADB-PG 时，数据保留期遵循 ADB-PG 的欠费策略，为 **7**天（非 **14**天）。
...
1.  **子账号可以开通知识库或查看账单吗？**
    ...
    可以。经授权的子账号（系统策略AliyunBailianFullAccess 或 AliyunSFMFullAccess）可开通知识库，费用归属于主账号。相关权限配置请参见[权限管理](raw/model-user-guide/security-and-compliance/permission-management-overview.md)。
...
4.  **配置变更跨小时如何计费？**
    ...
    按变更发生时间分段计费，同一小时内按各时间段占比累加。示例请参见[配置变更（分段计费）](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#d90304901atdb)。
    
5.  **为什么我的排序（Rerank）费用特别高？如何降低模型调用费用？**
    
    排序（Rerank）模型的费用与您最终返回的结果数量无关，而是由**初步召回**的文本切片总数决定的。降低模型调用费用详见本文[2.2.4 费用优化建议](https://help.aliyun.com/zh/model-studio/billing-for-knowledge-base#9a9e30ecc3pbe)内容。
...
6.  **如何彻底停止知识库的计费？删除库内文件可以吗？**
    ...
    **重要**删除操作会**永久清除**知识库内的数据**且无法恢复**，请**谨慎操作**。删除知识库实例后，关联该知识库的知识检索和知识问答服务将自动失效，不再产生检索调用费用。
