...
通过 API 调用知识库时，检索行为由请求体中 `rag_options` 字段的参数决定。请检查以下 API 参数是否与控制台的调试设置保持一致：

**API 参数（`rag_options` 字段）**
...
若 API 调用时未显式传入上述参数，系统将使用默认值，该默认值可能与控制台当前配置不同，从而导致检索结果存在差异。建议在 API 请求中明确指定这些参数，确保与控制台调试设置一致。
