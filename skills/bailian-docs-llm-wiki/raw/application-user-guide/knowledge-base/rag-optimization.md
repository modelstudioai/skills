...
1.  **典型问题：**知识库中含多个“类别”的文件。希望在A类文件中检索时，召回结果中却包含其他类别（比如B类）的文本切片。
    ...
    **解决方案：**为文件添加标签，知识库会在向量检索前根据标签筛选相关文件。
    ...
    -   **在上传文件时设置标签**：控制台操作步骤请参见[导入数据](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)，相关API是[AddFile](raw/application-api-reference/application-component-api-reference/api-bailian-2023-12-29-dir/api-bailian-2023-12-29-dir-data-connection-original-application-data/api-bailian-2023-12-29-addfile.md)。
    -   **在数据管理页面编辑标签**：对于已上传的文件，可单击右侧的**标签**进行编辑，相关API是[UpdateFileTag](raw/application-api-reference/application-component-api-reference/api-bailian-2023-12-29-dir/api-bailian-2023-12-29-dir-data-connection-original-application-data/api-bailian-2023-12-29-updatefiletag.md)。
    
    目前阿里云百炼支持以下方式使用标签：
    
    -   [通过API调用阿里云百炼应用](https://help.aliyun.com/zh/model-studio/application-calling-guide)时，可以在请求参数`tags`中指定标签。
        
    -   在调试应用时设置标签（本方式仅适用于[智能体应用（Agent 1.0）](raw/application-user-guide/llm-application/single-agent-application.md)）。
...
2.  **典型问题：**知识库中含多个结构内容相同/相近的文件，如文件A和文件B的内容都包含“功能概述”章节，只希望在A文件的“功能概述”中检索。
    ...
    **某知识库中有大量手机产品说明文件，文件名称为手机的型号（比如阿里云百炼X1、阿里云百炼Zephyr Z9等），且所有文件都包含「功能概述」章节。**
    ...
    接下来，我们将手机名称，按照知识库一文[Meta信息抽取](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)中提到的步骤，设置为元数据（相当于让每篇文件相关的文本切片带上各自对应的手机名称信息），然后再进行一次相同的测试作为对比。
    ...
    除此以外，元数据的一个常见应用是在文本切片中嵌入日期信息，以便过滤最近的内容。更多元数据用法，请参见[metadata抽取](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)。
...
2.  **人工检查和修正文本切片内容：**确保文件被正确地解析和切分。
    ...
    **说明**此处只是修改了知识库中的文本切片，并未修改数据管理（临时存储）中的源文件或数据表。因此后续将此文件/数据表再次导入知识库时，仍需人工检查和修正。
...
1.  **提高“相似度阈值”：**放宽召回条件，避免因过滤标准过于严格而导致漏召回。
    ...
    **命中测试建议步骤**
    ...
    4.  基于召回结果，重新调整您的知识库的相似度阈值，具体操作请参见[编辑知识库](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)。
...
2.  **提高“召回片段数”：**对于需要总结、列举或比较的复杂问题，适当增加该值有助于大模型生成更完整的答案。
    ...
    ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3421457871/CAEQURiBgMCatMinohkiIDY3YTVhOWY2MjNjMjRkYzc5NTU1ZmVhNGQ2MGQ5ODc24762899_20250109142407.621.svg)
...
2.  **典型问题：**返回的结果没有按照要求，或者不够全面。
    ...
    **默认联系人**：如果找不到相关的在职人员，请直接返回“今日值班人：阿里云百炼客服01”。
    ...
    了解更多提示词优化方法，请参阅[文生文Prompt指南](raw/model-user-guide/use-cases/prompt-engineering-guide.md)。
...
4.  **典型问题：**提示词相同，希望每次返回的结果相同或不同。
    ...
    **enable\_thinking**：是否开启思考模式。
    ...
    通过 API 调用应用时，若不传 `temperature`、`top_p` 等生成参数，则默认使用控制台**应用参数配置**中的设置；若 API 显式传入这些参数，则以 API 参数为准，覆盖控制台配置。此规则与 `enable_thinking` 参数一致。
    

### 3.6 API 与控制台检索结果不一致

**典型问题：**通过百炼 API 调用知识库时，检索返回的结果与控制台调试结果不一致（例如，API 仅返回固定数量的召回片段，或召回内容与控制台存在明显差异）。

**解决方案：**对比 API 请求参数与控制台调试设置，确保两侧参数配置一致；同时借助**日志监控**，通过 requestId 追踪 API 请求的实际执行情况。

说明：排查 API 与控制台检索差异

**第一步：对比 API 参数与控制台设置**

通过知识库检索 API（Retrieve 接口）调用知识库时，检索行为由请求体中的检索参数决定。请检查以下 API 参数是否与控制台的调试设置保持一致：

**API 参数（Retrieve 接口请求体）**
...
Rank 模型重排后的最低相似度分数，低于该值的切片将被过滤
...
若 API 调用时未显式传入上述参数，`DenseSimilarityTopK` 默认值为 100、`RerankTopN` 默认值为 5，可能与控制台当前配置不同，从而导致检索结果存在差异；`RerankMinScore` 未传入时默认采用知识库配置的相似度阈值。建议在 API 请求中明确指定这些参数，确保与控制台调试设置一致。

**第二步：通过 requestId 在日志监控中核查实际请求**

开启**日志监控**功能后，可在日志详情中通过 `requestId` 查看 API 请求的完整 `request_body` 和 `response_body`，确认实际使用的参数值和召回的具体切片内容，再与控制台调试结果进行对比，从而定位差异根源。

**第三步：检查其他影响召回结果的参数**

除核心检索参数外，以下参数在 API 侧的默认行为可能与控制台不同，请一并核查：

-   **多轮对话改写**（`EnableRewrite`）：若控制台调试时已开启**多轮对话改写**功能，而 API 请求中未传入 `EnableRewrite: true`，则 API 侧不会对查询进行改写，在多轮对话场景下可能导致召回结果存在差异。
-   **标签过滤**（`SearchFilters`）：若控制台调试时设置了标签过滤条件，API 调用时请通过 `SearchFilters` 参数传入相同的标签条件，否则 API 侧召回范围与控制台不同。
...
最后，如果上述方法均已尝试且希望进一步提升效果，可考虑针对具体场景，选择一个大模型进行[模型调优](raw/model-user-guide/fine-tuning/fine-tune-text-generation-model/model-training-overview.md)。
