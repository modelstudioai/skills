...
详细限制见[支持的文件与限制](raw/application-user-guide/getting-started-overview/settings-configurations/supported-files-and-limits.md)，配置规则见[保存与复用配置](raw/application-user-guide/getting-started-overview/settings-configurations.md)。
...
完整参数和响应字段见[文档解析 API](/zh/model-studio/parse/parse-submit)。
```
...
`config_id` 与内联 `processing` 是二选一的处理定义；使用已保存配置时不要同时传递内联参数。字段定义和两种请求示例见[信息抽取 API](/zh/model-studio/parse/extract-submit)。
```
...
-   [API 概览](raw/application-api-reference/api-overview.md)：查看认证、Base URL 占位约定、公开业务路径和异步调用流程。
-   [错误码](raw/application-api-reference/api-overview/errors.md)：根据 HTTP 状态和错误码决定处理方式。
-   [任务与用量](raw/application-user-guide/getting-started-overview/settings-configurations/usage.md)：了解公开 REST 任务状态，以及控制台“任务记录”和用量界面。
