...
1.  **创建应用：**前往[应用管理](https://bailian.console.aliyun.com/?tab=app#/app-center)，创建一个新的智能体应用。智能体的配置如下：
    ...
    **说明**MCP 是一种协议框架，允许您将外部工具（如本例中的 Amap Maps 和 QuickChart）接入到智能体应用中，作为大模型可调用的工具。请前往[MCP 简介](raw/application-user-guide/model-context-protocol/mcp-introduction.md)了解更多信息。
...
2.  **发布组件：**在发布应用时选择**发布应用组件**。 在弹出的发布成功对话框中，单击**发布应用组件**右侧的**立即发布**链接。 也可以随时在**发布渠道**里创建组件。 在**组件**区域，单击**\+ 创建**。
...
-   imageList：用于传递用户输入的图像公网地址列表。仅在组件使用[图像与视频理解](raw/model-user-guide/model-experience/vision-model/vision.md)模型时有效。

**说明**预设的系统参数无法删除。如果您不需要某个预设参数，请在配置参数时隐藏该参数（将**是否可见**设置为否）。

#### 填写组件名称和描述

-   **组件名称：** 描述组件功能的名称。（填写示例：天气分析智能体）
    
-   **组件描述：** 组件的功能简介。（填写示例：用于分析城市的天气信息）
