...
**说明**对于RTOS或某些旧版Linux系统，建立安全Websocket 通信所依赖的 TLS 隧道，需要做如下配置：
...
语音识别支持的模型包括：[Paraformer实时语音识别](raw/model-api-reference/audio-api-references/speech-recognition-api-reference/paraformer-real-time-speech-recognition-api-reference.md)（Paraformer），[FUN-ASR实时语音识别](raw/model-api-reference/audio-api-references/speech-recognition-api-reference/fun-asr-real-time-speech-recognition-api-reference.md)（FunASR），[Qwen-Audio-3.0-ASR-Flash-Streaming](raw/model-api-reference/audio-api-references/speech-recognition-api-reference/fun-asr-real-time-speech-recognition-api-reference/fun-asr-realtime-java-sdk.md)，[千问3-ASR-Flash-Realtime](https://help.aliyun.com/zh/model-studio/real-time-speech-recognition-user-guide)（qwen3-asr-flash-realtime），多模态交互轻量版语音识别（AppSpecificASR-Realtime）。

语音合成支持的模型包括：[语音合成CosyVoice-v2大模型](raw/model-api-reference/audio-api-references/speech-synthesis-api-reference/cosyvoice-large-model-for-speech-synthesis.md)（cosyvoice-v2），[语音合成CosyVoice-v3-Flash大模型](https://help.aliyun.com/zh/model-studio/text-to-speech)（cosyvoice-v3-flash），[语音合成CosyVoice-v3-plus大模型](https://help.aliyun.com/zh/model-studio/text-to-speech)（cosyvoice-v3-plus），[语音合成CosyVoice-v3.5-Flash大模型](https://help.aliyun.com/zh/model-studio/text-to-speech)（cosyvoice-v3.5-flash），[语音合成CosyVoice-v3.5-Plus大模型](https://help.aliyun.com/zh/model-studio/text-to-speech)（cosyvoice-v3.5-plus），[Qwen-Audio-3.0-TTS-Plus](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide)（qwen-audio-3.0-tts-plus）、[Qwen-Audio-3.0-TTS-Flash](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide)（qwen-audio-3.0-tts-flash），[千问3-TTS-Flash-Realtime](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide)（qwen3-tts），[千问3-TTS-Instruct-Flash-Realtime](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide)（qwen3-tts-instruct），[千问3-声音设计](https://help.aliyun.com/zh/model-studio/qwen-tts-voice-design)（qwen3-tts-vd），[千问3-声音复刻](https://help.aliyun.com/zh/model-studio/qwen-tts-voice-cloning)（qwen3-tts-vc），[Sambert语音合成](raw/model-api-reference/audio-api-references/speech-synthesis-api-reference/sambert-speech-synthesis.md)（sambert），多模态交互轻量版语音合成（AppSpecificTTS）。

语音合成支持的音色，可以在控制台上选择了模型后，点击右侧语音交互体验区域的右上角查看音色列表。

官方音色也可以参考官方文档：[支持的音色](https://help.aliyun.com/zh/model-studio/realtime-tts-user-guide#bac280ddf5a1u)，sambert支持的音色参考[Sambert音色列表](raw/model-api-reference/audio-api-references/speech-synthesis-api-reference/sambert-speech-synthesis/sambert-java-sdk.md)（去掉开头的"sambert-"和末尾的"-v1"后就是voice的取值）。

使用复刻音色时，确认复刻音色状态为"OK"后才能使用。查询方法参考[查询特定音色](https://help.aliyun.com/zh/model-studio/cosyvoice-clone-design-api)。
...
上传的语音识别音频需满足：16bit（采样位深）、单声道、有符号、little-endian PCM编码，采样率参考[Start](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5b2363d40585y)消息的参数parameters.upstream.sample\_rate的取值说明。

如果希望减少网络流量和带宽占用，用户可以把PCM音频编码为Opus格式，同时设置上传音频格式为raw-opus。

上传音频时，根据[Start](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5b2363d40585y)消息中`upstream.mode`设置不同，采取的措施也不同：
...
-   mode为 `push2talk`：客户端无需持续上传音频，但需通过[SendSpeech](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#cec23c1556u8x)和[StopSpeech](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#d11933e7b5jz0)通知服务端音频识别的开始和结束。发送[SendSpeech](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#cec23c1556u8x)后需立即上传音频，否则会增加处理时间。
...
-   下发音频为16bit单声道，采样率和编码由[Start](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5b2363d40585y)消息参数定义。
-   下发速度取决于TTS服务性能，通常快于播放速度。
-   音频下发前发送[RespondingStarted](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#0693e9d4a5f91)事件；结束后发送[RespondingEnded](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#402cbee340r1m)事件。
-   客户端需在播放完成后上报[LocalRespondingEnded](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#f4b1a6bf22ymq)，通知服务端播放结束。
...
文本消息标记交互流程的关键节点，控制流程并传输关键信息。通过[时序图](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#6d25d6f8ddbtd)可了解不同消息的交互时序。

文本消息包含两部分：`header`和`payload`。

-   `payload`：内容随[消息类型](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#408ddb268eyt7)变化。
...
参数说明参考下方 [parameters.upstream的参数说明](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/multimodal-interaction-protocol.md)表格
...
参数说明参考下方 [parameters.downstream的参数说明](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/multimodal-interaction-protocol.md)表格
...
参数说明参考下方 [parameters.client\_info的参数说明](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/multimodal-interaction-protocol.md)表格
...
参数说明参考下方 [parameters.biz\_params的参数说明](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/multimodal-interaction-protocol.md)表格
...
三种模式的对比可以参考下方的[客户端使用的三种模式对比](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5e82453407pum)表格。
...
[RequestToSpeak](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#96541eff55v86)消息打断
...
**说明**SDK接收到**Started**之后，先不可以向服务发送音频，应等待**DialogStateChanged**消息，确认切换到Listening状态之后才发送音频。
...
-   调用部分agent时，text可以是""空字符串，服务端只需要使用parameters中的images或者biz\_params参数即可处理。具体参考[调用官方Agent](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/official-agent.md)。
...
与[Start](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5b2363d40585y)消息中`parameters.biz_params`相同，传递对话系统自定义参数。RequestToRespond的`biz_params`参数只在本次请求中生效。

**说明**除了videos参数外，`parameters.biz_params`与[Start](https://help.aliyun.com/zh/model-studio/multimodal-interaction-protocol#5b2363d40585y)消息中的`parameters.biz_params`相同，传递对话系统自定义参数。RequestToRespond的`biz_params`参数只在本次请求中生效。
