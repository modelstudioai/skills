...
支持欧美、亚洲热门国家的单一语言对话能力，语种、功能持续扩展中。详情请参考[多语言对话](raw/application-user-guide/application-gallery/multimodal-products/multimodal-guidelines/multi-language-dialogue.md)。

如需多语言自由对话能力，选择默认的中文即可。
...
在应用概览页面顶部单击**系统设置**，在下拉菜单中选择**热词库**进入热词配置页面。

配置应用时，可以在界面中下拉选择一个热词组并添加，发布后相应热词即可生效。

如需针对不同用户设置不同ASR热词，可通过API接口传入。通过接口传入热词后，控制台热词不再生效。接口文档：[管理热词](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/management-hot-words.md)

您还可以使用即时纠错功能，通过接口上传词表，对语音识别结果进行实时干预。接口文档：[ASR结果即时纠错](https://help.aliyun.com/zh/model-studio/multimodal-sdk-java#630d27822bdo8)

#### 语音合成

支持使用阿里云百炼的语音合成模型，包括CosyVoice-实时语音合成、 Qwen-Audio-TTS、千问3-实时语音合成以及其他语音合成。除系统音色外，还支持声音复刻能力[CosyVoice声音复刻/设计API](https://help.aliyun.com/zh/model-studio/cosyvoice-clone-design-api)（CosyVoice-v2大模型、CosyVoice-v3-Flash大模型、CosyVoice-v3-Plus大模型、CosyVoice-v3.5-Plus大模型、CosyVoice-v3.5-Flash大模型）、[Qwen-TTS声音复刻API参考](https://help.aliyun.com/zh/model-studio/qwen-tts-voice-cloning)（千问3-TTS-声音复刻）以及[声音设计（Qwen）](https://help.aliyun.com/zh/model-studio/qwen-tts-voice-design)能力（通义千问3-TTS-声音设计）。
...
关闭后仅支持闲聊对话。不再触发调用工具、Agent、联网搜索、对话承接语等基于语义理解的交互功能。
...
开启**文本模型**开关后，单击模型名称区域可打开**选择模型**对话框。对话框左侧为**模型系列**列表（如 Qwen3.6-Plus、DeepSeek 等），选中后右侧展示主干模型详情，下方可选择对应的**快照版本**（如 Qwen3.6-Plus-2026-04-02）。

若关闭文本模型开关，仅输出意图识别结果，不会调用大模型生成回复。
...
**欢迎语**输入框最多支持输入 50 个字符，例如`你好，我是小云，今天心情怎么样呀？`。

语音交互应用尚不支持通过控制台设置欢迎语。

### 提示词

支持自定义Prompt，用于设定对话风格和人设。您可以在右侧体验区测试提示词效果。

如未填写，则使用通用Prompt，适用于常见的口语对话场景。

支持使用阿里云百炼的Prompt[模板概述](raw/application-user-guide/prompt/prompt-template.md)和Prompt[自动优化](raw/application-user-guide/prompt/optimize-prompt.md)功能。

提示词编辑区支持使用 `##` 标题分段组织内容（如 **##角色**、**##系统条件**、**##风格**），变量使用 `${变量名}` 格式插入（如 `${location}`、`${date}`），字数上限为 8000 字。

支持插入自定义变量，可以设置变量并传参。传入变量值将替换提示词中对应的变量位置。

在**自定义变量**弹窗中，可通过入参变量表单填写或 `biz_params` 字段传递变量值。每个变量需配置**变量名**、**描述**和**默认值**三个字段，单击**\+ 添加自定义变量**可新增变量行。
...
开启**知识库**开关后，提示词编辑区域中会出现蓝色的 **documents** 按钮标签，表示知识库文档已关联至当前应用。

在提示词编辑区中，知识库对应的引用变量为 `${documents}`，输入"/"即可插入该变量。

### 长期记忆

开启长期记忆开关，大模型可以基于对话历史，分析挖掘个人信息和偏好、近期生活状况和行为计划，形成长期记忆库，并在后续对话中主动召回，为用户提供更个性化的智能服务。

如需根据业务场景自定义记忆策略、管理记忆结果，请查看接口文档：[长期记忆开放接口](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/long-term-memory-api.md)。

**说明**不同应用之间记忆内容隔离；同一个应用中，按照userid维度生成记忆库，不同userid之间记忆隔离。

### 联网搜索

由于训练数据的时效性，大模型无法准确回答如股票价格、今日资讯等时效性问题。

您可以通过开启联网搜索开关，启用联网检索功能，使大模型可以基于实时检索数据进行回复；支持选择四个版本，包括基础版、高速版、高级版、均衡版；同时您还可以设置搜索策略，默认搜索策略为自主判断，可手动切换为强制联网。

高速版、高级版、均衡版联网搜索均需额外付费，请您根据业务情况按需选择。
...
详细使用方法可查看：[对话日志接入](raw/application-user-guide/application-gallery/multimodal-products/multimodal-guidelines/multimodal-chatlog.md)。

## 技能

### 指令

指令是指由多模态交互下发，设备终端执行的任务，如打开台灯、调高音量等。具体指令可查看：[指令列表](raw/application-user-guide/application-gallery/multimodal-products/multimodal-guidelines/instruction-list.md)。
...
-   支持接入百炼官方MCP服务。
    ...
    在应用编辑页面左侧技能配置区域下方，单击**+MCP**按钮，打开**选择MCP服务**面板，在**MCP广场**中浏览并选择需要添加的 MCP 服务，然后单击**确认**完成添加。
...
-   当前仅支持接入部分百炼官方MCP服务，列表如下：
    ...
    **详情**
    ...
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi021863)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi021863](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi021863)
    
    图书信息查询
    
    通过10位或者13位ISBN查询图书信息，包含书名、作者、出版社、价格、出版日期、印次、装帧方式、语种、摘要等几十项图书相关信息。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00053669)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00053669](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00053669)
    
    黄历运势查询
    
    可查询传统日历、节假日、运势、宜忌等信息，广泛用于日程安排，出行指南，风水评估等。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00066017)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00066017](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi00066017)
    
    新电途-ChargeStation
    
    朗新集团新电途官方提供的充电站查询服务，用户可通过此服务按页查询指定文本地址或经纬度坐标周边范围内的充电站详细信息。服务支持灵活的距离范围设置，让用户能够精确获取所需区域内的充电设施数据。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/ChargeStation)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/ChargeStation](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/ChargeStation)
    
    火车票查询
    
    提供国内火车的车次查询，余票查询和站站查询三个子服务。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011240)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011240](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011240)
    
    星座运势查询
    
    本数据包含十二星座的每日运势，明日运势，每周运势，当年运势以及星座配对。具体包括缘份星座、吉利颜色、幸运数字、爱情提醒、运势简评、爱情运势、工作运势、财富运势、健康运势等。可用于微信公众号、小程序、网站等引流及增加与用户的联系等场景。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011529)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011529](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011529)
    
    八字排盘
    
    八字排盘是中国传统命理学工具，根据出生年、月、日、时干支共八字，分析五行生克关系，推算命运、性格与运势，用于趋吉避凶，解读人生轨迹。切勿迷信，仅供参考。
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011212)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011212](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/market-cmapi011212)
    
    12306车票查询
    
    开源社区开发者封装,提供 12306购票信息查询等服务
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/china-railway)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/china-railway](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/china-railway)
    
    今天吃什么
    
    开源社区开发者封装，让 AI 助手变身私人大厨，为你的一日三餐出谋划策！
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/how-to-cook)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/how-to-cook](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/how-to-cook)
    
    谣言鉴别
    
    快速鉴别新闻事件信息真伪
    
    [](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/rumor-identify)[https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/rumor-identify](https://bailian.console.aliyun.com/?tab=mcp#/mcp-market/detail/rumor-identify)
...
在应用编排页面的**技能**配置步骤中，单击底部**MCP服务**区域打开**选择MCP服务**面板，然后单击**自定义MCP**页签查看或接入自定义 MCP 服务。

单击**创建MCP服务**，可选择以下创建方式：
...
多模态交互应用提供一系列适用于不同场景的Agent，其中视觉理解Agent支持开启高分辨率模式，API参考[调用官方Agent](raw/application-user-guide/application-gallery/multimodal-products/multimodal-api-references/official-agent.md)。
...
-   视频通话：实时视觉理解，适用于带有摄像头的设备。点开右侧设置，可配置功能名称、视觉理解模型、欢迎语、退出语、启动指令、退出指令以及提示词。
    ...
    -   视觉理解模型：包含「推荐模型」及「更多模型」，其中「推荐模型」支持视觉理解高级版以及均衡版，「更多模型」支持Qwen3.6系列、Qwen3.5系列等多模态模型。
        ...
        -   单击 Agent（如**视频通话**）右侧的设置图标，打开**配置应用**面板。除视觉理解模型外，还可配置欢迎语、退出语、启动指令、退出指令及提示词等参数，并支持一键导入理解与生成模块的提示词和变量。
...
-   拍照问答：当识别到用户有画面理解的需求时，自动调用摄像头拍照并回复。适用于带有摄像头的设备。点开右侧设置，可配置启动指令、提示词，还能选择视觉理解模型。
    ...
    -   视觉理解模型：包含「推荐模型」及「更多模型」，其中「推荐模型」支持视觉理解高级版以及均衡版，「更多模型」支持Qwen3.6系列、Qwen3.5系列等多模态模型。
        ...
        -   在**拍照问答**Agent 的**配置应用**面板中，可设置以下内容：
...
-   极速视频通话：基于qwen3.5-omni 系列 realtime 模型的视频对话，支持语音和图片输入，可选语音输出或文本输出。点开右侧设置，可配置功能名称、启动指令以及退出指令。
    ...
    -   在配置面板中，**模型选择**选择**全模态大模型高级版**，还可配置**提示词**（支持自定义变量）。
-   拍照翻译：多模态应用专属，支持多语言、自动拍照的翻译能力，能够精准翻译眼前的文字。点开右侧设置，可配置触发指令、提示词以及对话变量，支持一键导入主对话链路中的提示词和变量配置。
    
    -   在**提示词**编辑区中，可使用 `##` 标题分段编写内容，例如 `##人物设定` 定义角色身份（如"你是小云，擅长处理文字识别和翻译任务的专家"），`##风格` 定义回复风格（如"亲切友好且口语化"）。
...
-   多模态备忘录：用户可以通过语音输入的方式，让模型创建备忘、记录关键信息，支持自动拍照并结合视觉信息提取备忘。备忘信息支持通过语音指令查询、修改、删除。
    ...
    -   在 Agent 配置区域单击**添加应用**，在弹出的对话框中勾选需要的预置 Agent（如**多模态备忘录**），单击**确定**后，所选应用将显示在左侧 Agent 列表中。
-   地图出行：提供复杂路线规划、周边地点查询和推荐能力。例如，“怎么去首都机场最快？”、“推荐附近评分最高的咖啡厅”。（本Agent仅支持上述场景的多轮对话，不支持实时语音导航。）
    
    -   在 Agent 配置页的**百炼应用**区域，单击**\+ 添加**，弹出**添加应用**面板。面板提供**推荐应用**和**我的应用**两个页签，勾选需要接入的 Agent 后单击**确定**完成添加。
-   录音纪要：提供实时录音字幕和总结，提炼核心信息、生成会议纪要。例如，“开始实时转写”、“开始录制并总结”。支持对录音总结的内容进行多轮对话问答，例如询问上次会议的讨论内容、昨天课程的课程要点等。还支持对历史内容进行问答，可提问的内容范围包括录音原文，以及大模型总结的纪要文本。
    ...
    **说明**对历史内容进行问答的功能对上线后创建的录音任务可用，对之前的历史任务不支持问答。
    
    -   在应用配置区选择**Agent** Tab，找到**百炼应用**区块，打开**添加应用**弹窗，在**推荐应用**页签中勾选需要接入的 Agent 应用。
...
-   主动陪伴：摄像头常开，会持续分析眼前景象（也会产生费用），支持功能如下：
    ...
    -   单击**主动陪伴**，在右侧**配置应用**面板中可设置**功能名称**、**启动指令**、**退出指令**及**欢迎语**，并勾选需要启用的支持能力。
-   拍照问答：当识别到用户有画面理解的需求时，自动调用摄像头拍照并回复。适用于带有摄像头的设备。点开右侧设置，可配置启动指令、提示词，还能选择视觉理解模型。
    
    -   视觉理解模型：包含「推荐模型」及「更多模型」，其中「推荐模型」支持视觉理解高级版以及均衡版，「更多模型」支持Qwen3.6系列、Qwen3.5系列等多模态模型。
        
        -   单击**拍照问答**进入配置面板，可设置以下参数：
...
-   极速视频通话：基于qwen3.5-omni 系列 realtime 模型的视频对话，支持语音和图片输入，可选语音输出或文本输出。点开右侧设置，可配置功能名称、启动指令以及退出指令。
    ...
    -   在配置面板中，还可设置**模型选择**（选择**全模态大模型高级版**）和**提示词**（支持自定义变量）。
-   拍照翻译：多模态应用专属，支持多语言、自动拍照的翻译能力，能够精准翻译眼前的文字。点开右侧设置，可配置触发指令、提示词以及对话变量，支持一键导入主对话链路中的提示词和变量配置。
    
    -   在**提示词**区域编写提示词，例如使用`##人物设定`定义角色（如"你是小云，擅长处理文字识别和翻译任务的专家"），使用`##风格`定义回复风格（如"请用亲切友好且口语化的方式帮助用户解答问题，适时加入一些语气词，让对话更加自然、贴近生活"）。
-   儿童故事：陪伴式讲故事功能，拥有自有故事库，支持讲故事过程中的对话交互、故事创作和续写改写。点开右侧设置，可配置功能名称、启动指令、退出指令、语音合成模型以及对应音色。
...
-   多模态备忘录：用户可以通过语音输入的方式，让模型创建备忘、记录关键信息，支持自动拍照并结合视觉信息提取备忘。备忘信息支持通过语音指令查询、修改、删除。
    ...
    -   典型场景：设定日程提醒如“明天下午两点提醒我开会”，设定闹钟如“五分钟后提醒我”，记忆视觉信息如“记一下我的停车位号码”、“记一下这本书的名字”。
    -   在 Agent 配置页面中，单击**添加应用**，在弹出的对话框中勾选需要接入的百炼应用（如**多模态备忘录**），单击**确定**后所选应用即出现在左侧 Agent 列表中。
-   地图出行：提供复杂路线规划、周边地点查询和推荐能力。例如，“怎么去首都机场最快？”、“推荐附近评分最高的咖啡厅”。（本Agent仅支持上述场景的多轮对话，不支持实时语音导航。）
    
    -   在应用编辑页第 4 步**Agent**配置中，单击**百炼应用**区域的**添加**按钮，弹出**添加应用**面板。面板顶部提供**推荐应用**和**我的应用**两个页签，以卡片网格展示可选 Agent；勾选需要接入的 Agent 后，单击**确定**完成批量添加。
-   录音纪要：提供实时录音字幕和总结，提炼核心信息、生成会议纪要。例如，“开始实时转写”、“开始录制并总结”。支持对录音总结的内容进行多轮对话问答，例如询问上次会议的讨论内容、昨天课程的课程要点等。还支持对历史内容进行问答，可提问的内容范围包括录音原文，以及大模型总结的纪要文本。
    
    **说明**对历史内容进行问答的功能对上线后创建的录音任务可用，对之前的历史任务不支持问答。
    
    -   在应用配置页面左侧单击**Agent**页签，找到**百炼应用**区块，单击进入**添加应用**弹窗。在**推荐应用**页签中，以卡片网格展示可选 Agent，勾选所需应用后单击**确定**。
...
-   主动导览：摄像头常开，会持续分析眼前景象，遇到需要讲解的展品，会主动进行介绍。也可以主动要求AI对眼前的展品做介绍或进行问答。
    ...
    -   单击**Agent**区域中**百炼应用**下的**主动导览**，在右侧弹出的**配置应用**面板中填写**功能名称**、**启动指令**、**触发之后的欢迎语**和**退出指令**，然后单击**确定**。
-   主动陪伴：摄像头常开，会持续分析眼前景象（也会产生费用），支持功能如下：
    
    -   手势互动：对着摄像头比动作，AI可快速进行动作、表情、语音回复的反馈。如果同时比动作和说话，优先响应说话内容。
    -   打招呼：结合视觉信息，与用户互动（类似进入语），如对用户的着装进行点评、针对表情回复相应的夸赞或安抚、对用户手持物品表达兴趣、随机唠嗑等。
    -   主动发起话题：结合用户情境（比如绘画/玩魔方、家中打扫卫生等）主动参与活动，发起话题。
    -   单击**主动陪伴**应用，打开**配置应用**面板。在面板中设置**启动指令**、**退出指令**和**欢迎语**，并在**支持能力**区域勾选所需能力（手势互动、打招呼、主动发起话题），然后单击**确定**。

#### 接入百炼平台Agent

-   可以接入您在阿里云百炼平台创建的Agent，以增强对话能力。
    
    -   在技能配置的 Agent 区域，单击右上角**\+ 添加**按钮，在弹出的**添加应用**对话框中，通过**推荐应用**或**我的应用**页签选择需要接入的百炼应用，选择完成后单击**确定**。
-   也可以前往百炼应用广场的[AI硬件方案专区](https://bailian.console.aliyun.com/cn-beijing/?tab=app#/app-market/app-template-recommendation/share/AIHardwareSolutions)快速复制与您业务场景需求相似的模板，如穿戴设备的身体指标分析、教育场景的口语陪练等。详情请参考：[百炼应用推荐模板](raw/application-user-guide/application-gallery/multimodal-products/multimodal-guidelines/agent-template.md)。
...
-   接入百炼Agent时，您可以在配置页面开启「视觉理解」开关，当识别到对应意图，客户端可以上传图片给agent，实现多模态理解能力。
    ...
    -   单击目标百炼应用右侧的配置按钮，在弹出的**配置应用**面板中将**输出模式**设为 `agent_format`，并开启底部的**视频理解**开关。

#### 接入三方Agent

-   支持客户开发的三方Agent接入，需基于Google A2A协议进行集成，详情请参考[三方Agent接入](raw/application-user-guide/application-gallery/multimodal-products/multimodal-guidelines/multimodal-integration-a2a.md)。
    
    -   在应用概览页面右上角，单击**系统设置**，在下拉菜单中选择**三方应用**进入三方 Agent 配置页面。
    -   在**Agent**配置页面底部找到**三方应用**区域，单击**\+ 添加**按钮，弹出三方应用选择面板。在面板中搜索并勾选需要接入的三方应用（如**超级AI助理**），完成添加。
-   Agent内部可以配置和语音助手不同的音色，但需要保持使用同一个模型。例如，语音助手选择通用的温柔女声，接入的童话故事Agent选择一个童声。当启动童话故事Agent时，系统自动切换为童声。
    
    -   单击 Agent 右侧的音频配置图标，打开**音色配置**面板。关闭**应用播报音色与主链路保持一致**开关后，可分别选择**语音合成模型**（如 CosyVoice-v3-Flash 大模型）和**音色**（如龙安欢），单击**确定**完成配置。
-   接入三方Agent时，您可以在配置页面开启「视觉理解」开关，当识别到对应意图，客户端可以上传图片给agent，实现多模态理解能力。
    
    -   在**技能**页签的**三方应用**列表中，单击目标应用的配置图标，打开**配置应用**面板，找到**视频理解**开关并开启。
