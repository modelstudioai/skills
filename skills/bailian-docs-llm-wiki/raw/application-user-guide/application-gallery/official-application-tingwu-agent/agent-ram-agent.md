...
2.  选择需要管理阿里云百炼服务的RAM用户，并定义用户名称，单击**确定，即完成添加用户。**在**新增用户**对话框中，选择**类型**（**RAM用户**或**RAM角色**），从下拉框中选择目标 RAM 用户（若无可单击**创建RAM用户**链接前往创建），填写**显示名称**、**登录名称**和**备注**，然后单击**确定**。
3.  对新增的用户需要进行权限配置，找到目标用户，点击其右侧的**权限管理**，进行授权操作。在用户列表中，找到目标用户（如 new\_account），单击其操作列的**权限管理**。
