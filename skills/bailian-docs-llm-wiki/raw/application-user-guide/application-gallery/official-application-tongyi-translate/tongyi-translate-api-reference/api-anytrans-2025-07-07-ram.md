...
本文为您介绍大模型服务平台百炼（AnyTrans）为RAM权限策略定义的操作（Action）、资源（Resource）和条件（Condition）。大模型服务平台百炼（AnyTrans）的RAM代码（RamCode）为 anytrans，支持的授权粒度为操作级。
...
下表是大模型服务平台百炼（AnyTrans）定义的操作，这些操作可以在RAM权限策略语句的`Action`元素中使用，用来授予执行该操作的权限。下面对表中的具体项提供说明：
...
大模型服务平台百炼（AnyTrans）不支持在RAM权限策略语句的`Resource`中指定资源ARN。如果要允许对大模型服务平台百炼（AnyTrans）的访问权限，请在策略语句中指定`"Resource": "*"`。

## 条件（Condition）

大模型服务平台百炼（AnyTrans）未定义产品级别的条件关键字。如需查看适用于所有云产品的通用条件关键字，请参见[通用条件关键字](https://help.aliyun.com/zh/ram/policy-elements)。
