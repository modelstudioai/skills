...
> 如果您希望将知识库部署在云端，直观、快速地创建RAG应用，请使用阿里云百炼提供的[0代码构建RAG应用](raw/application-user-guide/start-using/build-knowledge-base-qa-assistant-without-coding.md)功能。
...
下载[local\_rag.zip](https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/zh-CN/20250414/odwvrb/local_rag.zip)并解压，您可以看到如下图所示的目录结构：
...
请获取[阿里云百炼API Key](https://bailian.console.aliyun.com/?apiKey=1#/api-key)，并参考[配置API Key到环境变量](raw/model-api-reference/preparations/get-api-key.md)将API Key配置到环境变量中。
...
如果您想要直接在对话框中上传文件，并基于该文件进行问答，可以在**RAG问答**页面单击输入框旁的进行**临时知识库文件**的上传，便能直接输入问题并获得回复。该方法在页面刷新后无法找回上传的文件。
