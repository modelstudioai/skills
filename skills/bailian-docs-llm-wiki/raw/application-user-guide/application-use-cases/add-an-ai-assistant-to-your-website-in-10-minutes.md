...
> 阿里云百炼提供的[新用户免费额度](raw/model-user-guide/test-1/new-free-quota.md)可以覆盖本教程的模型调用消耗，额度消耗完后按 token 计费，相比自行部署大模型可以显著降低初期投入成本。本教程中使用的 AppFlow、函数计算 FC 及可选的 ADB-PG 向量存储由相应云产品单独计费，具体请参见各产品的计费文档。
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3601275771/p1066021.png) 3. 在页面右侧可以提问验证模型效果。不过目前它还无法准确回答您公司的商品信息。点击右上角的**发布**，我们将在后面的步骤中去解决这一问题。
...
**说明**如果首次使用阿里云函数计算服务，需要按照页面引导，授权“AliyunFCServerlessDevsRole”角色所需的权限。
...
### 嵌入 AI 助手后出现 request error 怎么办？

网站嵌入 AI 助手后，如果对话出现 request error，按以下步骤排查：

1.  **检查 API Key 是否可用**：在 `AppFlow` 控制台 AI 助手详情页，点击已导入模型旁的编辑图标，在**选择连接凭证**步骤中查看 API Key，前往百炼控制台[密钥管理](https://bailian.console.aliyun.com/?tab=app#/api-key)页面确认状态。
2.  **确认智能体与 API Key 在同一业务空间**：在**参数配置**步骤中检查**业务空间标识（`AgentKey`）**。智能体与 API Key 分属不同业务空间会导致鉴权失败。
3.  **核实应用 Id 是否正确**：在**参数配置**步骤中确认**应用 Id**字段值，前往百炼[应用管理](https://bailian.console.aliyun.com/?tab=app#/app-center)页面对比核实。

以上三项均正确配置后，AI 助手可正常对话。
...
### AI 助手报错"Failed to stream content"

**问题现象**

在网站中嵌入 AI 助手后，用户与 AI 助手对话时出现"Failed to stream content"错误。

**可能原因**

该错误通常是由于前端代码中的请求地址配置不正确导致的。例如，在定制前端组件时，直接复制了示例代码中的默认请求地址（如`/chat`），但未将其替换为实际的后端服务地址。

**解决方案**

1.  检查前端代码中的请求地址配置，确认是否已将默认地址替换为您实际的后端服务地址。
2.  如果您使用了 AppFlow 的悬浮挂件部署脚本，请确保脚本代码来自您自己的 AI 助手 Web 集成页面，而不是其他来源的示例代码。
3.  确认 AI 助手的模型配置正确，包括 API Key 和应用 ID 是否填写正确。
4.  使用浏览器开发者工具（F12）检查控制台和网络请求，查看具体的错误信息和请求地址，排查请求是否发送到了正确的后端服务。
