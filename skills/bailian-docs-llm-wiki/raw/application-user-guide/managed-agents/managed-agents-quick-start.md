...
也可通过 API 完成，指定名称、模型和工具（详见[创建 Agent API](raw/application-api-reference/managed-agents-api/agent-api/agent-create.md)）：
...
也可通过 API 完成，绑定智能体与环境（详见[创建 Session API](raw/application-api-reference/managed-agents-api/session-api/session-create.md)）：
...
向会话写入消息触发智能体处理（详见[发送 Event API](raw/application-api-reference/managed-agents-api/session-api/event-post.md)）：
...
通过 SSE 事件流实时接收处理过程与输出（详见[SSE 事件流 API](raw/application-api-reference/managed-agents-api/session-api/event-sse-stream.md)）：
...
-   [配置运行环境](raw/application-user-guide/managed-agents/managed-agents-environment/managed-agents-cloud-hosting.md)：沙箱类型、预装包与网络策略
-   [API 总览与认证](raw/application-api-reference/managed-agents-api/managed-agents-api-overview.md)：在业务代码中集成 Managed Agents
