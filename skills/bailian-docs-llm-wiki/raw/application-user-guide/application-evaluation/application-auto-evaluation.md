...
1.  自动评测仅面向已发布的[智能体应用（Agent 1.0）](raw/application-user-guide/llm-application/single-agent-application.md)。应用发布请参考[应用分享](raw/application-user-guide/application-publishing-and-sharing/share-an-application.md)。
2.  自动评测将基于知识库自动生成评测集，请确保应用已配置知识库。详情可参考[创建和使用知识库](raw/application-user-guide/knowledge-base/rag-knowledge-base.md)。
3.  智能体应用评测依赖于智能体推理的过程数据，请确保已开通`应用观测`功能，并将需要评测的应用添加到观测列表中。详情可参考[用量监控与性能分析](raw/application-user-guide/application-monitoring/application-observation.md)。
4.  子账号（RAM用户）需获取`管理员`或`应用评测-操作`权限，才能够使用自动评测功能。详情可参考[页面权限](raw/model-user-guide/security-and-compliance/permission-management-overview.md)。
...
3.  选择用于生成评测任务的模型。为确保生成质量，目前仅支持使用`qwen-max`和`qwen-plus`模型。各模型能力请参考[选择模型](raw/model-user-guide/get-started-with-models/models.md)，计费规则请参考[模型调用计费](raw/model-user-guide/test-1/model-pricing.md)。
...
2.  选择评测模型。为确保结果准确，目前仅支持使用`qwen-max`和`qwen-plus`模型，模型能力和计费规则请参考[选择模型](raw/model-user-guide/get-started-with-models/models.md)和[模型调用计费](raw/model-user-guide/test-1/model-pricing.md)。
...
2.  **分析归因，定位问题**：根据系统给出的归因类型，参考[归因分析](https://help.aliyun.com/zh/model-studio/application-auto-evaluation#c686b552abp3n)中的说明，快速定位每个 BadCase 的产生原因。
