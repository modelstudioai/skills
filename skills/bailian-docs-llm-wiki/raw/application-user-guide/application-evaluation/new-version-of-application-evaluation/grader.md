...
> 在[评估器](https://bailian.console.aliyun.com/cn-beijing/?tab=app#/efm/app_evaluate/tabs?activeKey=grader)页面左上角单击**返回旧版**，可返回[旧版应用评测](raw/application-user-guide/application-evaluation/application-auto-evaluation.md)。
...
**重要****评测集字段规划**：不同预置评估器对评测集的字段有不同要求。建议在构建评测集前，先确认要使用的评估器所需的必选参数，提前在评测集中包含对应字段。
