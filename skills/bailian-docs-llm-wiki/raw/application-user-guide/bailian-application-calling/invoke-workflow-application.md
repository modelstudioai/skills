...
> 如需使用 Responses API 调用，请参阅 [Responses API](raw/application-api-reference/application-call/openai-responses-api.md)。
...
1.  已[获取 API Key](raw/model-api-reference/preparations/get-api-key.md)并[配置API Key到环境变量](https://help.aliyun.com/zh/model-studio/configure-api-key-through-environment-variables)。
...
响应中的 `output.thoughts[]` 按执行顺序返回各节点的运行信息。数组每个元素的 `response` 字段是一段 JSON 字符串，包含以下字段：
...
节点输出结果，JSON 字符串。开始节点等无输出节点不返回该字段。

nodeExecTime

string

节点执行耗时，如 `1167ms`。节点执行中时不返回该字段。
...
节点执行中，`nodeResult` 为空。流式输出时，节点先返回 `executing`，执行完成后再返回最终状态。

### 深度思考

当工作流应用使用了[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)时，模型会在生成最终回复前进行一步或多步的推理，以提升处理复杂问题（如逻辑推理、代码生成、数学计算等）的准确性。
...
如果您想使用 Responses API 调用应用，请参见[Responses API](raw/application-api-reference/application-call/openai-responses-api.md)。
