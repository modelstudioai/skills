...
- **[模型部署](../concepts/model-deployment.md)控制**：管理模型是否可直接部署。
...
- 单个 API Key 只能归属**一个地域**内的**一个[业务空间](../concepts/workspace.md)**和**一个用户**，不可转移。
- API Key 的可调用功能和限流与归属[业务空间](../concepts/workspace.md)一致，**不受用户控制台权限管理影响**。
...
| 账号移出[业务空间](../concepts/workspace.md) | — | 失效（重新加入后恢复） |
| RAM 控制台删除账号 | — | 失效，不可恢复 |
...
RAM 用户默认**无权**调用百炼应用的数据、知识库、[Prompt 工程](../concepts/prompt-engineering.md)及长期记忆等功能的 OpenAPI。需由阿里云主账号在 RAM 控制台授予以下权限之一：
...
1. 非默认[业务空间](../concepts/workspace.md)需由超级管理员开通特定模型的调用权限。
2. **控制台调用**：为 RAM 用户添加「模型体验-操作」、「批量推理-操作」、「模型观测-操作」页面权限。
3. **API 调用**：在对应[业务空间](../concepts/workspace.md)为 RAM 用户创建或分配 API Key。

### 设置模型调优权限

需为 RAM 用户添加以下页面权限：模型体验-操作、模型调优-操作、我的模型-操作、[模型部署](../concepts/model-deployment.md)-操作、模型评测-操作、数据管理-操作、模型观测-操作。
...
将主账号总配额按比例分配给各[业务空间](../concepts/workspace.md)，建议预留 10% 作为缓冲。例如总配额 1000 QPM：生产 600 QPM（60%）、测试 200 QPM（20%）、开发 100 QPM（10%）、缓冲 100 QPM（10%）。
