...
- **限制模型部署**：管理某个模型可否在该业务空间直接部署。默认业务空间所有支持部署的模型均可部署。
...
2. 在「权限管理」页签内为 RAM 用户添加以下权限：**模型体验-操作**、**模型调优-操作**、**我的模型-操作**（管理调优后模型快照）、**模型部署-操作**（部署调优后的模型）、**模型评测-操作**、**数据管理-操作**（管理调优数据集）、**模型观测-操作**。
