...
统一使用 `Authorization: Bearer $DASHSCOPE_API_KEY`，API Key 通过环境变量配置。详见 [获取 API Key](https://help.aliyun.com/zh/model-studio/get-api-key)。
...
- **地域一致性**：模型、endpoint、API Key 必须同地域。
