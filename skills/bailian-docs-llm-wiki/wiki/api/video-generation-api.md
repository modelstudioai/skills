...
- 图片输入须为公网可访问 URL，且格式为 JPG/PNG，尺寸建议 ≥ 512×512；人像驱动模型要求人脸占比 ≥ 30%  
- 视频生成不支持实时流式响应，返回为完整 MP4 下载链接（`output.video_url`），有效期 24 小时  
- 同一 API Key 下，免费额度仅覆盖 HappyHorse 与万相的基础调用；其余模型需单独购买配额或开通后付费  

> **注意**：原始文档未说明错误码体系与重试策略，实际开发中应捕获 `429 Too Many Requests` 与 `503 Service Unavailable` 并实现指数退避重试。
